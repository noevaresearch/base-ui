...
mod snippet_language_guard {
    ...
    /// `CONTRACT.md` requirement 1's mapping table: upstream's `Meter.Root` is this port's
    /// `<Meter::Root>`, not a flattened `<MeterRoot>`. The gap report's AST layer counts the dotted
    ...
    fn every_snippet_uses_the_namespaced_spelling() {
        for (name, text, _) in page_snippets() {
            for tag in ["Root", "Label", "Track", "Indicator", "Value"] {
                assert!(
                    text.contains(&format!("<Meter::{tag}")),
                    "the '{name}' snippet does not use the namespaced <Meter::{tag}> spelling"
                );
            }
            for flattened in [
                "<MeterRoot",
