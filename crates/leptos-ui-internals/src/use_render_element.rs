...
mod host_tests {
    ...
    // The re-homed merge fold's own suite lives in `crate::merge_props` (the
    // standalone unit's mirrors of `mergeProps.test.ts`); the tests here pin what
    // evaluation adds on top — the enabled gate, the ref fork, and the resolved
    // prop flow.
