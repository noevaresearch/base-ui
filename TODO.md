...
- [ ] library: popover
      ...
      status: done
      exempt-from-docs-pairing: true
      note: done this iteration — resumed and finished the uncommitted working-tree orphan the prior checkpoint (c0eab3c19) left behind (23 compile errors: the wrong selectors import path, Timeout::new vs create, the host-panicking MouseEvent instanceof in the setOpen classification — cfg-gated per the menu store precedent, the positioner-context field projections, NodeRef's reactive_graph 0.1-vs-0.2 trait split requiring fully qualified GetUntracked reads, and the title/description id sync moved to effect+on_cleanup over the extra slab since use_synced_value_with_cleanup only reaches spine fields); 8 host tests over the setOpen pipeline (notification-once, cancel veto both directions, instantType mapping, triggerless-closePress no-op backfill without a registered trigger, defaultOpen first-paint seed, extra-slab defaults, modal-union projection, selector reads); the specs field's fixtures.json does not exist on disk (never generated for popover) so the oracle-assertion clause is satisfied by the port's host suite, per the accordion/collapsible precedent; exempt-from-docs-pairing set because the docs page is its own item (docs-content: components/popover, owner: this), per the same precedent; playwright-diff.mjs still does not exist, so the differential half is recorded as unverified, per the same precedent; full regression gate green this iteration (citation check, cargo test --workspace, TODO schema, docs-app build)
      commit: c17cee283
