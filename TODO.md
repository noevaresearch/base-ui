...
- [ ] library: namespaced part surface (ported batch)
      ...
      review-note: MEASUREMENT TOOLING CHANGED in this iteration's own commit d2634e4be4 — ralph/driver/ralph-baseui-hermes.sh ralph/scripts/lib/snippet-lang.mjs ralph/scripts/note-tooling-change.mjs ralph/scripts/scorecard-sweep.sh . A gate edit is not self-authorising: it needs review as a tooling change (what it now measures, and whether the bar it enforces moved). Recorded by the driver so the next iteration sees it rather than inheriting a quietly different gate.
