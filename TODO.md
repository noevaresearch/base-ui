...
- [x] tooling: the CI scorecard's rendered axes are UNMEASURED on every route (and its package-alias axis contradicts a local green)
      ...
      blocked-by: []
      status: done
      commit: (this iteration's done-marking commit — the sha is recorded by the follow-up ledger commit; no crate or gate change: the instrument fix and the CI write-back landed in 84286f3dec / ff74043d9, so this iteration's own product is the re-verification, the step-7 run and the decision record)
