...
- [ ] tooling: the CI scorecard's rendered axes are UNMEASURED on every route (and its package-alias axis contradicts a local green)
      ...
      note: ALSO RECORDED, NOT CLAIMED — the first cut of the language-axis guard nulled that axis whenever ANY invariant fired, and reading my own run's numbers shows what it masked: `react/utils/merge-props` (reactToReactBlocks 1 of 5 blocks) and `react/utils/use-render` (5 of 5) read `snippet language UNMEASURED` where the gate itself raises a P0 FAIL for upstream's code on the page. Narrowed in the commit after the measurement: the SIZE axes still go UNMEASURED on a violated report, the LANGUAGE axis no longer hides a react-to-react P0 behind an UNMEASURED, and a MISSING upstream side still makes it UNMEASURED (without the reference the ≥90%-verbatim net cannot run, so `react = 0` would be a green from a comparison that never happened). That narrowing is PREDICTED, not measured: the next CI run is what turns those two routes' language axis from UNMEASURED to FAIL, and this note does not claim it.
      note: STEP 0 RECORD, written BEFORE any implementation work — CHOSEN OVER the mechanical suggestion
