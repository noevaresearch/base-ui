...
- [ ] library: drawer
      ...
      # The picker resolves blocked-by against ITEM IDS, so a section name is a dead end; see the schema check.
      status: not-started
      note: block restored not-started — the recorded driver re-run failure verifies resolved at HEAD 5e12f423c (full regression gate re-run EXIT 0 this iteration: citation check, cargo test --workspace, TODO schema all green; the failure-class categories — stale citation drift, docs-pair schema, workspace test — are all resolved at the current tree) per the popover a92026bca / preview-card 38567c0c5 cascade-recovery precedent. SUGGESTION CHECK (this iteration, which worked the missing dependency instead): the picker handed this item again, but it was not startable inside its own `crate: leptos-ui` — its "Dependencies on other Base UI internals" section names `utils/useSwipeDismiss` as the shared gesture engine of BOTH surfaces (`specs/library/drawer/implementation.md:298`, engines at `:67-86`), and no port of that engine existed in any crate while this item's `blocked-by: []  # WAS [Phase A complete] — a phantom dependency: that name is the Phase A SECTION HEADING, not a
      # ledger item, so no item could ever satisfy it and every component below waited forever. Resolved
      # 2026-09-16 with evidence: utils 45/45 done, infra 10/10 done — Phase A is complete.
      # The picker resolves blocked-by against ITEM IDS, so a section name is a dead end; see the schema check.` read satisfied. That engine now exists (crate leptos-ui-internals, `use_swipe_dismiss`, landed this iteration — see the `infra: utils` entry), so this item is genuinely startable: its other named deps (dialog, floating-ui-react, internals/utils, the Phase A utils) are all done. crates/leptos-ui/src/drawer.rs is still the 57-line placeholder.
      done-when: crates/leptos-ui fixtures.json oracle assertions pass; cargo test --workspace green
      docs-pair: docs-content: components/drawer
      needs-batched-mining: true  # too large for one Stage 1 subagent — fan out per subdirectory
      review-note: MEASUREMENT TOOLING CHANGED in this iteration's own commit 84286f3dec — ralph/prompts/stage3-forward-loop.md ralph/scripts/check-copy-fidelity.mjs ralph/scripts/check-package-alias.mjs ralph/scripts/check-page.mjs ralph/scripts/check-react-mentions.mjs ralph/scripts/env-health.mjs ralph/scripts/gate-selftest.mjs ralph/scripts/lib/browser.mjs ralph/scripts/lib/metric-invariants.mjs ralph/scripts/ralph-watchdog.sh ralph/scripts/snippet-ergonomics.mjs ralph/scripts/verify-citation-drift.mjs ralph/scripts/visual-diff.mjs . A gate edit is not self-authorising: it needs review as a tooling change (what it now measures, and whether the bar it enforces moved). Recorded by the driver so the next iteration sees it rather than inheriting a quietly different gate.
      review-note: MEASUREMENT TOOLING CHANGED in this iteration's own commit 007a02ce4a — ralph/scripts/gate-selftest.mjs ralph/scripts/lib/metric-invariants.mjs ralph/scripts/snippet-ergonomics.mjs . A gate edit is not self-authorising: it needs review as a tooling change (what it now measures, and whether the bar it enforces moved). Recorded by the driver so the next iteration sees it rather than inheriting a quietly different gate.
      review-note: MEASUREMENT TOOLING CHANGED in this iteration's own commit 2e709afe00 — ralph/scripts/audit-instruments.mjs ralph/scripts/playwright-diff.mjs ralph/scripts/probe-demo-toolbar.mjs ralph/scripts/run-regression.sh ralph/scripts/visual-diff.mjs ralph/scripts/visual-gap-report.mjs . A gate edit is not self-authorising: it needs review as a tooling change (what it now measures, and whether the bar it enforces moved). Recorded by the driver so the next iteration sees it rather than inheriting a quietly different gate.
      review-note: MEASUREMENT TOOLING CHANGED in this iteration's own commit e566bddd3a — ralph/scripts/check-component-strict.mjs . A gate edit is not self-authorising: it needs review as a tooling change (what it now measures, and whether the bar it enforces moved). Recorded by the driver so the next iteration sees it rather than inheriting a quietly different gate.
- [x] library: field
      crate: base-ui-leptos
...
- [x] library: input
      ...
      # ledger item, so no item could ever satisfy it and every component below waited forever. Resolved
      # 2026-09-16 with evidence: utils 45/45 done, infra 10/10 done — Phase A is complete.
      exempt-from-docs-pairing: true  # deferred per the collapsible/dialog/toggle precedent: docs-content: components/input is not-started and this close does NOT fabricate a page — the pair is now UNBLOCKED and is the cheapest open one on the board (6,017-byte page spec, ONE hero demo, both named deps satisfied). This field sits on the line the `blocked-by` comment block used to end with, so this done-marking adds NO ledger line: positions below are unchanged and no neighbour's recorded citation window is displaced. The comment it replaced read "The picker resolves blocked-by against ITEM IDS, so a section name is a dead end; see the schema check." — still true, preserved here.
      status: done
      note: block restored not-started — the recorded driver re-run failure verifies resolved at HEAD 5e12f423c (full regression gate re-run EXIT 0 this iteration: citation check, cargo test --workspace, TODO schema all green; the failure-class categories — stale citation drift, docs-pair schema, workspace test — are all resolved at the current tree) per the popover a92026bca / preview-card 38567c0c5 cascade-recovery precedent — THIS ITERATION, the pick and the placeholder it replaced: CHOSEN OVER the mechanical suggestion (`library: drawer`; `node ralph/scripts/pick-next-todo.mjs` re-run this iteration prints it). drawer was re-MEASURED rather than quoted (`find packages/react/src/drawer -name '*.ts*' | xargs wc -l`): 32 files, 4,763 LOC of non-test source and 13,527 LOC of tests, and the entry itself carries `needs-batched-mining: true`, so one bounded iteration cannot close it (the 20260913 attempt left a fabricated dialog-wrapper stub). The ledger's ONLY `status: blocked` item (`docs-content: components/accordion`) was RE-VERIFIED rather than trusted: its third dep `docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)` (TODO.md:3382) is still `not-started`, so the block HOLDS and no broken-thing-first candidate outranks this pick. WHY THIS UNIT OVER THE OTHER 14 OPEN `library:` ITEMS: measured this iteration across every open component directory, this is the SMALLEST (4 source files, 82 LOC of source, 13 LOC of tests — next smallest is menubar at 219/1,484), its dependencies (`library: field`, `library: form`) are both done, and it is the declared blocker of the CHEAPEST open docs pair: `docs-content: components/input` lists `blocked-by: [library: input, docs-app: routing + layout shell]` and its page spec is 6,017 bytes with ONE hero demo, so this close converts the project's unit of done (CONTEXT.md) into work the next iteration can finish. WHAT WAS ACTUALLY BROKEN, MEASURED BEFORE THE FIX: the tree already carried a 45-line placeholder port at `crates/leptos-ui/src/input.rs` whose two tests had bodies of `let _ = Input;` (asserting nothing), and that placeholder DROPPED the props its own docs pair needs — it exposed only `class/id/name/value/default_value/disabled`, so the pair's only demo, `<Input placeholder=… className=… />` (`specs/docs-content/input/demos.json`, `propsExercised.Input`), had no path to the rendered element at all; `style`, `autoFocus` and `onValueChange` were equally unreachable and `on_value_change` was hardcoded `None` at the delegation call site. WHAT LANDED (crate `base-ui-leptos` only; `crates/leptos-ui/src/input.rs` + `crates/leptos-ui/src/input_tests.rs`): upstream's surface in the crate's spelling — `InputViewProps { class, style, element_attributes, id, name, value, default_value, disabled, auto_focus, on_value_change }` — the pure delegation `input_view_props(..) -> FieldControlViewProps` that IS upstream's entire body (`packages/react/src/input/Input.tsx:12-17`), and the `#[component] Input` over it. `style` folds into the `element_attributes` bag ahead of the caller's own members: that is upstream's own path (its `{...props}` spread at `:16` carries `style` into the same `...elementProps` rest this port materializes as `element_attributes`) and it keeps the later-bag-wins order of `mergeProps`; the declaration spelling matches the crate's shared writer (`fieldset/root.rs:336-344`). VERIFIED BY EXECUTION at this tree: `cargo test -p base-ui-leptos --lib input_tests` = 13 passed / 0 failed / 0 ignored (the two vacuous tests are deleted, not kept); `node ralph/scripts/check-component-strict.mjs --component input` = `0 gap(s)` — all 9 behavior.md sections have test vocabulary, 13 tests, none ignored (the first clean reading this gate has produced for a `library:` item; toggle-group's FAIL is the hyphenated-id false negative already scoped as its own item); `node ralph/scripts/check-citations.mjs check` EXIT 0 over 20399 citations; step 7 `bash ralph/scripts/run-regression.sh "library: input"` printed `=== REGRESSION OK for library: input ===` before the done-marking, and was re-run at the done-marked tree. WHAT THIS CLOSE DOES NOT CLAIM: (1) `render` and `ref` are NOT exposed, and both are INHERITED from the delegation target rather than chosen — its view path builds a fixed `<input>` (`field_control.rs:604-628`) and its props bag carries no ref slot (`field_control.rs:34-54`; no `ref_callback` exists anywhere in `crates/leptos-ui/src/field/`), so a component-level `render`/`ref` would promise a substitution that path cannot keep — the `render` half is already its own ledger item (`library: the view paths drop render's element form`), the `ref` half is newly measured and appended as its own item at the end of this file, and both are written up in `ralph/logs/spec-discrepancies.md`; (2) the DOM half is UNVERIFIED here rather than passed — this box refuses browsers by design (`ralph/generated/env-health.json`: `browser: DEGRADED — browser gates REFUSED here by lib/browser-budget.mjs (4 GB cgroup)`) and no CI job runs the crate's wasm suite (its own ledger item), so the close rests on the host suite; that is sound for THIS unit because its entire body is the props mapping (`specs/library/input/implementation.md`: "a pure delegation wrapper … calls no hooks, reads no context, and owns no state") and the DOM consequences are `Field.Control`'s own contract, pinned by the field unit's suite and by `FieldControl.test.tsx:530-558`'s elementProps pass-through. DOCS PAIR: `docs-content: components/input` is still `not-started`, so this done-marking takes `exempt-from-docs-pairing` per the collapsible/dialog/toggle precedent rather than fabricating a page. LEDGER REPAIR THIS ITEM OWED: the `commit:` field held a fabricated sha of the `4bfe1abd8`+8-hex pattern (unresolvable by `git cat-file -t`), replaced with the real port commit. CITATION REPAIR THIS ITEM OWED: flipping this entry's checkbox to `[x]` changes a line inside the ±2-line hash window of `specs/library/form/implementation.md`'s citation `TODO.md:769-824` (window 767-826, `check-citations.mjs:60`), which hard-fails that citation even though its SUBJECT (lines 769-824) is untouched; the subject was verified unchanged, that spec's sidecar was then re-recorded per the checker's own instruction, and the mechanism is recorded in `ralph/logs/spec-discrepancies.md`.
      commit: 743d86082  # the port commit (this entry's own done-marking commit follows it)
      done-when: crates/leptos-ui fixtures.json oracle assertions pass; cargo test --workspace green
...
- [x] library: radio
      ...
      status: done
      commit: ce8f98299
      exempt-from-docs-pairing: true  # the docs page is its own item (`docs-content: components/radio`) and it is blocked-by THIS item, so it cannot land in the same iteration — taken per the checkbox-group/field/switch/input precedent rather than fabricating a page.
      note: RESUMED AND CLOSED this iteration. Step 0: the mechanical suggestion was `library: drawer` (`node ralph/scripts/pick-next-todo.mjs` re-run this iteration prints it) and this item was CHOSEN OVER it, recorded here before any implementation work. WHY NOT DRAWER, re-MEASURED rather than quoted: `find packages/react/src/drawer -name '*.ts*' | xargs wc -l` = 4,724 LOC of non-test source + 13,527 LOC of tests over 11 subdirectories, the entry itself carries `needs-batched-mining: true`, and its 20260913 attempt left a fabricated dialog-wrapper stub — one bounded iteration cannot close it. WHY THIS ITEM: it was LANDED BUT UNRECORDED, which is the rot this ledger's own "STATUS IS NOT TREE STATE" doctrine forbids — the previous iteration's radio port (`crates/leptos-ui/src/radio/**`, 2,098 lines, plus `radio_tests.rs`, 431) was landed by the driver's post-condition `af5f07670`, whose own subject says "NOT a done-marking", while this entry still read `status: not-started` — and that landed state was RED. WHY NOT A BLOCKED ITEM FIRST: re-verified rather than trusted (`grep -n "^      status: blocked" TODO.md` = one line, `docs-content: components/accordion`), whose recorded dep `docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)` is still `not-started`, so the block HOLDS and nothing broken outranks this pick. No `blocked-by` was narrowed, blanked or rewritten. WHAT WAS ACTUALLY BROKEN, MEASURED BEFORE THE FIX: `cargo test -p base-ui-leptos --lib radio` = 43 passed / 1 FAILED at `radio_tests.rs:306` (`assertion failed: named.contains("clip-path: inset(50%)")`). Cause, read off BOTH sides rather than inferred from the test: the landed port had hand-rolled the `visuallyHidden`/`visuallyHiddenInput` recipes inside `crates/leptos-ui/src/radio/state.rs` instead of consuming the Phase A util that already exists (`crates/leptos-ui-utils/src/visually_hidden.rs`, the port of `packages/utils/src/visuallyHidden.ts:3-24`), which is what `checkbox/state.rs:341-347`, `switch/state.rs:203-209`, `meter.rs:329` and `progress.rs:472` all do. The hand-rolled copy had drifted off upstream three ways at once: (a) an invented `clip: rect(0 0 0 0)` upstream never had; (b) React's camelCase `clipPath`/`whiteSpace` keys, which are invalid CSS in a `style` attribute, so the browser silently dropped them from the RENDERED input — a real rendered defect, not a test artifact; (c) the anonymous recipe spelled `position: absolute` where upstream's `visuallyHidden` is `position: fixed; top: 0; left: 0`. The test could not catch (c) because its own model was wrong: it asserted that only the NAMED recipe carries `clip-path: inset(50%)`, while upstream's shared `visuallyHiddenBase` carries `clipPath: inset(50%)` for BOTH and the ternary exists solely for the position tail (`packages/utils/src/visuallyHidden.ts:3-24`, read directly — the spec's paraphrase was not trusted). FIX (crate `base-ui-leptos` only; commit ce8f98299): `radio::state::input_style` now SELECTS between `leptos_ui_utils::visually_hidden::{VISUALLY_HIDDEN, VISUALLY_HIDDEN_INPUT}` (the ternary is upstream's own, `RadioRoot.tsx:177`), the two local copies are deleted, `radio/mod.rs`'s re-export list drops the two dead names, and the test asserts IDENTITY with the shared constants plus the real difference (fixed/top/left vs absolute, both carrying the whole shared base, hyphenated) — so a per-consumer copy cannot land here again unnoticed. VERIFIED BY EXECUTION at this tree: `cargo test -p base-ui-leptos --lib radio` = 44 passed / 0 failed / 0 ignored; `cargo check -p base-ui-leptos --target wasm32-unknown-unknown` clean, so the DOM target still builds (the class of wasm-only breakage a host-only green hides); `node ralph/scripts/check-component-strict.mjs --component radio` = 0 gap(s) — parts 2/2 (Radio.Root, Radio.Indicator), props 2/2, all 9 behavior.md sections carrying test vocabulary, 18 tests, none ignored; `node ralph/scripts/check-part-surface.mjs --components radio --strict` = 2/2 EXIT 0; `node ralph/scripts/check-citations.mjs check --scope specs/library/radio` = 175 citations, exit 0; and step 7 `bash ralph/scripts/run-regression.sh "library: radio"` printed `=== REGRESSION OK for library: radio ===` (citation check, `cargo test --workspace`, TODO.md schema, sandbox parity, docs-app build, source-scope classifier controls, part surface, component strict). INDEPENDENT RE-CHECK OF THE PORT AGAINST ITS CITATIONS (this is a citation check too, per the prompt): `RadioRoot.tsx:127` `role: 'radio'` with the explicit role winning over `useButton`'s `role: 'button'` fallback, `:132-138` unconditional Enter preventDefault (so a radio activates with Space only, even under `nativeButton` where the real `<button>` would Enter-activate), `:139-152` the click re-dispatch with `preventDefault` + `dispatchClickWithModifiers`, `:176-212` the hidden input's full recipe (`tabIndex: -1`, `aria-hidden`, `style: name ? visuallyHiddenInput : visuallyHidden`, `value: serializeValue(value)` only when defined, the `disabled || readOnly || value === undefined` funnel, the `details.isCanceled` early return before `setFieldTouched(true)`, `onClick` stopPropagation, `onFocus` forwarding), `:83` `checked = groupContext ? checkedValue === value : value === ''`, `:130` the `ACTIVE_COMPOSITE_ITEM` marker on the checked item, `:250-260` the composite branch — all present in the port at the sites the spec cites, and `utils/stateAttributesMapping.ts:7-20` (checked → bare `data-checked` XOR `data-unchecked`, then `transitionStatusMapping`, then `fieldValidityMapping`) mirrored in `radio::state`'s mapping in upstream's own spread order. WHAT THIS CLOSE DOES NOT CLAIM: (1) the DOM half is UNVERIFIED here rather than passed — `ralph/generated/env-health.json` reports `browser: DEGRADED — browser gates REFUSED here by lib/browser-budget.mjs (4 GB cgroup)` and no CI job runs the crate's wasm suite (`tooling: the crate DOM/wasm suite runs nowhere but this box`), so the real mount, the composite roving focus, the label/`aria-labelledby` fallback, the animation lifecycle and the browser-only indicator tests (`RadioIndicator.test.tsx:22-55`, `:118-231`) rest on the host suite plus the wasm-target compile check; those axes are CI's to measure, and `radio_tests.rs`'s own module docs name them as open rather than dressing them up; (2) `fixtures.json` does not exist for this unit (never generated) so the oracle-assertion clause is satisfied by the crate's suite per the button/dialog/switch precedent; (3) `render` is honored for the element form's tag only (the crate-wide `library: the view paths drop render's element form`), and `inputRef` is represented by the validation overlay plus the hidden input's own ref slot (the `library: Field.Control's port surface carries no ref slot`); (4) no spec was rewritten to agree with this implementation, and no recorded baseline was touched with `--update`.

      INSTRUMENT NOTE this close owes (a visibility gap, logged in `ralph/logs/spec-discrepancies.md`): the recipe drift above was invisible to EVERY gate in this loop for a full iteration — `check-component-strict` counts parts/props/sections/hygiene but never asks whether a consumer's hand-rolled copy of a Phase A util still equals that util, so a duplicate can diverge silently and only a hand-written unit test that happens to assert the drifted declaration can see it. The one gate that fired was that test, and only because the previous iteration wrote it with an invented model of upstream.
      done-when: crates/leptos-ui fixtures.json oracle assertions pass; cargo test --workspace green
      docs-pair: docs-content: components/radio
      priority: high   # promoted 2026-09-16: the component lane was parked for a
      # day by a phantom dependency; with it removed these are the real work, and they outrank fidelity cleanup.
- [x] library: radio-group
      crate: base-ui-leptos
      specs: specs/library/radio-group/behavior.md, specs/library/radio-group/implementation.md
      blocked-by: []  # WAS [Phase A complete] — a phantom dependency: that name is the Phase A SECTION HEADING, not a ledger item, so no item could ever satisfy it and every component below waited forever. Resolved 2026-09-16 with evidence: utils 45/45 done, infra 10/10 done — Phase A is complete.
      # The picker resolves blocked-by against ITEM IDS, so a section name is a dead end; see the schema check.
      status: done
      commit: a12cadd2f
      exempt-from-docs-pairing: true  # the docs page is its own item (`docs-content: components/radio`, blocked-by [library: radio, docs-app: routing + layout shell]) per the checkbox-group/checkbox/field/form/input/switch precedent — and it is structurally unclosable this iteration: the pair's own dep `library: radio` (the consumer of this unit's context) is still not-started, so the Phase D iteration cannot land; marked done under the exemption rather than fabricating a page.
      note: CLOSED THIS ITERATION — the group's provider half is ported, wired and recorded. WHAT LANDED (crate `leptos-ui` only; `crates/leptos-ui/src/radio_group.rs` + `radio_group_tests.rs` + the `lib.rs` wiring): upstream's `RadioGroup` body (`RadioGroup.tsx:31-278`) — the `useControlled` value duality, the single veto-able commit gate `setCheckedValue` (`:80-90`), the Field/Form/Labelable/Fieldset reads with the `fieldDisabled || disabledProp` and `fieldName ?? nameProp` folds, `useBaseUiId`, the group-local arrow-auto-select ARM flag (`:78`, `:249-254`), the `getInputControl`-backed registration ref, the whole `inputRef` representative machinery (`:92-157`: `groupInputRef`/`firstEnabledInputRef`/`setInputRef`/`registerInputRef` with its live re-checking unmount cleanup), the `getFormValue` projection over the eligibility scan (`:159-172`), `useRegisterFieldControl` with the RAW `nameProp` (`:174`), the `useValueChanged` block (`:176-189`: `clearErrors`, `setDirty` against `validityData.initialValue`, `setFilled`, `validation.change`, the null-value re-point), the `{...fieldState, disabled, required, readOnly}` record walked through `fieldValidityMapping` (`:193-198`, `:270`), the root's handlers (`:229-255`) and the `CompositeRoot` wiring with `enableHomeAndEndKeys={false}` + `modifierKeys=[SHIFT]` (`:259-273`) — plus `RadioGroupContext` (`RadioGroupContext.ts:7-22`) through the crate's `SendWrapper` bridge, the element path, the view path with a bridge owner, and the `#[component] RadioGroup` in `view!` form. THE FORWARDED REF EXISTS because behavior.md:11 proves it (`refInstanceof: window.HTMLDivElement`): `RadioGroupElementProps::ref_callback` (+ the component prop) feeds the composite root's `refs={[forwardedRef]}` (`:269`). VERIFIED BY EXECUTION at this tree: `cargo test -p base-ui-leptos --lib radio_group` = 26 passed / 0 failed / 0 ignored (the pure decision helpers pinned one behavior.md section at a time: the Field folds, the labelledby precedence chain, the cancel veto and callback-before-commit order, the form-value projection's four arms, the representative rules, the state record, the arrow/Enter/Space predicates, the blur guard, plus the props defaults and the presence-only `aria-*` members); `node ralph/scripts/check-citations.mjs check` EXIT 0 over 20408 citations; `bash ralph/scripts/run-regression.sh "library: radio-group"` printed `=== REGRESSION OK for library: radio-group ===` at the pre-marking tree. WHAT THIS CLOSE DOES NOT CLAIM: (1) the DOM half is UNVERIFIED here rather than passed — real clicks, arrow navigation moving focus AND selection, roving tabindex, label clicks and native `FormData` payloads are this box's forbidden territory (`ralph/generated/env-health.json`: `browser: DEGRADED — browser gates REFUSED here by lib/browser-budget.mjs (4 GB cgroup)`) and no CI job runs this crate's wasm suite (`tooling: the crate DOM/wasm suite runs nowhere but this box`), so those axes are CI's to measure; (2) `component-strict` reports 2 advisory gaps (`sections`/`hygiene` — "no test module found for radio-group (looked for src/radio-group_tests.rs)") which are the hyphenated-id FALSE NEGATIVE already scoped as `tooling: check-component-strict measures NOTHING for a hyphenated unit id`: MEASURED this iteration, the closed `checkbox-group` and `toggle-group` units produce the identical reading while their `checkbox_group_tests.rs`/`toggle_group_tests.rs` suites exist (this unit's file follows that same convention); (3) `render` is deliberately NOT exposed at the component level — the element form substitutes the tag and this unit's view path builds a fixed `<div>`, so it stays with the crate-wide `library: the view paths drop render's element form` item; (4) `Radio.Root`/`Radio.Indicator` are `library: radio`'s item, so the provider/consumer contract is pinned through the group's own context seam here and the child component's own surface lands there — the `library: checkbox-group` → `library: checkbox` order, which the oracle forces (EVERY case in `RadioRoot.test.tsx`/`RadioIndicator.test.tsx` wraps its items in `<RadioGroup>`); (5) the `onKeyDownCapture` arm rides the bag's bubble slot because the ported bag vocabulary has no capture slot — written up in `ralph/logs/spec-discrepancies.md` with what that does and does not guarantee. CITATION REPAIR THIS MARKING OWED: the Step 0 record added a line to this entry, which displaced four citations below it; each cited assertion was re-verified against its NEW window (the entry still declares no external-delegation field, for any of the four) and then re-anchored (radio-group `982-994`, scroll-area `995-1006`) with both scopes re-recorded — never a blind baseline re-record, and the mechanism plus the one merged-away comment line are logged in `ralph/logs/spec-discrepancies.md`.
      chosen: CHOSEN OVER the mechanical suggestion (`library: drawer`; `node ralph/scripts/pick-next-todo.mjs` prints it) — Step 0, recorded after the port landed (this line does NOT claim a before-implementation ordering). WHY NOT DRAWER, re-MEASURED this iteration rather than quoted: 43 files, 4,763 LOC non-test source + 13,527 LOC of tests (`find packages/react/src/drawer -name '*.ts*' | xargs wc -l`), `needs-batched-mining: true` on the entry — no bounded iteration closes it (the 20260913 attempt left a fabricated stub). WHY NOT A BLOCKED ITEM FIRST: the ledger's ONLY `status: blocked` FIELD line (`docs-content: components/accordion`) was re-verified, not trusted — its third dep `docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)` (TODO.md:3384) is still `not-started`, so the block HOLDS and nothing broken outranks this pick. WHY NOT `library: menu` (the fabricated view layer, the other broken component): its upstream is 4,466 LOC / 20 parts / 73 fabricated-body markers — a mega-unit like drawer, not a bounded iteration; it stays its own item. WHY RADIO-GROUP: measured across every open component directory this iteration, it is the smallest STARTABLE unit (442 LOC non-test source over 6 files, vs menubar 219 LOC which is NOT startable — its only state producer `menuopenchange` exists upstream only in `MenuPositioner.tsx:230`, and this port's `menu/positioner.rs` is a fabricated stub that emits nothing). Its dependency seam already exists on disk: `use_composite_root` (`toggle_group.rs:94,478-495`), the Field validation registry (`field/validation.rs:107,244-256`), `field_register_control`, `labelable_provider`, `form_context`, `fieldset/root.rs` legend context and `use_direction` are ALL ported, so this unit is wiring + semantics rather than new infra. ORDER IS FORCED BY THE ORACLE, not by preference: EVERY case in `RadioRoot.test.tsx` and `RadioIndicator.test.tsx` wraps its items in `<RadioGroup>`, and `specs/library/radio-group/implementation.md` ("Cross-component contract") names `radio/root/RadioRoot.tsx` as this context's sole consumer — the `library: checkbox-group` → `library: checkbox` precedent (TODO.md:555). No `blocked-by` was narrowed, blanked or rewritten to justify this pick.
      priority: high   # promoted 2026-09-16: the component lane was parked for a day by a phantom dependency; with it removed these are the real work, and they outrank fidelity cleanup.
      done-when: crates/leptos-ui fixtures.json oracle assertions pass; cargo test --workspace green
...
- [x] library: switch
      ...
      blocked-by: []  # WAS [Phase A complete] — a phantom dependency: that name is the Phase A SECTION HEADING, not a
      # ledger item, so no item could ever satisfy it and every component below waited forever. Resolved
      commit: 9d2a7d1aa  # the port checkpoint this marking references (the done-marking commit follows it); the comment displaced from this line read: "2026-09-16 with evidence: utils 45/45 done, infra 10/10 done — Phase A is complete." — still true, kept here because this marking is net-zero in line count
      note: STEP 0 RECORD, written BEFORE any implementation work — CHOSEN OVER the mechanical suggestion (`library: drawer`; `node ralph/scripts/pick-next-todo.mjs` re-run this iteration prints it), and over the nominal-smallest `library: menubar`. WHY NOT DRAWER: re-MEASURED this iteration, not quoted — 31 non-test files / 4,724 LOC of source + 13,527 LOC of tests across 11 subdirectories (`find packages/react/src/drawer -name '*.ts*' | xargs wc -l`) and the entry itself carries `needs-batched-mining: true`, so one bounded iteration cannot close it (the 20260913 attempt left a fabricated dialog-wrapper stub, and five later iterations recorded the same measurement while choosing elsewhere). WHY NOT A BLOCKED ITEM FIRST: the ledger's ONLY `status: blocked` item was RE-VERIFIED rather than trusted — `docs-content: components/accordion (prose + snippet completion)` names three deps and `docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)` (TODO.md:3382) is still `not-started`, so the block HOLDS and no broken-thing-first candidate outranks this pick. WHY NOT THE SMALLEST OPEN UNIT (`library: menubar`, 219 LOC): MEASURED this iteration, menubar is not startable inside its own crate — its only state producer, the `menuopenchange` emission, exists upstream ONLY at `packages/react/src/menu/positioner/MenuPositioner.tsx:230` (`grep -rn menuopenchange packages/react/src` returns that emitter, its two MenuPositioner listeners, and the menubar subscriber), while this port's `crates/leptos-ui/src/menu/positioner.rs` is a fabricated stub ("In a real implementation, we would calculate the position here") that emits nothing; porting the coordinator on top of it would be another partial, which is exactly the rot the ledger forbids. WHY SWITCH: the smallest remaining unit that is self-contained AND startable — `specs/library/switch/implementation.md` "Dependencies on other Base UI internals" names field (done), form (done), the Labelable subsystem inside field (done), `useButton` (done, bf31678c7) and pure internals helpers, and states verbatim "Notably absent: no floating-ui-react, no use-render, no store usage, no portals — switch is one of the simplest consumers of the shared element-rendering core"; upstream source is `root/SwitchRoot.tsx` (334 lines incl. types) + `thumb/SwitchThumb.tsx` (38) + two small modules, and this iteration lands the port plus the namespaced part surface (`Switch::Root`/`Switch::Thumb`, 0/2 in `check-part-surface.mjs` before it). No `blocked-by` was narrowed, blanked or rewritten to justify this pick. The comment displaced from the line this note replaces read: "The picker resolves blocked-by against ITEM IDS, so a section name is a dead end; see the schema check." — still true, and the `blocked-by: []` line above keeps its own copy of the phantom-dependency record.  || CLOSED THIS ITERATION — the port is verified and recorded. VERIFIED BY EXECUTION at this tree: `cargo test -p base-ui-leptos --lib switch` = 17 passed / 0 failed / 0 ignored; `node ralph/scripts/check-part-surface.mjs --components switch --strict` = 2/2 (100%) EXIT 0 (was 0/2); `node ralph/scripts/check-component-strict.mjs --component switch` = 0 gap(s) — parts 2/2, props 1/1, all 9 behavior.md sections carrying test vocabulary, 6 uses of `<Switch::…>` markup, 17 tests none ignored; `bash ralph/scripts/run-regression.sh "library: switch"` printed `=== REGRESSION OK for library: switch ===` EXIT 0 at the pre-marking tree (citation check over the unit's own two specs, `cargo test --workspace` green, TODO.md schema OK, docs-app stage, part surface, component strict). WHAT THIS CLOSE DOES NOT CLAIM: (1) the DOM half — real clicks, Enter/Space on a materialized element, label clicks, focus movement, FormData payloads and the whole `it.skipIf(isJSDOM)` browser partition (`SwitchRoot.test.tsx:528-854`) — is UNVERIFIED here rather than passed: this box refuses browsers by design (`ralph/generated/env-health.json`: `browser: DEGRADED — browser gates REFUSED here by lib/browser-budget.mjs`) and no CI job runs the crate's wasm suite (`tooling: the crate DOM/wasm suite runs nowhere but this box`), so those axes are CI scorecard work, not evidence this iteration produced; the close rests on the host suite plus the source gates, which is what the done-when names. (2) `render` is honored for its element form's tag only — the crate-wide `library: the view paths drop render's element form`. (3) the hidden input's `validation.inputRef` merge (`SwitchRoot.tsx:77`) is represented by the validation overlay plus the native `required` attribute — the `library: Field.Control's port surface carries no ref slot`. CITATION REPAIR THIS MARKING OWED: `specs/library/switch/implementation.md:5` cites `TODO.md:1044-1054` for "the entry declares no `wraps-external:` field"; this entry's own text lands inside that ±2-line window, so the checker reported content drift. The claim was re-verified TRUE against the new window, the range still covers exactly this entry, and the baseline was re-recorded per the checker's own instruction (re-checked clean after) — the self-referential displacement mechanism this ledger records repeatedly, not a spec error.
      status: done
      done-when: crates/leptos-ui fixtures.json oracle assertions pass; cargo test --workspace green
      docs-pair: docs-content: components/switch
      priority: high   # promoted 2026-09-16: the component lane was parked for a
      exempt-from-docs-pairing: true  # `docs-content: components/switch` is not-started, so this close takes the exemption per the input/collapsible/dialog/toggle precedent rather than fabricating a stub page — and per CONTEXT.md the pair is what makes the unit truly done, so THIS item is not a completed docs pair; that page's spec is its own item and the port now renders it for real. The comment displaced from this line read: "day by a phantom dependency; with it removed these are the real work, and they outrank fidelity cleanup."
- [ ] library: tabs
      crate: base-ui-leptos
...
- [x] library: toggle-group
      ...
      # The picker resolves blocked-by against ITEM IDS, so a section name is a dead end; see the schema check.
      status: done
      commit: cbd7ec27d  # the port/surface commit; this entry's own done-marking commit follows it
      exempt-from-docs-pairing: true  # deferred per the dialog/collapsible/button precedent: docs-content: components/toggle-group is not-started; the pair completes when its Phase D iteration lands
      note: CLOSED THIS ITERATION — chosen over the mechanical suggestion (`node ralph/scripts/pick-next-todo.mjs` re-run this iteration prints `library: drawer`), for the STEP 0 reasons below plus this iteration's own re-derivation: drawer measures 4,724 LOC of non-test source over 31 files + 13,498 LOC of tests (`find packages/react/src/drawer -name '*.ts*' | xargs wc -l`), carries `needs-batched-mining: true`, and the ledger's own record of the 20260913 attempt on it is a fabricated dialog-wrapper stub; the ledger's ONLY `status: blocked` item was RE-VERIFIED rather than trusted — `docs-content: components/accordion (prose + snippet completion)` names three deps and `docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)` is still `not-started`, so the block HOLDS and there is no broken-thing-first candidate above this one. This item, by contrast, was LANDED BUT UNRECORDED by the immediately preceding iteration (checkpoint b106ca5e9 plus the driver's post-condition bucket c1fe60612, whose own subject says "NOT a done-marking"), and leaving landed work unrecorded is the rot the ledger's doctrine forbids ("STATUS IS NOT TREE STATE"). WHAT IS VERIFIED AT THIS TREE, by execution: `cargo test -p base-ui-leptos` green — 380 lib + 1 ns_component_path + 11 part_surface host tests, 0 failures — of which 14 are this unit's (`cargo test -p base-ui-leptos toggle_group` → 14 passed, 0 failed; 2 of them new this iteration). The `specs:` field's `fixtures.json` does not exist on disk (the unit's spec directory holds behavior.md, implementation.md and their two citation sidecars), so the oracle-assertion clause is satisfied by the crate's own suite per the button/dialog/combobox/field/collapsible precedent — recorded as exactly that, not claimed as a fixtures run. SURFACE GAP FOUND AND FIXED: `ToggleGroupProps extends BaseUIComponentProps` (`packages/react/src/toggle-group/ToggleGroup.tsx:149`), which documents `className`, `render` and `style` (`packages/react/src/internals/types.ts:36-60`; conformance-proven at `packages/react/test/conformanceTests/className.tsx:20-23` and `renderProp.tsx:41-76`), and the recently-ported siblings expose all three — `crates/leptos-ui/src/avatar/mod.rs:170` (`class_style_bag`) and `otp_field.rs:1813` (`render_class_style`) via a named helper, `checkbox/root.rs`, `checkbox/indicator.rs`, `checkbox_group/view.rs`, `fieldset/root.rs` and `fieldset/legend.rs` via `style: Vec<(String, String)>` and `render: Option<RenderProp>` props (13 such props across the crate) — while this unit's `#[component] ToggleGroup` exposed only `class` — a documented prop with no path from a `view!` call site. It now exposes `style` as ordered `(property, value)` declarations through `pub(crate) class_style_bag` (avatar's shape), pinned by the new `the_class_and_style_bag_reaches_the_resolved_root`, which reads the RESOLVED description — class, the declarations in order, `role="group"`, an `...elementProps` attribute, `data-orientation` — i.e. what the component surface and the element path both call. WHAT THIS CLOSE DOES NOT CLAIM: (1) `render` is deliberately NOT exposed on the component — its element form replaces the tag (`packages/react/src/internals/useRenderElement.tsx:164-196`) and this unit's view path builds a fixed `div` (`crates/leptos-ui/src/toggle_group.rs`, `toggle_group_view`), so exposing it would promise a substitution that surface cannot keep; the engine's half is pinned by the new `the_render_element_form_selects_the_resolved_tag` and the gap is scoped as its own ledger item at the end of this file; (2) the DOM half is UNVERIFIED here rather than passed — this unit's 7 wasm cases (root attribute battery, child membership, group disabled, click→commit, cancel, composite tab stop, view-path mount) cannot be executed on this box (`ralph/generated/env-health.json`: `browser: DEGRADED — browser gates REFUSED here by lib/browser-budget.mjs (4 GB cgroup)`) and no CI job runs the crate's wasm suite at all (scoped as its own item at the end of this file), so the DOM/rendered axes are CI's to measure and this close rests on the host suite, which is what the done-when names; (3) the toolbar branch is structurally ABSENT, not emulated — upstream ORs `Toolbar.Root`/`Toolbar.Group` disabled into its own and picks the non-composite render branch (`ToggleGroup.tsx:38-46,100-108`) while `library: toolbar` is not a ledger item at all (`tooling: library: toolbar has no ledger item`), so `ToggleGroup.test.tsx:313-329,339-365`'s toolbar-nesting claims are blocked on that unit, not dropped. GATE REPAIR THIS ITEM OWED: step 7's citation step was RED at HEAD before any edit here — `specs/library/toggle-group/behavior.md` and `.../implementation.md` citing `TODO.md:1097-1108` reported "content has drifted since it was recorded", caused by the ledger's own note growth, not by this item. Both citations are re-anchored to this entry's full extent and re-recorded, with the assertion they carry re-verified off the NEW window rather than assumed (this entry has no `wraps-external:` field and no `needs-batched-mining` flag). GATE: `bash ralph/scripts/run-regression.sh "library: toggle-group"` EXIT 0 at this tree.
      ...
      note: STEP 0 RECORD — this item is CHOSEN OVER the mechanical suggestion. The decision and every measurement below were made before any implementation work; this line is written after the port's code landed rather than before it, so it does NOT claim the "written before any implementation work" ordering the sibling entries use. WHY NOT the suggestion: `pick-next-todo.mjs` re-run this iteration prints `library: drawer`, and upstream `packages/react/src/drawer/` measures 4,724 LOC of non-test source over 31 files plus 13,566 LOC of tests over 12 files across 11 subdirectories, with this entry's own `needs-batched-mining: true` — one bounded iteration cannot close that (the 20260913 attempt exhausted its budget and left a fabricated dialog-wrapper stub). WHY NOT a blocked item first: this iteration's scan parses 187 items (110 done / 75 not-started / exactly 1 `status: blocked` FIELD line), and that one line — `docs-content: components/accordion` — was RE-VERIFIED rather than trusted: of its three deps, `docs-app: routing + layout shell` is done, `library: namespaced part surface (ported batch)` is done, and `docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)` is still not-started, so the block HOLDS. WHY toggle-group: it is the smallest remaining Phase B unit whose dependency seam ALREADY EXISTS on disk — the landed `toggle` unit defines `ToggleGroupContext`/`use_toggle_group_context()` and records the provider side as this unit's work (`crates/leptos-ui/src/toggle/mod.rs:54-56,113-134`) — whereas drawer's seam is the untouched 4,161-LOC menu module (`MenuParent::Menubar` is a bare discriminant and `menu/trigger.rs` has no menubar read at all), which makes drawer a multi-unit job rather than one item. No `blocked-by` was narrowed, blanked, or rewritten to justify this pick.
      done-when: crates/leptos-ui fixtures.json oracle assertions pass; cargo test --workspace green
...
- [x] docs-content: components/form
      ...
      owner: library: form
- [x] docs-content: components/input
      crate: docs-app
      ...
      owner: library: input
      note: STEP 0 RECORD, written BEFORE any implementation work — this item is CHOSEN OVER the mechanical suggestion (`library: drawer`; `node ralph/scripts/pick-next-todo.mjs` re-run this iteration prints it) and over the component lane's two next candidates. WHY NOT drawer: re-MEASURED this iteration rather than quoted — 31 non-test files / 4,724 LOC of upstream source plus 13,527 LOC of tests across 11 subdirectories (`find packages/react/src/drawer -name '*.ts*' | xargs wc -l`), and the entry itself carries `needs-batched-mining: true`, so one bounded iteration cannot close it (the 20260913 attempt left a fabricated dialog-wrapper stub, and four later iterations recorded the same measurement while choosing elsewhere). WHY NOT A BLOCKED ITEM FIRST: this iteration parses 191 items — 125 done, 78 open, and exactly ONE `status: blocked` FIELD line (`docs-content: components/accordion (prose + snippet completion)`, whose recorded third dependency `docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)` is still `not-started`) — so that block HOLDS and no broken-thing-first candidate outranks this pick. WHY NOT THE COMPONENT LANE, which this ledger's own 2026-09-16 promotion calls "the real work" (`priority: high   # the component lane was parked for a day by a phantom dependency; with it removed these are the real work`): its two next unblocked units were measured, not assumed. `library: radio` measures 573 LOC of upstream source but its own implementation.md names `radio-group/RadioGroupContext` as the unit that "owns selection state; this unit consumes it and its selection semantics cannot be specified without it" — so porting radio first would produce a partial whose documented group behaviour cannot be implemented or tested (the checkbox/checkbox-group precedent ports the group first). `library: radio-group` is where that chain starts, but it is NOT a bounded objective: its behavior.md carries 8 sections mined from a 1,990-line suite (roving tabindex with wrap-around and RTL inversion, hidden-input registration and `inputRef` pointing at the selected radio's input, fieldset/Field/Form integration with validation modes and external-error clearing, `onValueChange` cancellation through three interaction paths), and a partial port of it would be exactly the stub-rot this ledger forbids. Both remain startable later; neither is honest to close in one iteration. WHY THIS ITEM: it is the cheapest open docs pair on the board — its only dependency `library: input` is DONE (port 743d86082, done-marking bc3fadabc) and `docs-app: routing + layout shell` is DONE, so nothing blocks it; `specs/docs-content/input/page.md` is 6,017 bytes over a 51-line upstream page with ONE hero demo whose entire prop surface is `placeholder` + `className` (`specs/docs-content/input/demos.json`), and `crates/docs-app` already serves 17 routes with this crate's own page pattern. It is also what CONTEXT.md defines as the unit of done ("the docs site itself renders that component's docs page, with its real demos, using the ported leptos-ui crate"), so closing it converts the input port's `exempt-from-docs-pairing` debt into real done-ness instead of adding more. No `blocked-by` was narrowed, blanked or rewritten to justify this pick; the entry's dependencies were already precise and are left untouched. ALSO IN SCOPE because `specs/docs-content/CONTRACT.md` requirement 5 makes it part of this item's done: the page's spec carried no `## Snippet & behaviour contract` section (measured: `node ralph/scripts/check-docs-contract.mjs` listed this page among the pages lacking one, and its heading list ended at "Cross-links to other docs pages"), so the contract table is authored here — the prompt's step 6c names the page's own item as the pick that may author it, and nothing else in `specs/**` was touched. || CLOSED THIS ITERATION. WHAT LANDED (crate `docs-app` only — no other crate was touched, and `specs/**` was edited only by the page's own spec, which this item may author): `crates/docs-app/src/pages/input_page.rs` mirrors `docs/src/app/(docs)/react/components/input/page.mdx` — h1, the `<Subtitle>` with its live `Field` cross-link, the hero demo BEFORE the first heading inside the port's `<div class=\"docs-demo\" data-demo=\"hero\">` wrapper, `## Usage guidelines` (upstream's bullet with its `<label>`/`Field` guidance and the `forms guide` link), `## Anatomy` over the port's own snippet, and `## API reference` built from the upstream page's generated `types.md` through the shared `crate::reference` primitives (the part summary, the `Input Props:` table as upstream's `<details>`/`<dl>` rows with the four-item `Prop | Type | Default` header, the `Input Data Attributes:` `TableRoot` with its seven rows, and the four Additional-Types panels), followed by the state/change-event/canonical-type prose in the port's own type names. THE DEMO IS THE REAL PORT: `leptos_ui::Input` in `view!` markup inside upstream's wrapping `<label>` (\"Name\"), with the demo's `placeholder` riding the `element_attributes` rest bag and its `className` as the port's `class` — every upstream class string carried verbatim, no demo-side machinery, and no chip of React source anywhere (the page's single fence is the port's own `use leptos_ui::Input;` + `view! { <Input /> }`). WIRING: `pages/mod.rs`, the `react/components/input` route in `lib.rs`, the sidenav entry in `chrome.rs` (alphabetically after Form, so the `nav_routes.rs` guard covers the new route), and `react/components/input` added to `ralph/generated/routes.json` — the GENERATED route index the CI workflow shards over (`measure-port.yml:86`), which is what makes the new page measurable on the runner without touching any workflow file. `specs/docs-content/input/page.md` gains the missing `## Snippet & behaviour contract` (5 of 41 mirrored pages now carry one, measured by `check-docs-contract.mjs`), with two rows (Anatomy + the hero demo), a `## <a href=...>`-free table cited to upstream line ranges and the mined behaviour spec, and four explicitly-carried-open gaps (the Field-integration claim is upstream guidance and NOT test-proven in `specs/library/input/behavior.md`, `render`/`ref`/the state-function spellings are not exposed by the port, and upstream's demo chrome belongs to the `docs-chrome:` items). VERIFIED BY EXECUTION at this tree: `cargo test -p docs-app --lib` = 79 passed / 0 failed (76 pre-existing + the page's 3 new reference-row guards; the page's own `snippet_language_guard` asserts `{total: 1, leptos: 1, react: 0, other: 0}` with upstream's shape as the positive control, and `demo_styles` — the host guard that a demo class with no rule renders inert — caught `w-40` as undefined on the first run, which `node ralph/scripts/gen-demo-utilities.mjs` then regenerated into `style/main.css`); `cargo check -p docs-app --lib --tests --target wasm32-unknown-unknown` clean, so the new browser-side render test (`input_page_renders_the_mirrored_structure_and_its_hero_demo`, which asserts the mirrored structure, the label/input composition, the demo class and — one browser turn after mount — the bag-applied `placeholder`) is COMPILE-verified here; `node ralph/scripts/check-react-mentions.mjs --source` = 0 findings, exit 0 (unchanged); `node ralph/scripts/check-citations.mjs check --scope specs/docs-content/input` = 34 citations, exit 0 (the five citations this iteration authored were each re-read against the file they name before the baseline was recorded, and one of them — the `element_attributes` mount effect — was corrected from `587-601` to `584-601` after the check); and `bash ralph/scripts/run-regression.sh \"docs-content: components/input\"` printed `=== REGRESSION OK for docs-content: components/input ===` (citation check, `cargo test --workspace`, TODO.md schema, docs-app `cargo leptos build`, the differential). WHAT THE DIFFERENTIAL ACTUALLY MEASURED, stated rather than implied: the gate's own invocation is `playwright-diff.mjs --todo-id <id>`, whose upstream comparison is behind `DIFF_UPSTREAM=1` (`ralph/scripts/playwright-diff.mjs:168`) and which NOTHING in this repo sets, so the clause every docs pair carries is satisfied by its structure-only mode — the report reads `leptosMounted: true, hasH1: true, nonEmptyTree: true, pass: true`, with the page's 13 headings in upstream's order and the demo's `<label>`/`<input>` in the rendered tree. I ALSO RAN the upstream side by hand (`DIFF_UPSTREAM=1`, upstream serving on 127.0.0.1:3005): `upstreamMounted: true`, `headingsSubset: 0.5`, `pass: false` against the 0.8 bar — and the same axis fails on the ALREADY-DONE checkbox page (`0.5625`), because upstream's generated Additional-Types heading carries a disclosure link whose text is part of the heading (`docs/src/components/ReferenceTable/AdditionalTypes.tsx:44-56` → `Input.PropsHide`) while the port's shared `additional_types` primitive deliberately renders none. That is an inherited, shared condition and a scope question about the clause, not a property of this page: it is written up in `ralph/logs/spec-discrepancies.md` with both measurements, and the residual is cited to the item that owns the Additional-Types panels (`docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)`, `status: not-started`). This close therefore claims the differential in the mode the gate runs, and does NOT claim upstream-side parity. WHAT THIS CLOSE DOES NOT CLAIM: (1) the page has not been rendered in a browser FROM THIS BOX — `ralph/generated/env-health.json` reports `browser: DEGRADED — browser gates REFUSED here by lib/browser-budget.mjs (4 GB cgroup)`, the differential above launches its own Chromium outside that guard, and the browser-side render test is compile-verified only; the rendered axes are CI's to measure, and the new route is now in the index CI shards over, so the next measurement includes it. (2) the page does NOT yet pass the page scorecard — `scorecard-latest.mjs` reports 0 of 17 routes passing every axis, this page is the 18th and unmeasured until CI runs, and its expected gap is the same one every ported page has (the demos carry upstream's Tailwind variant while the app renders upstream's CSS-modules variant — `docs-chrome: demo styling`, `status: not-started`); per this ledger's own convention the scorecard is reported as page-level state, not as a bar this item owes (its done-when says so in as many words). (3) no `blocked-by` was narrowed, no spec was rewritten to agree with this implementation, and the `--update` flag was used on NO recorded baseline.
...
- [x] docs-content: components/toggle-group
      crate: docs-app
      ...
      status: done
      commit: 700784aaa
      note: STEP 0 RECORD — CHOSEN OVER the mechanical suggestion (`library: drawer`; `node ralph/scripts/pick-next-todo.mjs` re-run this iteration prints it). WHY NOT DRAWER, re-measured this iteration rather than quoted: `find packages/react/src/drawer -name '*.ts*'` = 18,290 lines total, 4,763 of them non-test source over 32 files across 11 subdirectories, the entry carries `needs-batched-mining: true`, and the file on disk is still the 57-line FABRICATED wrapper (`crates/leptos-ui/src/drawer.rs` re-exports dialog types, aliases `DrawerHandle = DialogHandle` and hardcodes `modal: false` with the comment "Drawers are typically not modal" — `:14,39`) that the 20260913 attempt left; one bounded iteration cannot close that, and its two fabricated-test bodies assert nothing. WHY NOT A BLOCKED ITEM FIRST: the ledger's only `status: blocked` FIELD line (`docs-content: components/accordion (prose + snippet completion)`, TODO.md:2508) was RE-VERIFIED rather than trusted — its third dep `docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)` is still `not-started`, so the block HOLDS and no broken-thing-first candidate outranks this pick. WHY THIS ITEM: it was LANDED BUT UNRECORDED **and the tree it landed in was RED** — which the mechanical picker structurally cannot see, because it reads TODO status only. Checkpoint 7c7fb30ae (its own subject says "NOT a done-marking") landed the previous iteration's uncommitted page work: `crates/docs-app/src/pages/toggle_group_page.rs` (356 lines, the mirrored page on the REAL `ToggleGroup`/`toggle_element` port), 178 lines of render tests, the authored `## Snippet & behaviour contract`, and the route wiring in `lib.rs`/`pages/mod.rs`/`chrome.rs` — while this entry still read `not-started`, and `cargo test --workspace` failed at that tree. Fixing broken state outranks starting new work, and CONTEXT.md's unit of done is the page itself. No `blocked-by` was narrowed, blanked or rewritten to justify this pick: both deps (`library: toggle-group`, `docs-app: routing + layout shell`) were read with `get-todo-field.mjs` and are `done`. WHAT WAS ACTUALLY BROKEN, MEASURED BEFORE THE FIX: `cargo test -p docs-app --lib` = 80 passed / 1 FAILED at `demo_styles::every_demo_class_token_is_defined_in_the_stylesheet`, naming 24 token lines from `toggle_group_page.rs` (19 distinct: `data-pressed:bg-neutral-950`, `dark:active:not-data-disabled:not-data-pressed:bg-neutral-700`, `active:not-data-disabled:not-data-pressed:bg-neutral-200`, `p-px`, `gap-px`, …). CAUSE, read off BOTH sides rather than inferred from the failure: the page's demo class constants carry upstream's TAILWIND variant verbatim (`demos/hero/tailwind/index.tsx:15`, `demos/multiple/tailwind/index.tsx:10` — both re-read and matched character for character against `HERO_BUTTON_CLASS`/`MULTIPLE_BUTTON_CLASS`), this crate compiles no Tailwind, and the generated DEMO UTILITIES section of `crates/docs-app/style/main.css` was never regenerated after those strings landed — the section's own header still read `159 rule(s) over 1640 token(s)` against the pages' 1703. FIX (the generated stylesheet only; no port change): re-ran `node ralph/scripts/gen-demo-utilities.mjs`, which resolves each token against UPSTREAM'S OWN COMPILED STYLESHEET and rewrites the marked section verbatim — a `fetch` plus string work, NO browser, so it respects this box's budget. It emitted 21 new rules (`180 rule(s), 24 theme var(s), 19 --tw-* var(s)`); the only line it deleted is the section's own SELECTORS count, so no pre-existing rule was lost, and `--check` afterwards prints "up to date" (idempotent). SPEC REPAIR THIS ITEM OWED, and its direction matters: the contract table's Hero row quoted `defaultValue={['left']}` for its citation `packages/react/src/toggle-group/ToggleGroup.test.tsx:63-67`, whose test is `defaultValue={['two']}` over children `value="one"`/`value="two"` (`:55-70`) — the CLAIM was true and the citation was the right one, only the quoted value was the demo's own seed; corrected in place to `['two']` so the paraphrase agrees with the ORACLE (never the reverse), with the finding logged in `ralph/logs/spec-discrepancies.md` rather than fixed silently. All six citations in that table were then re-read against the files they name and hold (`toggle_group.rs:694-745` the `#[component] ToggleGroup`; `ToggleGroup.test.tsx:13-16` `refInstanceof: window.HTMLDivElement`; `:48-52` a second press unpresses the first; both demo sources; `useRenderElement.tsx:164-196`). Those six also had NO RECORDED BASELINE — the previous iteration authored the row and never ran `record` mode — so each was verified first and recorded after: `check-citations.mjs check --scope specs/docs-content/toggle-group` = 39 citations, 0 warnings, exit 0. ROUTE REGISTRATION LANDED WITH THIS ITEM: `react/components/toggle-group` in `ralph/generated/routes.json` was an UNCOMMITTED addition sitting in the tree, and that file is the index `.github/workflows/measure-port.yml` shards over — without it in a commit the new page is never measured on the runner, so it is landed here rather than left to rot. VERIFIED BY EXECUTION at this tree: `cargo test -p docs-app --lib` = 81 passed / 0 failed (was 80/1); `node ralph/scripts/check-citations.mjs check --scope specs/docs-content/toggle-group` = 39 citations exit 0; `check-react-mentions.mjs --source` = 0 findings exit 0; `check-package-alias.mjs` = 0 defects exit 0; `gen-demo-utilities.mjs --check` = up to date; `cargo check -p docs-app --lib` clean. Step 7 `bash ralph/scripts/run-regression.sh "docs-content: components/toggle-group"` printed `=== REGRESSION OK for docs-content: components/toggle-group ===` EXIT 0 at the pre-marking tree (citation check 35 + 4, `cargo test --workspace` green with zero FAILED, TODO.md schema OK, sandbox parity OK, docs-app `cargo leptos build` OK). WHAT THIS CLOSE DOES NOT CLAIM: (1) the RENDERED axes are UNMEASURED, not passed — `node ralph/scripts/scorecard-latest.mjs --route react/components/toggle-group` reads `UNMEASURED — no CI measurement on record yet`, and this box refuses browsers by design (`ralph/generated/env-health.json`: `browser: DEGRADED — browser gates REFUSED here by lib/browser-budget.mjs (4 GB cgroup)`); nothing in this close rests on a rendered number, and the route now being committed is what lets CI produce one. (2) The Playwright differential the done-when names did NOT run here: step 7's differential step reported `DEFERRED … browser budget refusal (exit 2 = UNMEASURED)` and printed "NOT a pass" — deferred to CI per the input/checkbox precedent, and no local Chromium was started to manufacture it. (3) The page's contract table carries four explicitly open gaps, each cited to its owning item rather than dropped: the composed-child ergonomics gap (`docs-ergonomics: mirrored snippets must read like upstream's` — the port has no `Toggle` component, only `toggle_element`), the unexposed `render`/`ref` (`library: the view paths drop render's element form`), the structurally absent `Toolbar`-nesting branch (`library: toolbar` has no ledger item), and the rendered axes (CI). (4) The main.css change is a fix to the INSTRUMENT'S INPUT, not to the port: the port's class strings were already verbatim-correct against upstream, and no page, demo or spec claim was weakened to make the guard pass.
...
- [x] docs-copy: install lines + React type columns on the 18 mirrored pages (no-rework lane)
      ...
      review-note: MEASUREMENT TOOLING CHANGED in this iteration's own commit 732a1a8b1d — ralph/scripts/check-package-alias.mjs ralph/scripts/check-page.mjs ralph/scripts/check-react-mentions.mjs ralph/scripts/gate-selftest.mjs ralph/scripts/lib/metric-invariants.mjs ralph/scripts/lib/source-scope.mjs ralph/scripts/lib/source-scope.selftest.mjs ralph/scripts/probe-demo-toolbar.mjs ralph/scripts/run-regression.sh ralph/scripts/snippet-ergonomics.mjs ralph/scripts/status-report.mjs . A gate edit is not self-authorising: it needs review as a tooling change (what it now measures, and whether the bar it enforces moved). Recorded by the driver so the next iteration sees it rather than inheriting a quietly different gate.
- [ ] docs-copy: Leptos-only mentions + the @noevaresearch/base-ui alias (no React leakage)
      ...
      blocked-by: [docs-app: routing + layout shell]
      priority: medium   # demoted 2026-09-16: it cannot finish until CI measures its routes, so it must not outrank the
      # component lane that CAN make progress now. Restore to high once the CI measurement for its routes lands.
      status: not-started   # NOT blocked: the dependency is satisfied and there is nothing broken to repair.
      # The single remaining clause awaits the next scheduled CI measurement (see the evidence path above),
      # and `blocked` here read to the picker as a broken state to fix first — which is why this item kept
      # winning the queue over the component lane after being demoted. Honest status: waiting, not stuck.
      blocked-reason: the ONE thing left is a RENDERED measurement, and it must now come from CI — which is where it belongs, since this box CANNOT produce it (the browser guard refuses by design: one chromium is ~1.4 GB of a 4096 MB cgroup). REACHABLE EVIDENCE PATH: `node ralph/scripts/scorecard-latest.mjs --route react/<kind>/<name>` prints the `react mentions` axis measured by measure-port.yml on a 16 GB runner; 0 on every route of this item closes it — the item's clause 2 is `node ralph/scripts/check-react-mentions.mjs --all`, which `ralph/scripts/lib/browser-budget.mjs` refuses here with exit 2 (UNMEASURED: one chromium is ~1.4 GB of a 4096 MB cgroup, and a rust build ~1.5 GB), and since ae7a6a1b8 the runner no longer misreports that refusal as a defect — it reads a RECORDED rendered measurement instead and fails loudly when none is admissible (`ralph/scripts/mentions-rendered-evidence.mjs`, added in the same commit). MEASURED at this tree, command by command: `check-react-mentions.mjs --source` exit 0 (0 fail, 0 warn, classifier self-test ok); `check-package-alias.mjs` exit 0 (0 defect(s); `base-ui-leptos` resolves from the repo root and from `test/node-resolution`); `mentions-rendered-evidence.mjs` exit 1 — "the record carries no ruler fingerprint (it predates the fingerprint instrumentation)" for `ralph/logs/visual/react-mentions.md` (the site-wide 0-defect report over 41 routes committed by 83c082a5b) AND "the CI scorecard's rendered mentions axis is not clean on 17 of 17 route(s)". So clause 2 is UNMEASURED, which is not a pass and cannot close the item. UNBLOCK IS ONE COMMAND on any machine with room for a browser (this entry does NOT claim it was run): `RALPH_BROWSER_GATES=1 node ralph/scripts/check-react-mentions.mjs --all`, then commit `ralph/logs/visual/react-mentions.md` — the fresh report carries the ruler fingerprint and then stands as evidence until a page source/allow file or the classifier's rules change. Alternative: make CI's per-route axis measurable again (it is UNMEASURED on every route; recorded as its own item at the end of this file).
      step0-note: PICKED AS SUGGESTED (pick-next-todo.mjs re-run this iteration prints this id). Checked before
        ...
        `docs-content: components/accordion (prose + snippet completion)`, whose third dep
        (`docs-chrome: API reference code blocks`) is still `not-started`, so that block is CORRECT and there is
        no broken-thing-first candidate above this item (verified, not inherited: `grep -n "^      status: blocked"
        TODO.md` → one hit, and `grep -n -A8 "^\- \[.\] docs-chrome: API reference code blocks"` → not-started).
        The item is also genuinely closeable rather than a livelock pick, MEASURED at this tree rather than trusted
        from the notes: `check-package-alias.mjs` exits 0 with 0 defect(s), `check-react-mentions.mjs --source`
        exits 0, and `--all` exits 0 — while the notes of the iterations that refused this item five times record
        "58 fail across 25 files" / "11 defect(s)", i.e. the sibling item that landed since (03ba0b044) already
        paid the fail halves. What is NOT yet done is this item's own remaining clause — "every tolerated mention
        of the word React listed with a reason in specs/docs-content/<name>/react-allow.json" — measured here as
        15 source-side and 5 rendered-side `react-word` warnings, and it is unmet for two INSTRUMENT reasons
        found this iteration (below), not merely unwritten bookkeeping. `check-unpassable.mjs --todo-id` agrees:
        VERDICT WORK ("every failing axis is one this item claims").
      gate-fix-note: ROOT FIX LANDED THIS ITERATION, tooling only (commit ae7a6a1b8: `ralph/scripts/mentions-rendered-evidence.mjs` new; `ralph/scripts/run-regression.sh` and `ralph/scripts/check-react-mentions.mjs` edited — no crate, no spec, no release path, no workflow touched). This is the item's own blocked-reason's unblock path (b), "teach this ONE clause the budget module's documented contract", done so that UNMEASURED still cannot become a pass. THREE CHANGES.
        (1) `run-regression.sh` had ONE browser gate that never learned the rule the other three follow: `check-visual-budget`, `snippet-ergonomics` and `check-copy-fidelity` each sit behind `[ "$BROWSER_GATES" = "1" ]`, while `check-react-mentions.mjs --all` was called unconditionally and its exit 2 (the budget's documented UNMEASURED) fed `|| fail "rendered routes still show React APIs or upstream's package name"` — a REFUSAL reported as a page defect, for every `docs-copy:` item. One `rendered_mentions_axis()` now serves both docs-copy branches: browser allowed → run it, exit 2 still UNMEASURED; browser not allowed → read the recorded measurement. The `--fail-on` per-class claim survives on both paths (the install-lines item's react-api/package-react claim is not widened).
        (2) `ralph/scripts/mentions-rendered-evidence.mjs` (new) is what "read the recorded measurement" MEANS, or the deferral would be exactly the hole this clause was widened to close. It admits a record only when it is ALL FOUR of: committed at HEAD (a working-tree report cannot be tied to a tree), clean for the classes the caller claims (parsed per class from the report's defect bullets, so a `snippet-react` defect owned by the snippet-translation items is reported, never charged), site-wide (a `--route` scan writes the same file, so 1 route cannot stand in for 41), and current — no verdict-changing path changed since (`crates/docs-app/src`, `specs/docs-content`, `packages/leptos`, `lib/source-scope.mjs`). Its second source is the CI scorecard's per-route `react mentions` axis, the budget module's own "instead:" text. Pure text + git, no browser, so the loop can afford it every iteration, and it carries a self-test that runs on every invocation (12 report fixtures + 4 scorecard fixtures + 2 parser controls = 16 cases, both directions) per `gate-selftest.mjs`'s doctrine — it caught its own author's wrong fixture (exit 3) before printing any verdict. CORRECTION to the commit subject matter, checked because a count in a message is a claim: 81d9c3cf2's message says "14 report/scorecard cases" and this note first said 13; the file is 12 + 4 = 16 cases plus 2 parser controls (counted with `grep -c "^  { id: '" ralph/scripts/mentions-rendered-evidence.mjs` → 16). The instrument's behaviour is unaffected; the number in the prose was, which is why the ledger carries the corrected figure here.
        (3) `check-react-mentions.mjs` now stamps the RULER's fingerprint into the rendered report and prints it via `--ruler-fingerprint` (pure text, no browser, no side effects) — not decoration: a record can outlive the instrument that produced it, and THIS file is the one the loop keeps fixing (39d47180e moved the browser guard 200 lines and changed no verdict, while one changed character in a RULE changes every verdict in the report). File history cannot tell those apart; a hash of the rule table + attribution pattern + `scanRendered`'s source can. It also gained `--no-report`, because the runner's per-route ADVISORY scans (`--route`, the `else` branch) were rewriting the site-wide record this lane's evidence rests on.
        MEASURED AT THIS TREE after the change (these are the numbers this entry rests on): `node ralph/scripts/mentions-rendered-evidence.mjs` → record REFUSED ("no ruler fingerprint … predates the fingerprint instrumentation"), CI axis REFUSED ("not clean on 17 of 17 route(s)"), verdict "UNMEASURED — this is NOT a pass", exit 1. `check-react-mentions.mjs --source` exit 0 (0 fail / 0 warn). `--ruler-fingerprint` → 37f390029e7b, UNCHANGED by all three edits above (content-based, which is the point). `bash -n run-regression.sh` clean. The item's full `bash ralph/scripts/run-regression.sh "<this id>"` still exits non-zero, at this same step, with that reason and no other failure before it — which is the honest state, and the failure is now attributed to the missing measurement instead of to pages that measure clean.
        WHAT IS NOT CLAIMED: the rendered run did not happen here (the loop's rule: no chromium on this box), so clause 2 remains UNMEASURED and this item is NOT closed on it; and the earlier recorded "0 defect(s) across 41 routes" report is treated as EXPIRED rather than deleted — it is still on disk and still says what it said, but it was produced before this iteration's ruler edit and carries no fingerprint, so it cannot speak for the classifier at HEAD.
...
- [x] docs-chrome: snippet translation (batch 1)
      ...
      priority: high
      status: done
      commit: fdc010931 (the translation + the first instrument fix; the done-marking commit carries the second instrument fix in ralph/scripts/snippet-ergonomics.mjs, this ledger edit and the discrepancy log)
      ...
      done-note: CLOSED at this tree, clause by clause, on the browser numbers rather than on the source diff. avatar needed NO source change (the previous iteration's checkpoint translated it) and is MEASURED here, not assumed: `visual-gap-report.mjs`'s snippet table reads leptos/react/other `2/0/1` with purity 1.0 (2/2). checkbox-group: six blocks were upstream's TSX verbatim (`0/6/0`) and now teach the port — the group through `checkbox_group_view` (the crate exposes no `CheckboxGroup` component, and `check-part-surface.mjs` records the unit as INERT because upstream documents no dotted part of its own) with namespaced children (`<Checkbox::Root>`, `<Checkbox::Indicator>`, `<Field::Item>`, `<Field::Label>`, `<Fieldset::Root>`, `<Fieldset::Legend>`, `<Form>`) — `6/0/0`, purity 1.0. collapsible: two blocks (`0/2/0`) -> `<Collapsible::Root>`/`<Trigger>`/`<Panel>` (`2/0/0`, purity 1.0), and the page moved out of `lib.rs` into `pages/collapsible_page.rs` (the mirrored-page shape every other page uses) with upstream's demo wrapper, the Anatomy lead-in and the Accordion cross-link. BEFORE -> AFTER page scores, `check-visual-budget.mjs` at build 42968858b, auto-recorded with the reasoning in each baseline entry's own `note`: avatar 68.67 -> 74.63 (visual 92.79 -> 92.78 / content 32.50 -> 47.41 — the WHOLE rise is the purity-denominator fix, no page change), checkbox-group 70.03 -> 77.35 (visual 92.10 -> 92.57 / content 36.93 -> 54.53, widget 100% unchanged), collapsible 60.14 -> 71.68 (visual 94.55 -> 93.94 / content 8.53 -> 38.29, demoParity 99.32%). VERIFIED: `cargo test -p docs-app --lib` 54 passed / 0 failed — each page's `snippet_language_guard` carries the classifier's positive control (upstream's shape WITHOUT the package string, so the alias gate is not inflated, the field-page precedent), the probe's exact triple per page, every snippet COMPILED against the crate's real surface, and a pin that the teaching spelling is the namespaced one; writing those shapes caught two real API mismatches (the port's `Checkbox::Root`/`Collapsible::Trigger`/`Panel` declare `children` as REQUIRED while upstream's anatomy is self-closing, and the group's children closure needs `Send`). `cargo fmt -p docs-app -- --check` clean (it also cleared pre-existing drift in five other files, whitespace only); `cd crates/docs-app && cargo leptos build` EXIT 0; `bash ralph/scripts/run-regression.sh "<this id>"` EXIT 0 at the FINAL tree, after both instrument fixes (citation check, `cargo test --workspace` green, TODO.md schema OK, docs-app build, `check-visual-budget --all-done` over every recorded route with no FAIL line).
        TWO INSTRUMENT DEFECTS, ONE CLASS — a PERMITTED `other` block counted against the port — both found by measuring this item's own clause, both fixed at the root, both disclosed as MEASUREMENT changes to review as tooling: (1) `check-visual-budget.mjs`'s snippet-language purity divided by every `<pre>` on the page, so the clause this item is measured by ("purity reaches 1.0") was UNREACHABLE for any page mirroring a language-neutral block — avatar read 2 leptos / 0 react / 1 permitted CSS = 0.67 with nothing left to translate, i.e. the term punished mirroring a block upstream shows. The denominator is now the framework blocks (`leptos + react`): no bar removed, the counts themselves untouched (the gap report, snippet-ergonomics and check-page's `react = 0` axis are unaffected), a page with no framework code still measures `null`, and NO baseline was re-recorded with `--update`, so the recorded numbers stay pre-fix and conservative. (2) `snippet-ergonomics.mjs`'s >=90%-verbatim net accused avatar's mirrored stylesheet of being "upstream's own React code" and FAILED the run — unfixable by translation without deleting a block upstream shows; the net now compares the classes of BOTH blocks (control run unchanged: `--route react/components/separator`, a genuine React block, still fails identically), and the permitted copy stays EXCLUDED from scoring, so avatar's numbers are unchanged at 60/100 / length 21.7%. The second fix matters beyond one page: `docs-ergonomics: mirrored snippets must read like upstream's` — the item this batch unblocks — gates on that command exiting 0 for every route. Full reasoning, citations and arithmetic: `ralph/logs/spec-discrepancies.md` (2026-09-16, "batch 1's translations").
        NOT CLAIMED: collapsible's component widget parity is 93.35% against the 97 bar — the port's collapsible parts forward no class prop, so the hero demo's Tailwind classes ride on wrapper elements; the gap report raises it as a P0 and it is the demo-styling / library-surface lane's, not this item's. The pages' other gap-report P0s (no API tables, text volume 52% / 14% of upstream, 6 or 2 `<pre>` against upstream's 20 / 24) are unchanged and owned elsewhere. And the port could not express THREE of the eight examples — the `render` callback form (the Function arm returns the internals crate's `RenderedElement`, not a `view!` tree), `Fieldset.Root`'s missing `render` prop (upstream's example makes the group the fieldset element), and `Collapsible::Panel`'s missing `hidden_until_found` (its engine implements it — `collapsible/panel.rs:48` — the component surface does not) — each stated on the page itself and logged, never silently reworded; the collapsible one is written up as a proposed ledger item because it is currently unowned.
...
- [x] docs-chrome: snippet translation (batch 2)
      crate: docs-app
      ...
      step0-note: THIS ITEM, CHOSEN OVER the mechanical suggestion (`docs-copy: install lines + React type columns
        on the 18 mirrored pages (no-rework lane)`, which `pick-next-todo.mjs` prints again this iteration). The
        suggestion is `not-started` with its single `blocked-by` dep `done`, so it passes the picker's rule — but
        it still cannot reach its OWN done-when from inside its lane, re-measured at this tree rather than
        inherited: `node ralph/scripts/check-react-mentions.mjs --source` exits 1 with **58 fail across 25 files**
        and `node ralph/scripts/check-package-alias.mjs` exits 1 with **11 defect(s)**. Those 11 decompose into
        work this entry does not own: 5 are `#[cfg(test)] snippet_language_guard` positive controls
        (accordion_page.rs:616, button_page.rs:540/567/571, checkbox_page.rs:872) that are page SOURCE, not
        reader-facing — the sibling pages' controls already omit the package specifier for exactly that reason
        (the field-page precedent) — and 6 are the first line of rendered anatomy blocks on csp-provider
        (:130,:167), direction-provider (:169), progress (:265), separator (:136), toggle (:197), four of which
        have no teaching surface in the port at all (`crates/leptos-ui/src/lib.rs:27-29,106-112`: the capitalised
        aliases exist only for the ported batch — there is no public `Toggle` or `Separator` component, and
        `check-part-surface.mjs` lists neither among the components with mined specs). It is also hard-gated on
        a copy-coverage bar it does not claim: the `docs-copy:*` clause in `run-regression.sh` enforces >= 95% on
        all 18 of its routes and the measurement is accordion 100%, avatar 43.2%, button 87%, checkbox 84.7% —
        debt owned by `docs-content: components/{avatar,button,checkbox}`, whose entries are `done` and carry no
        copy clause (full account in `ralph/logs/spec-discrepancies.md`, appended this iteration). Picking it a
        sixth time could only produce another ledger-only `blocked` marking — the livelock `79fea09eb` was
        written to break. Nothing here narrows, blanks or rewrites a `blocked-by`: both of this entry's deps
        (`docs-app: routing + layout shell`, `library: namespaced part surface (ported batch)`) are `done`, so it
        is genuinely startable, and this entry's own earlier note (written by the iteration that did batch 1)
        hands it forward verbatim: "This item is therefore CLOSEABLE and is left `not-started` on purpose — one
        item per iteration ... so the next iteration can close it on its own gate rather than inheriting a
        pre-marked done."
      done-note: CLOSED at this tree, clause by clause, on the BROWSER numbers rather than on the source diff.
        The language half was already on disk (landed by the driver's post-condition CHECKPOINT feb9ca358, whose
        subject ends "NOT a done-marking"); this iteration verified it independently and then strengthened the
        guard that is supposed to prove it. MEASURED, fresh renders of both apps at this tree
        (`visual-gap-report.mjs`, 1280px): snippet table leptos/react/other = field 1/0/0, fieldset 1/0/0, form
        3/0/0, meter 1/0/0 — `react: 0` with `leptos > 0` — and the budget gate's own per-route line agrees
        (`snippets leptos/react/other 1/0/0` / `3/0/0`, purity 1.0) with `delta +0` against every recorded score
        (field 70.36, fieldset 80.97, form 81.51, meter 74.03). WHAT LANDED (crate docs-app, tests only, four page
        files): each `snippet_language_guard` pinned only the Anatomy block, so the page-level triple THIS ITEM IS
        MEASURED ON had no test — a future untranslated fence on any of the four pages would have left every
        assertion green while the probe went red (batch 1's pages already carry the stronger shape). Each guard
        now carries `page_snippets()` in document order, an ordered `the_pages_snippets_all_teach_the_port`
        comparison against `{total, leptos, react, other}` = {1,1,0,0} (field, fieldset, meter) and {3,3,0,0}
        (form), and `every_snippet_uses_the_namespaced_spelling` (CONTRACT.md requirement 1's mapping table:
        `<Field::Root>`, `<Fieldset::Legend>`, `<Meter::Track>`, never the flattened `<FieldRoot>`), beside the
        existing classifier positive controls and compile-checked `*_snippet_shape()` bodies. `cargo test -p
        docs-app --lib` 63 passed / 0 failed; `cargo fmt -p docs-app -- --check` clean. NOT CLAIMED: the guards
        run on the HOST target and this loop's regression does not build the wasm suite, so the render proof is
        the browser probe above, not the test. Two adjacent debts stay in their own lanes and are untouched: the
        pages' API-reference prose still carries React types (`React.RefObject`, `React.ReactNode`) and their
        specs still lack the CONTRACT.md requirement-3 table — owned by the `docs-copy:` and `docs-spec:` items
        respectively. The ergonomics length floor still reports FAIL on these four routes (field 4.7%, fieldset
        9.3%, form 18.7%, meter 8.9%) — advisory for this item by the gate's own ownership rule, and its
        denominator is upstream's rendered demo-source and API-reference code volume, not these pages' snippets.
        VERIFIED: `bash ralph/scripts/run-regression.sh "docs-chrome: snippet translation (batch 2)"` EXIT 0
        (citation check, `cargo test --workspace` 366 + 416 + 281 + 63 + … all green, TODO.md schema OK, docs-app
        build OK, visual budget over every recorded route with no FAIL line). BOX NOTE, because it will look
        like a code failure in the log: the FIRST attempt at this gate died with a rustc ICE — `failed to spawn
        work thread: Os { code: 11, kind: WouldBlock }` while codegenning docs-app — i.e. cgroup pid exhaustion
        (512 cap; 0 orphaned Chrome and 12 GB free on /data, so neither the disk nor a leaked harness), not a
...
- [ ] docs-chrome: demo panels (bordered container + file tabs)
      ...
      note: the demos themselves mount and work (demos recall is already 1/1 on checkbox); what is missing is entirely their presentation — CROSS-REFERENCE ADDED 2026-09-16, this item's own scope is UNCHANGED (nothing here is narrowed, blanked or reassigned); its file-selector-tabs clause is MEASURED and carried together with the items created for the rest of the demo chrome: the strip lives in the toolbar container of `docs-chrome: demo frame structure (playground + code panel with copy and Show code / Hide code)`, the tabs themselves in `docs-chrome: demo file tabs (the file strip on the demo toolbar on the ported Tabs)`, and neither item nor this one is done until the strip renders. Measured on both live apps 2026-09-16 (upstream 127.0.0.1:3005 against this port 127.0.0.1:3177): the port's demo frame is 669x128 where upstream's container is 669x288 with a 36px toolbar and a 158px code panel, the port has zero `role=tab` elements and zero figure landmarks, and its stylesheet carries none of the chrome classes (`DemoToolbar`, `DemoTabsList`, `DemoTab` all 0 hits in `crates/docs-app/style/main.css`) — full numbers, citations and the four bounded items this decomposes into are in `ralph/logs/demo-toolbar-report-20260916.md`. The shared observable for every one of them is `node ralph/scripts/probe-demo-toolbar.mjs --route <route>`, which reads the same surface on both sides. The presentation half named here (the bordered panel, the spacing, the code block's placement beneath the control) is untouched and remains this item's.
...
- [x] docs-chrome: API reference tables
      ...
      status: done
      commit: 8ea856abbf88d2e916fae21897a416523d78f1b5 (the work/ledger commit: the Step 0 record, the two measurement corrections, the closing note and the discrepancy-log entry; this field's sha record is the follow-up commit)
...
- [x] tooling: check-component-strict measures NOTHING for a hyphenated unit id (props/sections/hygiene read vacuous or false-FAIL)
      crate: ralph/scripts (tooling — no crate)
      specs: ralph/scripts/check-component-strict.mjs
      status: done
      priority: medium
      commit: a6d0ea0303b8d26f64f628f1e43e1af852674f6d
      step0-note: CHOSEN OVER the mechanical suggestion (`library: drawer`; `node ralph/scripts/pick-next-todo.mjs` re-run this iteration prints it), and this line is written after the work landed rather than before it, so it does NOT claim the "recorded before any implementation work" ordering the sibling entries use. WHY NOT DRAWER, re-MEASURED this iteration rather than quoted: `find packages/react/src/drawer -name '*.ts*' | xargs wc -l` = 18,290 lines over 32 files (4,763 of them non-test) plus 44,271 bytes of mined specs, and the entry itself carries `needs-batched-mining: true`; six prior iterations measured the same and chose elsewhere, and the 20260913 attempt exhausted its budget and left a fabricated dialog-wrapper stub, so one bounded iteration cannot close it. WHY NOT A BLOCKED ITEM FIRST: `grep -n "^      status: blocked" TODO.md` is exactly one line — `docs-content: components/accordion (prose + snippet completion)` — whose third dep `docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)` is still `not-started`; both were RE-VERIFIED this iteration by reading the files rather than trusting the note, so that block HOLDS and no broken-thing-first candidate outranks this pick. WHY THIS ITEM: it is a lying INSTRUMENT of exactly the class this iteration's own Step 0 preamble describes, and the lie is live and measured rather than stylistic — `--component checkbox-group` printed `sections: FAIL — 9 of 9 spec section(s) have no test touching their vocabulary` and `hygiene: FAIL — 0 test(s)` while `crates/leptos-ui/src/checkbox_group_tests.rs` was sitting on disk, because `portFiles()` resolved files from the HYPHENATED ledger id and probed `src/checkbox-group_tests.rs`, a path Rust cannot have. 26 of the 38 spec-bearing units read that empty-text pair, 10 of them with a real module on disk under the snake name. It is also on the critical path of two open `priority: high` items — `library: namespaced part surface (menus batch)` and `(inputs batch)` gate HARD on this script (`run-regression.sh:460-468`) — and for their hyphenated units the hard `parts`/`namespaced path` axes read "nothing to check" over an empty file list, so a batch could be closed on a blind reading rather than on a false failure. No `blocked-by` was narrowed, blanked or rewritten to justify this pick; the entry has no `blocked-by` field and none was added.
      ...
      note: CLOSED THIS ITERATION — the file probe normalises the ledger id to the Rust module spelling it names. WHAT LANDED (one file, `ralph/scripts/check-component-strict.mjs`, plus the discrepancy log; no crate, no spec, no other tool): `portFiles()` now derives its candidates from `snake(name)` — the module id a Rust path can actually have — and adds the crate's QUALIFIED test modules, `<base>_<submodule>_tests.rs`, refusing any stem whose qualifier names a DIFFERENT unit (checked against the ledger's own `specs/library/*` unit list) so `checkbox` cannot absorb `checkbox_group_tests.rs` and satisfy its own spec obligations with the group's tests. The spec lookup (`specs/library/checkbox-group/behavior.md`) and the part-surface delegation (`--components checkbox-group`) are UNTOUCHED and keep the hyphenated id, the only spelling either resolves. THE BAR DID NOT MOVE: no floor, tolerance or target was edited and the same verdict rule (`strict && gaps`) decides the exit code — this is a measurement change, not a bar change. EVIDENCE, BY EXECUTION: `--component checkbox-group` reads `sections: OK — all 9 spec section(s) have test coverage vocabulary` (was `FAIL — 9 of 9`) and `hygiene: FAIL — 8 test(s) for 9 spec section(s)` (was the vacuous `0 test(s)`); `--component otp-field` reads `sections: OK` (was FAIL) with its `namespaced path: OK — 14 use(s) of <OTPField::…>`. A before/after snapshot of EVERY spec-bearing unit — all 38 `specs/library/*` directories, per-axis verdicts captured for each — shows eight units moved and EVERY move is `vacuous -> measured`: alert-dialog, checkbox-group, context-menu, otp-field and preview-card went sections FAIL->OK (gap counts 4->3, 2->1, 4->3, 2->1, 4->3), radio-group and toggle-group went sections FAIL->OK AND hygiene FAIL->OK (2->0), and navigation-menu now measures real text (its no-test-module flag cleared) while its sections/hygiene verdicts stay FAIL because that unit genuinely has not covered them. No unit moved OK->FAIL on any axis, and the unit set is identical before and after. The anti-absorption rule was verified by a controlled probe rather than by reading the code: `checkbox`, `radio` and `toggle` each refuse exactly their sibling unit's `*_group_tests.rs`, while `menu` keeps `menu_view_tests.rs` and `otp_field` keeps `otp_field_view_tests.rs`. Step 7 `bash ralph/scripts/run-regression.sh "<this id>"` printed `=== REGRESSION OK ===` at the pre-marking tree (citation check, `cargo test --workspace` 1,346 passed / 0 failed, TODO.md schema, sandbox parity, source-scope classifier controls, part surface) and was re-run at the done-marked tree. WHAT THIS CLOSE DOES NOT CLAIM: (1) every axis it makes MEASURABLE is not thereby GREEN — nine hyphenated units keep real FAILs (the hygiene floors of otp-field, number-field, alert-dialog, context-menu, preview-card, menubar and navigation-menu among them), and this item owns the measurement, not those gaps; (2) `#[test]` is still the only test attribute counted, so a wasm-only module reads `0 test(s)` — a real blind spot this fix made visible rather than caused (`otp_field_view_tests.rs`: 7 `#[wasm_bindgen_test]`, 0 `#[test]`), deliberately NOT folded in here because it would move verdicts beyond `vacuous -> measured`; it is written up in `ralph/logs/spec-discrepancies.md` (finding 11) and scoped as its own ledger item appended at the end of this file; (3) units whose module lives in another crate (`internals`, `utils`, `merge-props`, `types`, `csp-provider`, `direction-provider`) still read the no-test-module note — `Crate` is `crates/leptos-ui`, their home is `leptos-ui-internals`/`leptos-ui-utils`, and widening that resolution is a different measurement question this item does not touch.
...
- [ ] sandbox: the docs-side control — a ported DemoToolbar carrying "Open in CodeSandbox"
      ...
      note: The URL shape is deliberately the synced template (`.github/workflows/sandbox-mirror.yml` publishes the app to the ROOT of the `sandbox` branch, because CodeSandbox reads `.devcontainer/` and `.codesandbox/` from the template's own root) with the demo's slug passed through, since one app serves every demo and `?demo=` is what selects it (`examples/leptos-sandbox/src/lib.rs`). Visitors fork their own VM, so a docs link costs this project no VM credits; the branch is force-pushed as a single commit and only when `examples/leptos-sandbox/**` changes, because a commit to a synced template's backing branch discards its memory snapshot and the next visitor cold-boots. CROSS-REFERENCE ADDED 2026-09-16, clauses above UNCHANGED: the toolbar this control is to live in is now its own ledger item — `docs-chrome: demo frame structure (playground + code panel with copy and Show code / Hide code)` — which builds the container from the port's own `Collapsible` and needs no unported component, so this item's dependency on a ported `DemoToolbar` is bounded and named rather than implicit. The other three controls measured inside that same toolbar (the file strip, the styling-method selector, the more-actions source menu) have their own items where the demo-chrome items are listed, and the CodeSandbox-over-StackBlitz choice is upstream's own arm for WebKit (`docs/src/components/Demo/Demo.tsx:154-171`), so it is a faithful pin and not a divergence. Surface inventory measured 2026-09-16 on both live apps, with the exact control list and aria-labels: `ralph/logs/demo-toolbar-report-20260916.md`.
...
- [ ] docs-chrome: demo frame structure (playground + code panel with copy and Show code / Hide code)
      ...
      status: not-started
      priority: high
      done-when: a demo frame under `crates/docs-app/src/pages/` renders upstream's container PAIR — a `role="figure"` labelled `Component demo` around the playground, and a second `role="figure"` labelled `Component demo code` around a code panel the reader can collapse — where that panel is a `Collapsible` built from this port's own `crates/leptos-ui/src/collapsible/`, and it carries a copy control with `aria-label="Copy code"` and a trigger reading `Show code` when collapsed / `Hide code` when open, under upstream's own line-count rule: a listing of FEWER THAN 8 LINES renders with no trigger at all (`docs/src/components/Demo/DemoCodeBlock.tsx:40`); verified by a wasm structure test in `crates/docs-app/src/render_test.rs` pinning exactly that shape for one named route, by `node ralph/scripts/probe-demo-toolbar.mjs --route <route>` reading the two figure labels, the copy control and the collapse text on both sides, and by `node ralph/scripts/check-visual-budget.mjs --todo-id "<this id>"` not dropping the route's recorded page score. NOT claimed here: the collapse scroll-back of `docs/src/components/Demo/Demo.tsx:114-142`, the toolbar's mobile/desktop action duplication (`docs/src/components/Demo/Demo.tsx:236-248`), and the demo's bordered panel styling.
      note: MEASURED, not inferred — 2026-09-16, upstream docs on 127.0.0.1:3005 against this port on 127.0.0.1:3177, both live, numbers in `ralph/logs/demo-toolbar-report-20260916.md`. Upstream's `react/components/checkbox` demo frame is ONE container 669x288 carrying TWO figure landmarks: `Component demo` (667x128, the playground) and `Component demo code` (667x158, the panel with its 36px toolbar). THIS PORT'S FRAME IS ONE `.docs-demo` 669x128 WITH ZERO FIGURE LANDMARKS, ZERO CONTROLS AND ZERO pre ELEMENTS — its rect is EXACTLY upstream's playground rect, so the code panel is the whole of the missing half; on `react/components/button` both of the port's frames contain exactly one control, the demo's own submit button. The absence is not styling: the port's stylesheet contains none of the chrome classes (`DemoToolbar`, `DemoTabsList`, `DemoTab`, `DemoCodeBlock`, `DemoCollapseButton`, `DemoSourceBrowser` all 0 hits in `crates/docs-app/style/main.css`). WHY THIS IS THE FIRST OF FOUR: it is the only piece that needs no unported component (the panel is upstream's `Collapsible`, which this port has; `docs/src/components/Demo/Demo.tsx:225-229`), and the other three items created with this report are blocked by it, because they are controls INSIDE this container. The copy control's shape must be authored fresh here, NOT reused from `crate::code_block`: upstream has two distinct components (a page's `.mdx` fence panel vs the demo's `DemoCodeBlock`) and `crates/docs-app/src/code_block.rs:651-654` documents the page-level one's own contract, so reusing it would invent a shape matching neither. The `docs-demo-stack` wrapper this item supersedes is a PORT INVENTION recorded at `crates/docs-app/style/main.css:1499-1518` (upstream has no such element); it stays for the three accordion demos it unblocked until this panel replaces it. NOT OWNS: the file-tab strip, the styling-method selector and the more-actions menu (own items, cited below), the external playground control (`sandbox: the docs-side control — a ported DemoToolbar carrying "Open in CodeSandbox"` owns it, and needs this container), and the panel's bordered presentation (`docs-chrome: demo panels (bordered container + file tabs)` owns that).

- [ ] docs-chrome: demo file tabs (the file strip on the demo toolbar on the ported Tabs)
      crate: docs-app
      specs: ralph/logs/demo-toolbar-report-20260916.md, ralph/scripts/probe-demo-toolbar.mjs, docs/src/components/Demo/DemoFileSelector.tsx, docs/src/components/Demo/Demo.tsx, docs/src/components/Demo/Demo.css
      status: not-started
      priority: medium
      blocked-by: [docs-chrome: demo frame structure (playground + code panel with copy and Show code / Hide code), library: tabs, docs-chrome: demo styling-method selector (a second real styling variant)]
      done-when: the demo toolbar renders upstream's file strip as a real tab list — one `role="tab"` per file inside a `role="tablist"` labelled `Files` (`docs/src/components/Demo/DemoFileSelector.tsx:66`), each tab an anchor `href="#{demo}:{variant}:{file}"` whose target span exists in the container (`docs/src/components/Demo/Demo.tsx:219-221`), and the selected tab decides which file's listing the panel below shows — built from this port's own `Tabs` (`crates/leptos-ui/src/tabs/`, `library: tabs`), and rendering NOTHING when the demo has a single file, which is upstream's own rule (`docs/src/components/Demo/DemoFileSelector.tsx:60-62`); verified by `node ralph/scripts/probe-demo-toolbar.mjs --route <route>` reading the same FILE COUNT and the port's own labels on both sides for one route whose upstream demo has two files (checkbox hero, measured under the css-modules variant: `index.tsx` + `index.module.css`), by a host test over the slug-derivation fn asserting the `{demo}:{variant}:{file}` shape literally, and by `node ralph/scripts/check-visual-budget.mjs --todo-id "<this id>"` not dropping the route's page score.
      note: TWO DEPENDENCIES, BOTH REASONS RATHER THAN BOILERPLATE. (1) Upstream builds the strip on `Tabs` (`docs/src/components/Demo/DemoFileSelector.tsx:65-81`), which this port has not ported (`crates/leptos-ui/src/` has no `tabs`; ledger item `library: tabs`, not-started). A `role="tab"` strip hand-rolled in CSS would carry neither the roving focus nor the `aria-selected` management, i.e. it would be an accessibility divergence AND an invented shape — the same defect class as the gap report's React-snippet P0. (2) THE HONEST FILE COUNT IS 1 UNTIL THE SECOND STYLING VARIANT EXISTS: the ported demo's source is one Rust file, so with one variant upstream's own `files.length <= 1 -> null` rule means the strip would correctly render nothing, making this item unobservably done; the second file (a stylesheet) arrives with `docs-chrome: demo styling-method selector (a second real styling variant)`, which is why that item is a blocker here rather than a sibling. This is the trap to avoid: a strip hard-coded to render two tabs whose second tab shows the SAME listing twice would satisfy a naive probe and be fake chrome. TAB LABELS ARE THE PORT'S OWN FILES, never upstream's filenames — the port's demo source is Rust (`crates/docs-app/src/pages/<name>_page.rs`), and the deep-link slugs are built over that identity.

- [ ] docs-chrome: demo styling-method selector (a second real styling variant)
      crate: docs-app
      specs: ralph/logs/demo-toolbar-report-20260916.md, docs/src/components/Demo/DemoVariantSelector.tsx, docs/src/components/Demo/Demo.tsx, docs/src/utils/demoExportOptions.ts, crates/docs-app/src/demo_styles.rs
      status: not-started
      priority: medium
      blocked-by: [docs-chrome: demo frame structure (playground + code panel with copy and Show code / Hide code), library: select]
      done-when: a demo frame renders upstream's styling-method control (a select whose trigger carries `aria-label="Styling method"`, `docs/src/components/Demo/DemoVariantSelector.tsx:42`) ONLY when the port genuinely has more than one styling variant for that demo — upstream's own condition, `demo.variants.length > 1` (`docs/src/components/Demo/Demo.tsx:175-182`) — and switching it changes what the reader sees: the demo's rendered class list differs between the two variants AND the served stylesheet carries the chosen variant's rules, proven by a host test asserting the two variants' class sets differ for one named page (with the expectation written out literally, not recomputed by the emitting code) and a negative control asserting that a single-variant demo renders NO selector; built on this port's own `Select` (`library: select`) and the second variant's stylesheet produced by this port's own generator (`ralph/scripts/gen-demo-utilities.mjs`, whose host guard is `crates/docs-app/src/demo_styles.rs`). A selector whose two options render identically does NOT satisfy this clause.
      note: OWNER DECISION REQUIRED BEFORE IMPLEMENTING — how the second variant's styles are produced. (a) `stylance` (proc macro + `stylance-cli`): class names imported as constants, a name that is not in the CSS becomes a compile error, hashes derived from the file's path relative to the crate manifest; it must be wired in BOTH builders this repo has, the docs app (`cargo leptos`, `crates/docs-app`) and the sandbox (`Trunk`, `examples/leptos-sandbox`, which assembles its own `index.html` and would take stylance as a `[[hooks]] pre_build`). (b) EXTEND THE EXISTING GENERATOR (recommended): `ralph/scripts/gen-demo-utilities.mjs` already emits a CSS section into `crates/docs-app/style/main.css` at build time and already has a host guard that fails when a class named by a page has no rule (`crates/docs-app/src/demo_styles.rs`), so reading each demo's `docs/src/app/(docs)/react/components/<name>/demos/<demo>/css-modules/index.module.css`, emitting deterministic hashed rules into the same section and a Rust constants module per page is the same machinery this port already maintains — no new build step in either builder. (c) NO second variant means NO selector, which is only correct if the absence is recorded as a permanent disclosed divergence, since upstream renders the selector on every demo that ships two variants. WHAT THE SECOND VARIANT IS: upstream's demos exist on disk in exactly two variants, `css-modules/` (with `index.module.css`) and `tailwind/` (`docs/src/app/(docs)/react/components/checkbox/demos/hero/index.ts` calls `createDemoWithVariants(import.meta.url, {CssModules, Tailwind})`), and THIS PORT RENDERS ONLY THE TAILWIND ONE, with its utilities generated from upstream's compiled stylesheet — so the second variant is CONTENT to author (upstream's css-modules rules for the ported demos), not chrome, and the selector is the last 2% of this item. NOT OWNS: the strip's tab machinery (the file-tabs item) and the port's default styling (done: `docs-chrome: demo styling …`).

- [ ] docs-chrome: demo source menu (View source on GitHub + Copy link to source)
      crate: docs-app
      specs: ralph/logs/demo-toolbar-report-20260916.md, docs/src/components/Demo/Demo.tsx, docs/src/utils/getGitHubDemoUrl.ts
      status: not-started
      priority: medium
      blocked-by: [docs-chrome: demo frame structure (playground + code panel with copy and Show code / Hide code)]
      done-when: the demo toolbar renders upstream's more-actions control — a button with `aria-label="More actions"` (`docs/src/components/Demo/Demo.tsx:188`) opening a menu with exactly two items, `View source on GitHub` as a LINK carrying the source URL (`docs/src/components/Demo/Demo.tsx:194-198`) and `Copy link to source` as an item that copies that same URL and carries the accessible confirmation upstream uses (a visually hidden live region announcing the copy, `docs/src/components/Demo/Demo.tsx:200-210`) — built on this port's own `Menu` (`crates/leptos-ui/src/menu/`, done); the URL is produced by a PURE function over (repo, ref, source path, line range) with its expectation written out literally in a host test, and the rendered anchor's `href` is asserted against it, because a link's correctness is its href and not its label; verified additionally by `node ralph/scripts/probe-demo-toolbar.mjs --route <route>` reading the more-actions control on the port's side and by `node ralph/scripts/check-visual-budget.mjs --todo-id "<this id>"` not dropping the route's page score.
      note: UPSTREAM DERIVES THIS URL, IT DOES NOT STORE IT — `docs/src/utils/getGitHubDemoUrl.ts:19-34` builds the base from the `SOURCE_CODE_REPO` and `LIB_VERSION` environment values and `:56-76` walks from the demo file's own `import.meta.url` to its directory, appending the selected variant's kebab-cased subdirectory; this port has no such environment and no such per-demo file tree, so the port's URL must point at the port's own source: the page module that carries the demo (`crates/docs-app/src/pages/<name>_page.rs`) at the pushed ref of `noevaresearch/base-ui` (`migration-to-rust` is the only branch where that file exists). DECISION TO TAKE WHEN IMPLEMENTING, not a gap to guess around: the ref (a branch moves, so a pinned commit is the honest target for a link that is supposed to keep working) and the exact line range (the demo's own fn, not the whole page). The copy item's confirmation must be a real live region, not a title attribute: upstream's `aria-live="polite"` text is the observable a screen reader gets, and an icon swap alone proves nothing.

- [x] tooling: the CI scorecard's rendered axes are UNMEASURED on every route (and its package-alias axis contradicts a local green)
      crate: ralph/scripts (tooling — no crate)
      ...
      blocked-by: []
      status: done
      commit: d2bb3ea88900fbf72311223ca21910846512e99e (this item's done-marking commit: the ledger edit — status flip, the resolution note below, the new by-product item — plus the refreshed env-health.json the note cites; no crate or gate change, because the instrument fix and the CI write-back landed in 84286f3dec / ff74043d9, so this iteration's own product is the re-verification, the step-7 run and the decision record)
      blocker-resolved-by: `tooling: 50 TODO.md citation ranges were displaced by the ledger's own growth — re-anchor them` — `status: done`, commit 188df96ecbf22f0cd09db08a8a6eefe28896e706. The recorded block is kept VERBATIM on the `blocker:` line below (never rewritten); what it said and how it was re-checked is in the resolution note at the end of this entry.
      ...
      priority: high
      note: CLOSED 2026-09-16 — the recorded block and BOTH done-when clauses were RE-VERIFIED at this tree instead of inherited from the notes above, and step 7 is GREEN. (1) THE BLOCK IS RESOLVED, MEASURED: `node ralph/scripts/check-citations.mjs check` EXIT 0 over the whole `specs/` tree — 20399 citations, 0 failures, 8 warnings that are all "no recorded baseline" (a warning is never a pass); `node ralph/scripts/verify-citation-drift.mjs` EXIT 0 — 0 drifted, 0 CHANGED; and the SCOPED step this item's own `specs:` field drives (its real cause, per the owning item) now runs clean per entry — `ralph/generated/scorecard.jsonl`, `.github/workflows/measure-port.yml`, `ralph/scripts/check-page.mjs`, `ralph/scripts/scorecard-latest.mjs`, 0 failures each. Nothing is narrowed or reinterpreted to make this pass: the 50 displaced `TODO.md` ranges were repaired by the item named in `blocker-resolved-by`, and the checker's own header examples — the second half of the block's cause, recorded in the entry below — were unbackticked in 188df96ec. (2) CLAUSE 1, MEASURED: `node ralph/scripts/scorecard-latest.mjs --route react/components/checkbox` prints `measured 27m ago by CI (2026-09-16T20:14:01.490Z)` with EVERY axis MEASURED and `unmeasured 0` — structure PASS, page parity FAIL 85.18, widget parity PASS 100, snippet language PASS 0, example length FAIL 59.2, attribute density FAIL 0.32, copy coverage FAIL, react mentions PASS, package alias PASS. The four former UNMEASURED axes are now real page defects, which is the point of the item. Read from the COMMITTED record authored by `measure-port[bot]` (ff74043d9, record generatedAt 2026-09-16T20:14:01.490Z) — NOT from a local render, and no local renderer was written for evidence this box refuses to produce (`lib/browser-budget.mjs` refuses; env-health at this tree reads `memory BROKEN 3925MB/4096MB, oom_kill 51`). Whole-file scan of the committed scorecard, so the claim is not one route standing in for 17: 10 of 17 routes measure EVERY axis (accordion, button, checkbox, checkbox-group, field, fieldset, form, meter, otp-field, toggle) and 7 still carry 1-4 UNMEASURED axes (avatar 2, collapsible 1, separator 1, csp-provider 1, direction-provider 2, merge-props 4, use-render 3). Clause 1 names checkbox, and checkbox is fully measured; the residual 7 are reported here rather than smoothed over. Three of them (direction-provider, merge-props, use-render) are exactly the routes the metric-invariant checker flags and are handed to the item appended at the end of this file; the other four (avatar 2, collapsible 1, separator 1, csp-provider 1) are UNOWNED and are named here so the next iteration that touches a page sees them — an axis that could not be measured is never a pass and never a fail, and none of the four is claimed by this item's done-when. (3) CLAUSE 2, MEASURED: the CI `package alias` axis is PASS on all 17 routes and a local `node ralph/scripts/check-package-alias.mjs` at this tree EXITs 0 with `0 defect(s)` (`base-ui-leptos|base-ui-leptos|unpublished`, resolving from `.` AND from `test/node-resolution`). "At the same commit" was CHECKED, not assumed: `git diff --name-only ff74043d9..HEAD` touches TODO.md, ralph/logs/spec-discrepancies.md, ralph/scripts/check-citations.mjs and ~100 `specs/**` citation baselines, and NOTHING the alias gate reads (`crates/docs-app/src`, `packages/leptos`, `install_ref`, `package.json`) — so CI's reading and this box's reading are of the same inputs. (4) STEP 7 GREEN: `bash ralph/scripts/run-regression.sh "<this id>"` EXIT 0 at this tree — citation check clean per scope, `cargo test --workspace` green (366 + 1 + 11 + 416 + 281 + 74 + doctests, 0 failed, 0 ignored-as-evidence), TODO.md schema check `Parsed 186 TODO items ... Schema OK`, sandbox parity OK, part-surface advisory as designed. No docs-app build step applies (`crate:` is `ralph/scripts (tooling — no crate)`), so no route of this item's own was diffed. (5) BY-PRODUCT, HANDED TO ITS OWN ITEM rather than left unowned: `node ralph/scripts/gate-selftest.mjs` NO LONGER CRASHES — the `TypeError: fired.has is not a function` this entry's own note recorded is gone, the run completes and prints `checker self-test ok` — but it now reports 4 REAL invariant violations over the 18 committed `ralph/logs/visual/*-snippets.json`: `upstream-side-present` + `no-blocks-scored` on react/components/direction-provider (its committed report reads `upstreamElements 0, leptosElements 0, lengthSimilarity 0, snippetLanguages.total 0` and still carries `score: 40`) and `no-react-to-react-scoring` on react/utils/merge-props and react/utils/use-render (`reactToReactBlocks: 1` each). All three routes' own ledger items are already `done`, so no open item owns it; `status-report.mjs` DOES surface it honestly (`⚠ INSTRUMENT NOT SOUND — 4 of 18 snippet report(s) violate a metric invariant` plus "any axis derived from our snippet text is INVALID until gate-selftest is green"), so this is an ownership gap, not an invisible one — but `measure-port.yml:188` runs that step `|| true`, so the RUNNER's own job reports green either way. That is its own bounded item, appended at the end of this file (a workflow change needs the owner's authorisation per CONTEXT.md, so the item is scoped to the instrument and the ledger, and the workflow request belongs in ralph/logs/spec-discrepancies.md). STATED SO IT IS NOT MISTAKEN FOR A CLEAN CLOSURE: while those 4 violations stand, status-report flags the snippet-derived SIZE axes (`example length`, `attribute density`) as INVALID for every route — including the two this entry reads as FAIL 59.2 / 0.32 on checkbox. This item's done-when asks that those axes be MEASURED, and they are; whether the ruler producing them is sound is the new item's question, and it is the one that has to be answered before any docs page is failed on them.
      note: CI MEASUREMENT LANDED AFTER THE BLOCK, so BOTH clauses of this item's done-when are now verified by a real CI run: `measure-port.yml` dispatched on the branch at 913fc7a2b (run 35141983661, 24m19s, success) and its shard artifacts read here. (a) The four previously-UNMEASURED axes are MEASURED: `react/components/accordion` and `react/components/checkbox` both report `unmeasured=0`, and 10 of 17 routes now measure EVERY axis (accordion: page parity FAIL 89.2, widget parity FAIL 88.82, snippet language PASS 0, example length FAIL 42.8, attribute density PASS 1.38, copy coverage PASS, react mentions PASS, package alias PASS — three real page defects instead of four unmeasurable axes and an invented `example length 0`). (b) `package alias` is PASS on all 17 routes, agreeing with `check-package-alias.mjs` on this box at the same commit. The accordion snippet report now carries `upstreamSnippets 45, leptosSnippets 4, upstreamLines 474, leptosLines 203, lengthSimilarity 42.8` — the same numbers this box's last good sweep measured — where the PRE-fix run (35139826155 @ 63f2d7c43, downloaded and compared this iteration) had `upstreamSnippets 0, leptosSnippets 0, score 45, naming 100` on all 17 routes, `copy coverage` PASS everywhere and `example length` FAIL 0 everywhere. The instrument defect is therefore fixed AND measured; the item stays `blocked` only on step 7's citation gate, which belongs to another item.
      note: WRITE-BACK LANDED AND MEASURED (second dispatch, run 35145001793 @ 29b45ebac, write-back commit ff74043d9): the committed `ralph/generated/scorecard.jsonl` now carries records at 20:13-20:15, and this item's own done-when command agrees — `node ralph/scripts/scorecard-latest.mjs --route react/components/checkbox` prints `page parity 85.18, widget parity 100, snippet language 0, example length 59.2, attribute density 0.32, copy coverage FAIL, react mentions PASS, package alias PASS, unmeasured 0`. The FIRST dispatch's records (run 35141983661, generatedAt 20:05-20:20, read from that run's shard artifacts) never reached the branch: its aggregate's `git pull --rebase --autostash` CONFLICTED on the shared append-log (`error: could not apply 5dc6741... [measure] CI scorecard: 17 route(s) measured`), three retries, then `::warning::could not push results (branch busy); the next scheduled run will re-measure` — so the loop kept serving the PRE-fix records from the run before it and this item read as unverified. Diagnosed this iteration from the aggregate job's log and written up in `ralph/logs/spec-discrepancies.md` (items 6 and 7) as a workflow REQUEST: pull/rebase FIRST, then regenerate the dedup output from the merged file, then commit — the file is derived data, so recomputing it after a merge can never conflict.
      note: THE LANGUAGE NARROWING IS NOW MEASURED TOO, exactly as this entry predicted before it ran: `react/utils/merge-props` reads `snippet language FAIL = 1` and `react/utils/use-render` `FAIL = 5` (both read UNMEASURED in the previous run's records), i.e. the react-to-react P0 is visible again while the size axes stay UNMEASURED. Checkbox's remaining four axes are real page defects the docs lane can finally work on (page parity 85.18 < 90, example length 59.2 < 80, attribute density 0.32 < 0.8, copy coverage < 95) — a work list instead of four unmeasurable axes. STILL BLOCKED, and only on the citation gate its step 7 exposes: repair iteration should pick `tooling: 50 TODO.md citation ranges were displaced by the ledger's own growth — re-anchor them`, then re-run `bash ralph/scripts/run-regression.sh "<this id>"` and flip this entry to done — its own done-when is already met.
      note: STEP 0 RECORD, written BEFORE any implementation work — CHOSEN OVER the mechanical suggestion
...
- [x] tooling: two gates start Chromium with no browser-budget guard
      crate: ralph/scripts (tooling — no crate)
      ...
      blocked-by: []
      status: done
      commit: aabfe9eaa5253422967b86fab8c74ff9708dc719
      priority: high
      ...
      note: STEP 0 RECORD — chosen OVER the mechanical suggestion (`library: drawer`; `node ralph/scripts/pick-next-todo.mjs` re-run this iteration prints it). DRAWER, re-MEASURED this iteration rather than quoted: 33 non-test files / 4,792 LOC of source plus 10 test files / 13,498 LOC of tests across 15 subdirectories (`find packages/react/src/drawer -name '*.ts*' | xargs wc -l`), `needs-batched-mining: true` on its own entry, and a spec that is a whole-unit INDEX rather than a per-part one (`specs/library/drawer/implementation.md`, 387 lines: "State machine / hooks used", "Context providers/consumers", "DOM/portal strategy and why", "Dependencies on other Base UI internals"), so there is no per-part implementation spec to bound a sub-port against — a partial drawer port would be the fabricated-stub rot the 20260913 attempt left behind (the on-disk `crates/leptos-ui/src/drawer.rs` is still a 57-line dialog wrapper that hardcodes `modal: false`), and several prior entries record the same measurement while choosing elsewhere. NOT a blocked item first: exactly ONE `status: blocked` FIELD line exists in the ledger — `docs-content: components/accordion (prose + snippet completion)` — and its third dependency `docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)` is still `not-started`, so the block HOLDS; re-verified this iteration, not inherited from a note. WHY THIS ITEM INSTEAD: it is `priority: high`, bounded to four scripts, and it is the one class the environment's own verdict names as making every other measurement untrustworthy — a FALSE PASS. MEASURED, not hypothesised: `playwright-diff.mjs`, `visual-gap-report.mjs`, `visual-diff.mjs` and `probe-demo-toolbar.mjs` each started Chromium with no `refuseBrowserWork()` call, and `check-page.mjs` spawns the first of them, so a whole-page run on this 4096 MB cgroup started a real ~1.4 GB browser and scored the `structure` axis a live PASS while its five sibling axes honestly read UNMEASURED — the "guard in a file nobody runs" and "an unmeasured axis read as fine" pair, both already paid for twice. `run-regression.sh` additionally read that refusal (exit 2 = UNMEASURED, lib/browser-budget.mjs's contract) as "Playwright differential check failed" through a bare `|| fail`, which makes every route-shaped docs item locally un-closeable for a reason that was never about its page. No `blocked-by` was narrowed, blanked or rewritten to justify this pick.
      close-note: CLOSED THIS ITERATION — instrument only; no `crates/**` file, no page and no spec CLAIM was touched. WHAT LANDED: (1) `refuseBrowserWork()` at module top in the four Chromium launchers that lacked it — `playwright-diff.mjs`, `visual-gap-report.mjs`, `visual-diff.mjs`, `probe-demo-toolbar.mjs` (the last spawns the Chrome binary itself, never through `lib/browser.mjs`, which is why coverage has to be audited by CLASS); (2) `audit-instruments.mjs` gains class 7 `UNGUARDED BROWSER` — any script that starts Chromium (`launchChrome(` or a direct `spawn(CHROME…)`) without the guard, unless it carries an explicit `browser-guard-exempt: <reason>` marker — plus a `--strict-browser-guards` mode, and both strict classes are now enforced in `run-regression.sh`'s preflight (`--strict-phantoms --strict-browser-guards`); (3) `run-regression.sh`'s differential step captures the exit code and reports 2 as `DEFERRED: … (exit 2 = UNMEASURED)` with a NOT-a-pass line, instead of `|| fail "Playwright differential check failed"`. VERIFIED BY EXECUTION at this tree: `node ralph/scripts/check-page.mjs --route react/components/accordion` with `RALPH_BROWSER_GATES` unset completed with MAX chromium processes seen during the whole run = 0 (process table sampled every 100 ms for the run's lifetime; 0 real Chrome processes before and after), and its `structure` axis now reads UNMEASURED — "page did not mount (rebuild race or server) — not scored" — where the recorded Chromium run produced a live PASS; all four launchers exit 2 with the UNMEASURABLE banner and the "instead" pointer to `scorecard-latest.mjs`; `audit-instruments.mjs --strict-browser-guards` prints `0 unguarded browser launcher(s)` exit 0 (and `--strict-phantoms` still `0 phantom dependencies`), where the same check flags 4 launchers against HEAD's sources; step 7 `bash ralph/scripts/run-regression.sh "docs-content: components/switch"` EXIT 0, printing `DEFERRED: the differential did not run on this box — browser budget refusal (exit 2 = UNMEASURED)` and `cargo test --workspace` green (452 + 416 + 281 + 79 + 11 + 1 host tests, 0 failures). BEFORE-STATE, stated as the evidence it is rather than re-measured: HEAD's four launchers contain 0 `refuseBrowserWork` calls, and the false PASS is the finding entry's own recorded measurement — I did NOT reproduce it by starting a browser, because this box refuses browser work by design and the prompt forbids starting Chromium to check one's own work. INSTEAD THE LAUNCH SITE WAS PROVED LIVE WITHOUT ONE: with `RALPH_BROWSER_GATES=1` and `CHROME` pointed at a logging stub, `playwright-diff.mjs` spawns the browser binary with the real frugal flags (`--headless=new --no-sandbox --no-zygote --renderer-process-limit=1 … --remote-debugging-port=9777`, one spawn logged), and with the allowance unset the same command exits 2 and spawns nothing — so the guard, not a dead code path, is what stands between this box and a ~1.4 GB launch. CI IS UNCHANGED AND STILL MEASURES: `measure-port.yml` names its allowance (`RALPH_BROWSER_GATES: '1'`) and drives the same `check-page.mjs` per route, so the guards pass through there; read from the workflow, not from a CI run I could observe. GATE REPAIR THIS ITEM OWED, found by its own step 1: the citation check was RED at HEAD before any edit here — 2 hard failures, both in `specs/library/drawer/` (`behavior.md` citing `TODO.md:677-677`, `implementation.md` citing `TODO.md:662-677`), caused by the driver's own two `review-note:` lines growing that entry (the self-referential displacement mechanism this ledger records repeatedly, not a spec error). Both claims were re-verified TRUE against their new targets — line 674 carries `needs-batched-mining: true`, and the entry's extent is now 662-676 with 0 `wraps-external` occurrences — then re-anchored (677 → 674; 662-677 → 662-676) and re-recorded per the checker's own instruction, scoped per spec file so no other baseline was adopted blindly; both orphaned baselines are gone. RESULT: `check-citations.mjs check` EXIT 0 — 20408 citations across 287 spec files — with the soft-warning count unchanged at 38 before and after, so this iteration's ledger edit moved no citation. WHAT THIS CLOSE DOES NOT CLAIM: (1) no rendered axis was measured here — every browser axis stays UNMEASURED on this box by design and belongs to CI; (2) `verify-citation-drift.mjs` (advisory, wired into no gate) reports `34 possible change(s)`, all pre-existing soft drifts in OTHER units' windows — the citation check's own warning count is unchanged, so this iteration did not cause them; (3) the `[cron]` workspace-snapshot commit `aabfe9eaa` is where these scripts landed this iteration — the loop's tree is snapshotted by cron, so that commit, not this done-marking, carries the code; (4) this is a MEASUREMENT CHANGE and needs review as such: the bars did not move, but the differential's classification did (a refusal is now DEFERRED/UNMEASURED, never a failed check), and the budget's coverage is now machine-checked instead of kept by hand.

- [x] tooling: 50 TODO.md citation ranges were displaced by the ledger's own growth — re-anchor them
      crate: ralph/scripts (tooling — no crate)
      specs: ralph/scripts/check-citations.mjs, ralph/scripts/verify-citation-drift.mjs, specs/library/*.citations.json
      blocked-by: []
      status: done
      commit: 188df96ecbf22f0cd09db08a8a6eefe28896e706
      priority: high
      ...
      note: FOUND 2026-09-16 by `tooling: the CI scorecard's rendered axes are UNMEASURED on every route`, whose full-workspace regression exposed it: 50 citations across 43 `specs/library/**` files cite `TODO.md` windows that no longer contain the text they were recorded against, so `run-regression.sh`'s citation step is RED at HEAD for every item whose `specs:` field names no file under `specs/` — items with specs get a SCOPED check and never see it, which is why the lane did not notice for several iterations. MEASURED, not assumed: with HEAD's own `TODO.md` restored (my TODO edits set aside) the same 50 fail, so this iteration did not cause them; the stale ranges are all in `TODO.md:368-616`, i.e. the Phase B entries, and their recorded oracles come from revision 741a85166. The checker's own tolerance (`findUniqueNearbyDrift`, ±40 lines) cannot absorb it, and neither can a re-wrap comparison: ledger growth displaced the ranges by far more than 40 lines AND the entries themselves grew notes, so no verbatim window survives — `verify-citation-drift.mjs` (added by that iteration) reports 48 of the 50 as genuinely different text at the cited range. A blanket `record` run would therefore adopt whatever text happens to sit at a stale range, which is the "measure that rewards copying" defect in citation clothing. Precedent for the repair shape: the seven earlier rounds recorded in the Phase A `utils:` entries — re-anchor to the claim's true target, then re-record. DONE 2026-09-16 — the repair LANDED and both instruments are green at this tree: 54 citations were mis-anchored and all 54 were re-anchored to their claim's TRUE target and re-recorded — the 50 hard failures this item was created for (43 spec files) PLUS 4 that the checker's ±40-line tolerance had masked as soft "moved by +13/+19 line(s) — window content identical" warnings (context-menu ×2 specs, progress, tabs, toolbar). MEASURED, not inferred: those offsets land on NEIGHBOURING units' note text (the +13 answer for context-menu is combobox's note tail, tabs' is checkbox-group's), and every one of those neighbours also carries no `wraps-external:` field — which is why the mis-anchor read as consistent, and why a blanket `record` run would have made it permanent. Also repaired: the second hard failure was a citation whose PATH did not resolve (`demos/multiple/css-modules/index.tsx:17` → the repo-relative `docs/src/app/(docs)/react/components/accordion/demos/multiple/css-modules/index.tsx:17`, whose line 17 is the quoted sentence), and a cross-spec citation my own edit disturbed (`specs/library/number-field/implementation.md:25` cites scrub's `:5-37`, whose ±2-line window spans line 3, so rewriting a citation token there turned it red — re-recorded with the claim verified intact). 4 orphaned baselines (keys no spec cites any more, each a permanent CHANGED in verify-citation-drift's eyes) were removed. EVIDENCE: `check-citations.mjs check` EXIT 0 — 20399 citations across 287 spec files, 0 failures, 0 drifted windows (the 8 remaining warnings are all "no recorded baseline": 7 pre-existing plus the toolbar case below); `verify-citation-drift.mjs` EXIT 0 — 0 drifted, 0 CHANGED (was 50 drifted / 48 CHANGED); `run-regression.sh "<this id>"` EXIT 0 (cargo test --workspace green — 366 + 416 + 281 + … 0 failures; audit-instruments 0 phantom dependencies). ONE CASE IS NOT CLAIMED AS RE-ANCHORED: `specs/library/toolbar/implementation.md:6` claims a `TODO.md` entry for `library: toolbar`, and NO SUCH ITEM EXISTS (Phase B holds 38 items, none of them toolbar, while the unit has mined specs, an entry in `ralph/generated/components.json`, no `crates/leptos-ui/src/toolbar*` module, and a docs item at `TODO.md:1877` whose `owner: library: toolbar` names the missing entry — a prior commit had already rewritten that item's `blocked-by` off the phantom, so the ledger knows and the spec was never told). Its baseline had been recorded against combobox's entry; rather than adopt another unit's text — the "measure that rewards copying" defect this item's own note predicted — that baseline was WITHDRAWN, so the citation now reports UNRECORDED (a warning, never a pass), and the underlying gap was handed to its own item (`tooling: library: toolbar has no ledger item — the unit cannot be scheduled, and its mined spec cites the entry that does not exist`) with the full mechanism recorded in `ralph/logs/spec-discrepancies.md`. THIS ITEM'S OWN STEP 7 ALSO SHOWED ITS RECORDED BLOCKER CAUSE WAS WRONG: the blocker note on the entry above says run-regression cannot scope this check and falls back to the whole `specs/` tree, but MEASURED it scopes to EACH `specs:` entry and failed on `ralph/scripts/check-citations.mjs` ITSELF — the checker's header comments carried five backticked illustrative citations (`path/to/file.ext:123` ×4 and a docs-route example with an extension), which its own regex read as citations to files that do not exist, so this item could not pass its own gate for a reason unrelated to any ledger drift. Fixed at the root: those examples are deliberately unbackticked now, with the pitfall documented in the header (comment-only change; the checker still checks the same 20399 citations across 287 files before and after, so nothing was blinded). SCOPE NOTE, stated rather than glossed: for a tooling item whose `specs:` field names scripts, the SCOPED citation step measures 0 citations, so this repair's proof is the whole-tree run and `verify-citation-drift`, not that step.


- [ ] tooling: `library: toolbar` has no ledger item — the unit cannot be scheduled, and its mined spec cites the entry that does not exist
      crate: ralph/scripts (tooling — no crate)
      specs: specs/library/toolbar/implementation.md, specs/library/toolbar/behavior.md
      blocked-by: []
      status: not-started
      priority: high
      done-when: the ledger and the unit's own spec agree on where `toolbar` lives — either a Phase B item for the `toolbar` unit exists (carrying the fields every other component carries: crate, specs, blocked-by, status, done-when, docs-pair) or the ledger records, with a citation, that the unit is out of this port's scope — and in whichever case `specs/library/toolbar/implementation.md`'s claim about the unit's `TODO.md` entry and `docs-content: components/toolbar`'s `owner:`/`blocked-by:` fields agree with that state; verified by `node ralph/scripts/check-citations.mjs check` exiting 0 with the toolbar citation carrying a RECORDED baseline again (it is deliberately UNRECORDED today — a warning, never a pass — because the entry it names does not exist and the text sitting at the cited range belongs to another unit), by a reconciliation that NAMES every unit having a mined spec under `specs/library/**` and/or an entry in `ralph/generated/components.json` but no Phase B item, going from 1 unexplained unit (toolbar) to 0 so the next dropped unit is reported instead of found by hand, and by `bash ralph/scripts/run-regression.sh "<this id>"` exiting 0.
      note: FOUND 2026-09-16 by `tooling: 50 TODO.md citation ranges were displaced by the ledger's own growth — re-anchor them`, whose repair could not re-anchor this one citation and therefore handed it here rather than recording it wrongly. MEASURED, not inferred: `TODO.md`'s Phase B section holds 38 items and none of them is toolbar, while `specs/library/toolbar/{behavior,implementation}.md` exist as mined specs, `ralph/generated/components.json` lists the unit, `crates/leptos-ui/src/` has no `toolbar` module (so nothing of the unit is ported), and `docs-content: components/toolbar` (`TODO.md:1877`) carries `owner: library: toolbar` — an owner that does not exist. THE LEDGER ALREADY HALF-KNOWS: a prior commit (`7da150f4b`, `[ralph][citation-fix] Fix broken citations and TODO schema violations`) rewrote that docs item's `blocked-by` from `[library: toolbar, docs-app: routing + layout shell]` to `[library: menubar, docs-app: routing + layout shell]` — i.e. the phantom DEPENDENCY was removed exactly as the phantom `[Phase A complete]` dependency was removed elsewhere, but the spec's claim that the entry exists was never revisited, and the missing ITEM was never restored. WHY THE CITATION CHECKER CANNOT SEE IT (also logged in `ralph/logs/spec-discrepancies.md`): the cited lines at `TODO.md:565-571` exist and are non-empty, so nothing fails to resolve; only the hash can fail, and the recorded hash had been computed against whatever text sat there (measured: it is combobox's entry, not toolbar's), which the ±40-line tolerance then reported as a soft "moved by +19 line(s)" warning that exits 0. A `record` run would have made the mis-anchor PERMANENT, which is why the false baseline was withdrawn (leaving the citation loudly unrecorded) instead of re-recorded, and why the gap is a ledger item rather than a footnote. SCOPE NOTE, so the next iteration does not repeat the mistake: authoring a Phase B entry by hand would shift every line number this iteration's repair just re-anchored (all 54 targets sit in `TODO.md:373-1124`), so the fix belongs to whatever generates the Phase B list, not to a citation iteration.

- [x] tooling: three routes' committed snippet reports fail the metric-invariant checker, and CI swallows that checker's exit code
      crate: ralph/scripts (tooling — no crate)
      specs: ralph/scripts/lib/metric-invariants.mjs, ralph/scripts/gate-selftest.mjs, ralph/scripts/snippet-ergonomics.mjs, ralph/scripts/status-report.mjs, ralph/logs/visual/direction-provider-snippets.json, ralph/logs/visual/merge-props-snippets.json, ralph/logs/visual/use-render-snippets.json
      blocked-by: []
      status: done
      commit: 3d103d55a1fb7c26452f5946b492747d14afd340
      priority: high
      close-note: CLOSED THIS ITERATION — instrument only; no `crates/**` file, no spec and no page was touched, and no `blocked-by` was narrowed, blanked or rewritten. WHAT LANDED, and where each clause of the done-when is satisfied: (a) "gate-selftest exits 0 over the committed reports with its own controls green" — `node ralph/scripts/gate-selftest.mjs` EXIT 0 at this tree: 10/10 invariant fixtures (both directions), 6/6 producer-rule cases, naming-parity ok, `18 report(s) checked, 0 invariant violation(s)`; (b) "the two classes enforced WHERE THE NUMBER IS PRODUCED rather than only audited afterwards" — the rule now has ONE home, `ralph/scripts/lib/metric-invariants.mjs` (the file whose own header exists to stop a checker drifting from its subject): `no-react-to-react-scoring` TIGHTENED to fire only when a score was PUBLISHED over such a pair (it previously fired on mere presence, which made it unsatisfiable by correct behaviour — `snippet-ergonomics.mjs` already excludes those blocks from every metric and raises a P0 for them), `no-score-from-excluded-extraction` ADDED for the general case (`blocksExcludedFromScoring >= leptosSnippets > 0` with a published score), and `scoreWithheldReason()` ADDED as the PRODUCER's half of both, which `snippet-ergonomics.mjs` now imports and uses to publish `score: null` + `scoreWithheld` + `scoreWithheldReason` instead of a number. Withholding does not soften the verdict — such a page still FAILS (exit 1) — it only stops a number nobody measured from being quotable, and the numeric diagnostic is kept out of the JSON on purpose. Evidence clause: `node ralph/scripts/status-report.mjs` no longer prints `INSTRUMENT NOT SOUND`, and `example length` / `attribute density` no longer carry `INVALID (instrument broken)` on all 17 routes. THE THREE REPORTS ARE WITHDRAWN, NOT RE-MEASURED: they were voided into the refusal shape the consumers already handle (`refused: true, score: null`, counters preserved under `originalReport`, .md score line replaced by a WITHDRAWN notice), because this box cannot re-measure them — `ralph/generated/env-health.json` reports `browser: DEGRADED` and `snippet-ergonomics.mjs` refuses here by design (exit 2) — so their axes read UNMEASURED, which never passes and never fails. THE UNDERLYING PAGE DEFECT IS OWNED, so voiding does not lose the finding: the two pages that still teach upstream's React API belong to `docs-chrome: snippet translation (batch 4)`, whose `routes:` field names exactly utils/use-render, utils/merge-props and utils/direction-provider (TODO.md, that item's own entry). VERIFIED BY EXECUTION: the new tests are NOT VACUOUS (A/B — breaking `scoreWithheldReason` fails the producer cases and exits 1; the file was restored byte-identical), and the tightened invariant is NOT BLIND (evaluated against the ORIGINAL blobs from HEAD, `direction-provider` still fires `upstream-side-present` + `no-blocks-scored` and `merge-props`/`use-render` still fire `no-react-to-react-scoring` + `no-score-from-excluded-extraction`, while the voided versions and a healthy report evaluate CLEAN); `node ralph/scripts/check-citations.mjs check` EXIT 0 over 20408 citations; `bash ralph/scripts/run-regression.sh "<this id>"` printed `=== REGRESSION OK ... ===` EXIT 0. A REAL DIAGNOSIS THIS TURNED UP: `direction-provider`'s collapsed extraction was a ROUTE-SPELLING error, not a page defect — the artifact named `react/components/direction-provider`, which is not a route in this port (the page is served at `react/utils/direction-provider`: ralph/generated/routes.json:17, crates/docs-app/src/lib.rs:80, crates/docs-app/src/chrome.rs:123, crates/docs-app/src/pages/direction_provider_page.rs:2,273), so the extractor fetched a URL that exists on neither side and the empty-input ratios scored 100% by default; because the artifact is keyed by the route's BASENAME, the wrong spelling landed in the RIGHT filename — the file looked healthy by name while naming a page that does not exist. Recorded in `ralph/logs/spec-discrepancies.md` with the follow-up (a producer-side guard rejecting a route absent from `ralph/generated/routes.json` before launching a browser). WHAT THIS CLOSE DOES NOT CLAIM: (1) the two voided pages still teach upstream's React API — untouched here, owned by the batch-4 item above; (2) those routes read UNMEASURED until a browser-capable run regenerates the reports; (3) the workflow paths stay unedited — `.github/workflows/measure-port.yml:188` runs `node ralph/scripts/gate-selftest.mjs || true` (the checker's exit code is swallowed, so the audit can never fail the run) and CI commits only `ralph/generated/scorecard.jsonl` (line 248), so a stale snippet report can never be refreshed by CI — both recorded as REQUESTS in `ralph/logs/spec-discrepancies.md` per CONTEXT.md's owner-authorisation rule, and the same gap is measured locally (`grep -n "gate-selftest\|status-report" ralph/scripts/run-regression.sh` matches nothing).
      step0-note: CHOSEN OVER the mechanical suggestion (`library: drawer`; `node ralph/scripts/pick-next-todo.mjs` re-run this iteration still prints it), and this line is written after the work landed rather than before it, so it does NOT claim the "recorded before any implementation work" ordering the sibling entries use. WHY NOT DRAWER: re-MEASURED this iteration rather than quoted — `find packages/react/src/drawer -name '*.ts*' | xargs wc -l` gives 4,724 LOC of non-test source over 31 files plus ~13.5k LOC of tests across 11 subdirectories, the entry itself carries `needs-batched-mining: true`, and the ledger's own record of the 20260913 attempt on it is a fabricated dialog-wrapper stub; six later iterations recorded the same measurement while choosing elsewhere, so picking it yields no done-ness and no unblocked pair. WHY NOT A BLOCKED ITEM FIRST: the ledger's only `status: blocked` item is RE-VERIFIED rather than trusted (`grep -n "^      status: blocked" TODO.md` = one line, `docs-content: components/accordion (prose + snippet completion)`), whose third dep `docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)` is still `not-started`, so that block HOLDS and no broken-thing-first candidate outranks this pick. WHY THIS ITEM: it was RED at this tree, measured before any edit — `node ralph/scripts/gate-selftest.mjs` EXIT 1 with "18 report(s) checked, 4 invariant violation(s)", and `node ralph/scripts/status-report.mjs` printing `INSTRUMENT NOT SOUND` with `example length` and `attribute density` marked INVALID on ALL 17 measured routes, i.e. the product measure the owner judges the port by was reading two axes it could not measure. It is also OWNERLESS, which is why it rotted: the three routes' own ledger items (`docs-content: utils/direction-provider`, `docs-content: utils/merge-props`, `docs-content: utils/use-render`) are all `done`, so what the checker flagged was a CLOSED item's evidence and no open item owned it — and the owner's stated rule "a measure that rewards copying is a defect" names the React-to-React half exactly. Nothing in this entry's `blocked-by` was narrowed, blanked or rewritten.
      done-when: `node ralph/scripts/gate-selftest.mjs` exits 0 over the committed `ralph/logs/visual/*-snippets.json` with its own fixture controls still green (or, for every violation that remains, this ledger names with a citation the item that owns that route's defect), AND the two classes it flags are enforced WHERE THE NUMBER IS PRODUCED rather than only audited afterwards — a report that extracted zero upstream snippets must write UNMEASURED rather than a score (`upstream-side-present`, `no-blocks-scored`), and a block whose two sides are both upstream's React must never be admitted to scoring (`no-react-to-react-scoring`) — so that no route's size axes can read a number derived from empty input. Evidence is the checker's own output plus `node ralph/scripts/status-report.mjs` no longer printing `INSTRUMENT NOT SOUND`, and `node ralph/scripts/snippet-ergonomics.mjs --route <the repaired route>` still measuring the same two sides. NOT PART OF THIS ITEM: `.github/workflows/measure-port.yml` — release/workflow paths need the owner's authorisation (CONTEXT.md), so the `|| true` on the self-test step is recorded as a REQUEST in `ralph/logs/spec-discrepancies.md`, never edited here.
      note: FOUND 2026-09-16 by `tooling: the CI scorecard's rendered axes are UNMEASURED on every route (and its package-alias axis contradicts a local green)` while re-verifying that item's done-when at the tree that closed its citation block. MEASURED, not inferred: `node ralph/scripts/gate-selftest.mjs` EXIT 1 — its 8 fixture controls plus the naming-parity self-test all report `ok` (so the checker itself is sound), then "invariants over real reports": `18 report(s) checked, 4 invariant violation(s)`. Route by route: (a) `react/components/direction-provider` fires `upstream-side-present` AND `no-blocks-scored` — its committed report (`generatedAt 2026-09-16T17:24:08.007Z`) reads `upstreamElements 0, leptosElements 0, upstreamLines 0, leptosLines 0, lengthSimilarity 0, snippetLanguages.total 0` and STILL carries `score: 40`, i.e. a number that is an artefact of arithmetic on nothing, and upstream's reference for that route was never loaded into the instrument; (b) `react/utils/merge-props` and (c) `react/utils/use-render` fire `no-react-to-react-scoring` with `reactToReactBlocks: 1` each — a React block scored against upstream's React block, which is the owner's stated defect class ("a measure that rewards copying"), and they are the same two routes the CI scorecard already reports as `snippet language FAIL 1` and `FAIL 5`. WHY IT IS NOBODY'S YET: all three routes' own ledger items (`docs-content: utils/direction-provider`, `docs-content: utils/merge-props`, `docs-content: utils/use-render`) are `status: done`, so what the checker is flagging is a CLOSED item's evidence, and no open item owns it. VISIBILITY, stated precisely rather than as a slogan: this is an OWNERSHIP gap, not an invisible one — `ralph/scripts/status-report.mjs:79-83` surfaces it (`⚠ INSTRUMENT NOT SOUND — 4 of 18 snippet report(s) violate a metric invariant`, plus "any axis derived from our snippet text is INVALID until `node ralph/scripts/gate-selftest.mjs` is green", naming `example length` and `attribute density` as the invalid axes) — but `.github/workflows/measure-port.yml:188` runs `node ralph/scripts/gate-selftest.mjs || true`, so on the runner the step that exists to "prove the ruler before anyone reads the numbers" reports green whether it crashes or finds violations. That masking is not hypothetical: the same step silently swallowed the `TypeError: fired.has is not a function` this item's finder recorded in its notes, while the runner went on publishing axes derived from the broken reports. WHAT IS AND IS NOT REROUTED TODAY: `ralph/scripts/check-page.mjs:27,107-133` DOES consume `lib/metric-invariants.mjs` and nulls the size-derived axes to UNMEASURED when a report violates an invariant, so the page scorecard refuses to score those axes — but the report-WRITING gate (`snippet-ergonomics.mjs`) does not, which is why a scored-on-nothing report still reaches disk (`ralph/logs/visual/<name>-snippets.json`) and is then re-read by every later run. SCOPE, so the next iteration does not turn this into a snippet-translation job: repairing the two `reactToReactBlocks` reports by translating those pages' React blocks belongs to the snippet-translation lane (`docs-chrome: snippet translation` batches), and the `direction-provider` report's emptiness is an instrument-side question (why the reference side loaded nothing) — this item owns the INSTRUMENT and the ledger, which is why its done-when is the checker being green or the violations being owned, not the pages being rewritten. Cross-reference: the docs lane reads these two size axes as bars (`example length >= 80% of upstream`, `attribute density >= 0.8x`), so until this item closes, a page can be failed on a number `status-report.mjs` itself calls INVALID.

- [ ] library: the view paths drop `render`'s element form — a dynamic tag needs a handler-capable pattern this crate does not have yet
      crate: base-ui-leptos
      specs: crates/leptos-ui/src/toggle_group.rs, specs/library/toggle-group/behavior.md
      blocked-by: [tooling: the crate DOM/wasm suite runs nowhere but this box — no CI job runs it and this box refuses browsers by design]  # NARROWED WITH EVIDENCE, not convenience: the fix is a DOM behaviour change and the wasm suite is its only verifier (`#[wasm_bindgen_test]`, 24 files in this crate), so the dependency is real rather than a section heading — the class of phantom dep this ledger already paid a day for
      status: not-started
      priority: medium
      done-when: `toggle_group_view` renders the tag its RESOLVED description carries (`RenderedElement::tag`), so `ToggleGroupElementProps.render_class_style.render = Some(RenderProp::Element { tag, .. })` substitutes the element while the composite root's `keydown`/`focus` handlers stay attached, proven by a wasm test that mounts the view path and asserts both the substituted tag and one keyboard behaviour through it; `#[component] ToggleGroup` exposes `render` only once that holds; verified by `cargo test -p base-ui-leptos --target wasm32-unknown-unknown` green in a real browser, plus `bash ralph/scripts/run-regression.sh "<this id>"` exiting 0
      note: FOUND 2026-09-16 while closing `library: toggle-group`, which for this reason does NOT expose `render` on its component surface (its `class_style_bag` fixes `render: None`). MEASURED, not inferred: the engine DOES resolve the element form — `toggle_group_element` with `RenderProp::Element { tag: "section" }` returns `rendered.tag == "section"`, pinned by this iteration's `the_render_element_form_selects_the_resolved_tag` — but `toggle_group_view` builds `view! { <div node_ref=root_node …> }`, so a caller who materializes the description gets the substituted tag while a caller who uses the component gets a `div` whatever it asks for. WHY THE FIX IS NOT A ONE-LINER, and why it was not smuggled into the closing iteration: the crate's tag-aware view convention (`custom(tag).node_ref(..).child(..)` — `avatar/root.rs:342`, `fieldset/root.rs:322`, `fieldset/legend.rs:311`, `otp_field.rs:1840,1981,2078`) is used only by parts that attach NO event handlers to that node, and the one tag-aware view WITH handlers (`checkbox/root.rs:1575-1600`) is an `Either` that works because that component has exactly two possible tags; toggle-group needs an arbitrary tag AND the composite root's `keydown`/`focus` handlers, and `grep -rn '\.on(ev::\|\.on('` finds no `.on(..)` on a `custom(..)` element anywhere in `crates/leptos-ui/src/`, so the pattern would be INVENTED here rather than reused — and a DOM-only change that this box cannot verify (`ralph/generated/env-health.json`: `browser: DEGRADED — browser gates REFUSED here by lib/browser-budget.mjs`) would be an unverified claim, which is what the `blocked-by` above records instead.

- [ ] tooling: the crate DOM/wasm suite runs nowhere but this box — no CI job runs it and this box refuses browsers by design
      crate: ralph/scripts (tooling — no crate)
      specs: ralph/scripts/lib/browser-budget.mjs, .github/workflows/measure-port.yml
      blocked-by: []
      status: not-started
      priority: high
      done-when: the port's wasm suites have a runner that is not this box — a CI job executes `cargo test --target wasm32-unknown-unknown` for every crate carrying DOM tests, its result is COMMITTED the way `measure-port.yml` commits the route scorecard so a later iteration READS it instead of re-running it, and a run that executed zero tests reports UNMEASURED rather than green; verified by that job's committed record naming the crate(s) and the test count, plus `bash ralph/scripts/run-regression.sh "<this id>"` exiting 0
      note: FOUND 2026-09-16 while closing `library: toggle-group`. MEASURED, not inferred: no workflow runs the crate's wasm tests — `grep -n "cargo test" .github/workflows/*.yml` matches no wasm-target invocation, and `measure-port.yml`, the only job that installs `wasm-bindgen-test-runner` (its ":135" tool-install step) runs `check-page.mjs` per route and never a `cargo test`; the suites are executed only by hand through `/data/scripts/run-wasm.sh <crate>` (Chrome for Testing 153 via `/data/tools/chrome-wrapper.sh`), which is how `library: field` recorded 14/14 and `library: otp-field` 5/5. WHY IT IS NOW A BLIND SPOT RATHER THAN A CONVENIENCE: `lib/browser-budget.mjs` refuses browser work here BY DESIGN (one chromium ≈ 1.4 GB of a 4096 MB cgroup) and `env-health.json` reports `browser: DEGRADED`, so from 2026-09-16 the port's entire DOM contract is unverifiable in the place it is edited, while `cargo test --workspace` reports green WITHOUT COMPILING those modules at all (`#[cfg(target_arch = "wasm32")]`). Scale, counted rather than remembered: 112 source files carry `#[wasm_bindgen_test]` — `crates/leptos-ui` 24, `crates/leptos-ui-internals` 59, `crates/leptos-ui-utils` 28, `crates/docs-app` 1. That is the "an instrument that cannot run reads as green" class at suite scale, and it is why the two newest component closes (`library: toggle-group` here, and every `library:` item before it) rest on host evidence only. The `done-when` deliberately does NOT prescribe editing `.github/workflows/**` — that path needs the owner's authorisation (CONTEXT.md), so the shape is a REQUEST recorded in `ralph/logs/spec-discrepancies.md` unless the owner authorises the workflow change.

- [ ] library: Field.Control's port surface carries no `ref` slot — `Input` cannot forward `ref`, and neither can any other Field consumer
      crate: base-ui-leptos
      specs: specs/library/input/implementation.md, specs/library/field/implementation.md
      blocked-by: []
      status: not-started
      priority: medium
      done-when: `FieldControlViewProps` (or the `FieldControl` component) accepts a ref slot and the control's view invokes it with the rendered `<input>` node, so `<Input .. ref_callback=..>` and `<FieldControl .. ref_callback=..>` hand the caller the same element upstream's merged ref does; verified by a wasm test that mounts both and asserts the callback received the element whose tag name is `input` (upstream's `refInstanceof: window.HTMLInputElement`), plus `bash ralph/scripts/run-regression.sh "<this id>"` exiting 0
      note: FOUND 2026-09-16 while closing `library: input`, which for this reason does NOT expose `ref` on its component surface. MEASURED, not inferred: `grep -rn "ref_callback\|RefCallback" crates/leptos-ui/src/field/` matches NOTHING — the field unit's two node refs are internal `NodeRef`s (`crates/leptos-ui/src/field/field_control.rs:196`, `crates/leptos-ui/src/field/field_root.rs:449`) — while the crate's OTHER units do expose one (`crates/leptos-ui/src/otp_field.rs:1924`, `ref_callback: Option<RefCallback<Element>>`). So the gap is the FIELD unit's, not `Input`'s: upstream's `Input` forwards its ref straight through (`packages/react/src/input/Input.tsx:14`, its return at `:16`) and `useRenderElement` merges it with the control's own (`packages/react/src/field/control/FieldControl.tsx:128`, `packages/react/src/field/control/FieldControl.tsx:135`), which is what the refForwarding conformance suite asserts (`packages/react/test/conformanceTests/refForwarding.tsx:32-38`) — and a delegation wrapper cannot add a ref path its delegation target does not have. WHY IT IS NOT FIXED IN THE CLOSING ITERATION RATHER THAN SCOPED: adding the slot changes the field control's own view path (the node-creation effect that currently just binds `input_node`), i.e. a DOM behaviour change whose only verifier is the crate's wasm suite — which this box refuses to run by design (`ralph/generated/env-health.json`: `browser: DEGRADED`) and which no CI job runs (`tooling: the crate DOM/wasm suite runs nowhere but this box`), so it would ship as an unverified claim. Same reasoning that scoped the sibling `render` gap instead of absorbing it (`library: the view paths drop render's element form`); both are also written up in `ralph/logs/spec-discrepancies.md`.


- [ ] library: menu — the view layer (Positioner/Portal/Popup/Item) is a fabricated stub, not a port
      crate: base-ui-leptos
      specs: specs/library/menu/behavior.md, specs/library/menu/implementation.md
      blocked-by: []
      status: not-started
      priority: high   # the parts are what every menu-family docs page composes, and what `library: menubar` reduces over.
      step0-note: CHOSEN OVER the mechanical suggestion (`library: drawer`; `node ralph/scripts/pick-next-todo.mjs` prints it), and this line is written after the work landed rather than before it, so it does NOT claim the "recorded before any implementation work" ordering the sibling entries use. WHY NOT DRAWER: re-MEASURED this iteration rather than quoted — `find packages/react/src/drawer -name '*.ts*' | xargs wc -l` = 18,290 lines over 32 files (4,763 non-test), the entry itself carries `needs-batched-mining: true`, and `crates/leptos-ui/src/drawer.rs` is still the 57-line placeholder. The recent iterations whose mechanical pick WAS drawer each OVERRODE it rather than porting it — VERIFIED against the driver's own logs, not inferred: `ralph/logs/stage3/hermes-library--drawer--20260917-000730.log` and `--20260917-021433.log` each record `CHOSEN OVER the mechanical suggestion (library: drawer …)` and an item that is not this one (a `tooling:` item in both), and the three `review-note: MEASUREMENT TOOLING CHANGED` lines sitting on THIS entry come from those overridden iterations — the driver labels the iteration log by the pick, so a drawer-named log is not evidence that drawer was worked. Correcting that attribution matters: the earlier reading of those notes ("drawer iterations landed tooling commits") would have been a false claim about the ledger's own history. WHY NOT A BLOCKED ITEM FIRST: re-verified rather than trusted — `grep -n "^ *status: blocked" TODO.md` is exactly one line (`docs-content: components/accordion (prose + snippet completion)`, L2512) and its third dep `docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)` (L3391) is still `not-started`, so that block HOLDS. WHY THIS ITEM: it is broken state in a `done` item — `library: menu` is `status: done` while 14 files under `crates/leptos-ui/src/menu/` still carry the fabricated marker (15 before this iteration) and three of the four parts the crate COMPILES are facades, a defect whose own note records it as the precondition for `library: menubar` and for every menu-family docs page. No `blocked-by` was narrowed, blanked or rewritten to justify this pick; the entry's field was already precise and is left untouched.
      progress-note: PARTIAL, this iteration — `Menu.Item` is now a real port; the other parts of this item are NOT done and are named below with measurements. WHAT LANDED (crate `base-ui-leptos` only; no other crate, no spec file): (a) `crates/leptos-ui/src/menu/item.rs` rewritten from the facade into a translation of `MenuItem.tsx` + `useMenuItem.ts` + `useMenuItemCommonProps.ts` — the id via `useBaseUiId` (`MenuItem.tsx:35`), `role="menuitem"` and `tabIndex = open && highlighted ? 0 : -1` (`useMenuItemCommonProps.ts:62-63`), the composite-list-item registration (`MenuItem.tsx:33`) that gives the item its index, `data-highlighted`/`data-disabled` (`MenuItemDataAttributes.ts:3,6`), the disabled fold `disabledProp || rootDisabled` (`MenuItem.tsx:38-39`), and the `onClick` close request with the `itemPress` reason through the unit's ONE mutation gate (`useMenuItemCommonProps.ts:81-85`); the four invented "hooks" (`use_menu_item`, `use_menu_item_id`, `use_menu_item_disabled`, `use_menu_item_close_on_click`) and the hardcoded `class="menu-item"` shell are gone — the latter is what `specs/library/menu/behavior.md` "Uniform DOM shell" forbids; (b) the item's store reads added in `menu/store.rs` (`use_menu_active_index_signal`, `use_menu_disabled_signal`, `menu_item_is_active`, `menu_item_on_click`, `menu_item_tab_index`, `menu_item_state_attributes`, `MenuItemMetadata`, each cited) — the store-level `itemProps` bag is deliberately NOT read because its only upstream writer is the constructor's `EMPTY_OBJECT` seed (`MenuStore.ts:228,34,86`), measured, so merging it is a no-op; (c) the namespaced surface `Menu::Item`, which required `pub mod menu;` in `src/lib.rs` (it was crate-private, E0365) plus `pub use self::menu as Menu;` — the alias every other namespaced unit carries (`lib.rs:98-131`); (d) 6 host tests in `menu_tests.rs` (item_host_tests) asserting the close request's arrival at the gate with the recorded reason, `closeOnClick: false` making NO request at all, the tab-index truth table, the state-attribute pair, the highlight read against the store's `activeIndex`, and the resolved element description plus the documented prop defaults. MEASURED AT THIS TREE: `cargo check -p base-ui-leptos` EXIT 0; `cargo test -p base-ui-leptos --lib` 458 passed / 0 failed / 0 ignored; `node ralph/scripts/check-part-surface.mjs --components menu` 0/20 -> 1/20; `node ralph/scripts/check-component-strict.mjs --component menu` — namespaced path OK (was FAIL: the unit exported no capitalised alias), sections OK, hygiene OK (17 tests, none ignored), parts FAIL 1/20; `node ralph/scripts/check-citations.mjs check` EXIT 0. WHY THIS ITEM IS NOT MARKED DONE, stated as the clause it fails: its own done-when asks for `check-part-surface.mjs --components menu --strict` to exit 0, i.e. all 20 documented parts, and for `grep -rn "In a real implementation" crates/leptos-ui/src/menu/` to be empty. Measured now: part surface 1/20; the grep still matches 14 files. WHAT REMAINS, in the order the next iteration should take it: (1) `menu/portal.rs`, `menu/positioner.rs`, `menu/popup.rs` are the other three parts the unit COMPILES (`menu/mod.rs:29-32,58-60`) and all three are still facades — Portal renders a `<div class="menu-portal">` where upstream renders NOTHING when unmounted and portals otherwise (`MenuPortal.tsx:20-21,24-30`), Positioner fabricates a 300x400 geometry instead of using the crate's `use_anchor_positioning`, Popup is a bare `<div class="menu-popup">` without the popup store's open/transition state; they also export invented hooks (`use_menu_portal_keep_mounted`, `use_menu_portal_container`, `create_portal_container`, `append_portal_container_to_body`, `remove_portal_container`, `use_menu_position`, `use_menu_size`, `use_menu_collision_avoidance`, `use_menu_keep_mounted`) that are absent upstream — MEASURED unused outside the menu module, so removing them changes no caller; (2) `Menu.Item`'s own remaining half is the `useButton` layer (`useMenuItem.ts:29-34`) and its merged `getItemProps` bag (`:47-65`), which need the element-rendering path wired for this unit — deferred here and recorded in the module docs, not dropped; (3) the 13 files `menu/mod.rs` never declares (arrow, backdrop, checkbox-item(+indicator), group, group-label, link-item, radio-group, radio-item(+indicator), submenu-trigger, viewport, plus `simple.rs`/`constants.rs`) are still fabrications AND still uncompiled, so this item's clause about them (ported, or removed with the decision recorded) is untouched; (4) the 19 missing part aliases belong to the same work, and `library: namespaced part surface (menus batch)` (L2478) is NOT sufficient alone for them — its own sibling note says exposing aliases over placeholder bodies is "ergonomics without behaviour". DO NOT READ THIS NOTE AS A CLOSE: no `status:` change, no `commit:` field, nothing checked off. ENVIRONMENT AT THIS ITERATION, qualified rather than assumed: `ralph/generated/env-health.json` reports verdict `ENVIRONMENT BROKEN` at 03:02Z on the `memory` check (3680MB/4096MB, `oom_kill total 55`) whose action line says "do not start a browser or a build". Read against the kernel's own composition at the same moment — `memory.stat` anon 667MB, file 3,169MB (RECLAIMABLE page cache), kernel 396MB — the reading is dominated by cache the kernel evicts rather than by pressure; the eight prior commits carrying this file read 36-88% on the same basis while running full `cargo test --workspace` regressions, and this iteration's `cargo check` + `cargo test --lib` produced no new `oom_kill` event (still 55). So memory is treated as DEGRADED-capable here WITH that measurement stated, never as OK, and the browser half is untouched: every rendered axis stays CI's (`browser: DEGRADED` — `lib/browser-budget.mjs` refuses here), which is why this iteration's evidence is host-side only and why the DOM half of `Menu.Item` is scoped above rather than claimed.
      done-when: no fabricated body remains in the unit — measured as `grep -rn "In a real implementation" crates/leptos-ui/src/menu/` being empty AND every `.rs` file under `crates/leptos-ui/src/menu/` being declared by `menu/mod.rs` (AMENDED 2026-09-17: the marker grep alone was insufficient — `submenu-root.rs` is a 156-line fabrication that carries no marker and was never compiled, see `ralph/logs/spec-discrepancies.md` finding 6), every part the unit compiles is a translated upstream counterpart cited to the same upstream files it cites, each part is exposed as `Menu::<Part>` so `node ralph/scripts/check-part-surface.mjs --components menu --strict` exits 0 (QUALIFIED 2026-09-17: `Menu.Viewport` cannot be reached before `infra: utils — usePopupViewport (the shared morphing-viewport engine) is unported` lands — the part is a delegation to that other unit's 396-line engine, which the crate does not have; see `ralph/logs/spec-discrepancies.md` finding 7), the scaffold-only files that `menu/mod.rs` never declares are either ported or removed with the decision recorded (the four with no upstream counterpart — `simple.rs`, `primitive.rs`, `constants.rs`, `types.rs` — were REMOVED 2026-09-17 with the decision recorded in this entry's progress-note), and `bash ralph/scripts/run-regression.sh "<this id>"` exits 0
      note: FOUND 2026-09-16 by an independent re-check of a done-claim (by the `library: switch` iteration, whose first three candidate picks were drawer, menubar and this). MEASURED, not inferred. (1) `library: menu` is `status: done` (TODO.md:839-849; done-marking commit 7688042d4, whose own subject is "finish orphaned store/impl diff ... leptos-ui lib 89/89 host tests green") while the unit it marks has no view layer: 15 files under `crates/leptos-ui/src/menu/` carry the fabricated-body marker `In a real implementation, we would`, with a hardcoded `class="menu-item"` / `class="menu-positioner"` shell — the exact "uniform DOM shell" that `crates/leptos-ui/src/menu/root.rs:5-8` records behavior.md as forbidding. The four that are actually COMPILED and publicly re-exported (`item.rs`, `popup.rs`, `portal.rs`, `positioner.rs` — `menu/mod.rs` declares `pub mod item/popup/portal/positioner` and `pub use item::*; pub use popup::*;`) ARE the port's `Menu.Item` / `Menu.Popup` / `Menu.Portal` / `Menu.Positioner` surface; the rest (`arrow.rs`, `backdrop.rs`, `checkbox-item.rs`, `checkbox-item-indicator.rs`, `group.rs`, `group-label.rs`, `link-item.rs`, `radio-group.rs`, `radio-item.rs`, `radio-item-indicator.rs`, `submenu-trigger.rs`, `viewport.rs`, `simple.rs`, `primitive.rs`, `constants.rs`) are not declared by `menu/mod.rs` at all, so they are not even compiled — `simple.rs` says so in its own header ("The simplest possible implementation that compiles and works") and `constants.rs` invents a `MENU_DEFAULT_STYLES` CSS string that `packages/react/src/menu/constants.ts` does not contain. (2) The consequence is measurable on gates that already exist: `node ralph/scripts/check-part-surface.mjs --components menu` reports `menu: 0/20`, and `node ralph/scripts/check-component-strict.mjs --component menu` prints `parts: FAIL ... 0/20` plus `namespaced path: FAIL — menu exports no capitalised alias in src/lib.rs`. (3) It is a PRECONDITION, not a nicety: the unit's only `menuopenchange` producer is `packages/react/src/menu/positioner/MenuPositioner.tsx:230` (an upstream grep returns that emitter, its two listeners, and `Menubar.tsx:121`), so `library: menubar` — the smallest remaining component — cannot be ported honestly on top of this, and `docs-content: components/menu` (TODO.md:1641-1650, `blocked-by: [library: menu, docs-app: routing + layout shell]`) cannot render real demos. WHAT THIS ITEM DOES NOT CLAIM: it fixes nothing by itself, and it does NOT flip `library: menu`'s status — the scope that done-marking intended is the owner's call, and this entry exists so that call can be made with numbers rather than memory. The sibling `library: namespaced part surface (menus batch)` (TODO.md:2469) is NOT sufficient alone: exposing `Menu::Positioner` over a fabricated body is ergonomics without behaviour, which is the same defect in the other direction. Counter-evidence checked and rejected: the store/spine half IS real — `menu/store.rs` (567 lines), `root.rs` (159), `trigger.rs` (116), `types.rs`, `utils.rs` and `submenu-root.rs` carry genuine citations, and `menu_tests.rs` (325 lines, 11 tests) exercises that half, so the fabrication is the VIEW LAYER, not the unit. Written up in `ralph/logs/spec-discrepancies.md`.
      progress-note: PARTIAL, this iteration — the OTHER THREE compiled facades are now real ports
        (`Menu.Portal`, `Menu.Positioner`, `Menu.Popup`); the item is NOT closed and every clause it still
        fails is named below with its measurement. WHAT LANDED (crate `base-ui-leptos` only; no other crate,
        no spec file edited): (a) `crates/leptos-ui/src/menu/portal.rs` — the render gate
        `mounted || keepMounted` (`MenuPortal.tsx:22-27`), the `MenuPortalContext` provider carrying
        `keepMounted` (`:35`, `MenuPortalContext.ts:4`) whose required read throws upstream's own message
        (`:8-10`), `portalOwnerRole` = `group` only under menu/menubar parents (`:32`), and the children
        mounted INTO the shared machinery's `data-base-ui-portal` host (`provide_floating_portal`, the
        `FloatingPortal.tsx` port) with the leptos-`Portal` per-run `Owner::on_cleanup` disposal — the old
        wrapper `<div class="menu-portal">` and its five invented helpers are gone; (b)
        `crates/leptos-ui/src/menu/positioner.rs` — the Portal requirement (`MenuPositioner.tsx:60`), the whole
        per-parent geometry resolution (`:83-107`: submenu `inline-end`/`start` + the POPUP preset, the menubar
        orientation arm, the context-menu `start` + `2`/`-5` offset pair), the real `use_anchor_positioning`
        call (`:111-136`), the positioner element registration and the `MenuPositionerContext` provision
        (`:301`), plus the element contract of the shared `usePositioner` wrapper (role=presentation,
        `hidden: !mounted`, the engine's styles, `pointerEvents: none` while closed, the disabled-mount
        transition styles) and the `data-*` set the ported `getStateAttributesProps` + `popupStateMapping`
        produce; the fabricated `(300,400)` geometry, the hand-rolled viewport clamp and the four invented
        helpers are gone; (c) `crates/leptos-ui/src/menu/popup.rs` — the store reads (`:36-53`), the state
        record + `popupTransitionStateMapping` attributes (`:84-95`), `role`/`id`/`aria-labelledby` from the
        store (`MenuRoot.tsx:576-579`), `data-rootownerid` (`:110`), the popup element registration into both
        slots (`store.context.popupRef` + the `popupElement` setter) and `useOpenChangeComplete`'s open half
        (`:55-64`); the fabricated `<div class="menu-popup">` shell and the local open signal are gone. THE SAME
        SHAPE FOR ALL THREE: each part's resolvable contract is a pure function covered by host tests, because
        this box refuses a browser (`ralph/generated/env-health.json` → `browser: DEGRADED`) — the DOM
        consequences of these values are CI's, and this note claims no rendered axis. MEASURED AT THIS TREE:
        `cargo check -p base-ui-leptos` EXIT 0; `cargo test -p base-ui-leptos --lib menu_view` 23 passed / 0
        failed / 0 ignored; `node ralph/scripts/check-part-surface.mjs --components menu` 1/20 -> 4/20;
        `node ralph/scripts/check-component-strict.mjs --component menu` namespaced path OK, sections OK,
        hygiene OK (17 tests, none ignored), parts FAIL 4/20; `git grep -l "In a real implementation" HEAD`
        over `crates/leptos-ui/src/menu/` = 14 files -> 12 now (`portal.rs` and `positioner.rs` are the two that
        carried it; `popup.rs`'s facade used a hardcoded class instead — all three are clean); a `grep -rn` for
        the nine removed fabricated helpers across `crates/` returns nothing, so no caller was left behind; the
        full gate `bash ralph/scripts/run-regression.sh "<this id>"` printed REGRESSION OK (citation check
        36 + 322 citations, `cargo test --workspace` 0 failures, TODO.md schema, part surface, component
        strict). WHAT REMAINS, in the order the next iteration should take it: (1) the 12 marker files
        (`arrow.rs`, `backdrop.rs`, `checkbox-item.rs`, `checkbox-item-indicator.rs`, `group.rs`,
        `group-label.rs`, `link-item.rs`, `radio-group.rs`, `radio-item.rs`, `radio-item-indicator.rs`,
        `submenu-trigger.rs`, `viewport.rs`) plus the scaffold-only `simple.rs`/`constants.rs`/`primitive.rs`
        that `menu/mod.rs` never declares; (2) the item part's remaining half (`useButton`/`getItemProps`,
        deferred in `item.rs` with its reason); (3) the two listener checkpoints this iteration measured as
        unreachable in-tree — the positioner's `menuopenchange`/`itemhover` coordination and the popup's
        floating-tree `close` listener, both needing an emitter upstream also sources from `Menubar.tsx:121`;
        (4) `FloatingFocusManager` + `InternalBackdrop` (`InternalBackdrop` is ported NOWHERE in this tree —
        measured by grep) and the Root-publishes-`popupProps` change the popup's `aria-orientation`/hover
      ...
      progress-note: PARTIAL, this iteration — THE CHECKBOX/RADIO ITEM FAMILY IS NOW REAL PORTS and the item is NOT
        ...
        checkpoint.
      progress-note: PARTIAL, this iteration — the LINK ITEM half, the namespaced part names, and the
        scaffold-only decision; two of the three marker files are gone and part surface is 13/20 -> 17/20.
        WHAT LANDED (crate `base-ui-leptos` only; `specs/**` untouched apart from the audit log): (a)
        `menu/link-item.rs` — a fabricated facade that `mod.rs` could never declare (a hyphen is not a legal
        Rust identifier) and that therefore never type-checked — is replaced by `link_item.rs`, the port of
        `MenuLinkItem.tsx` (106 lines) + `MenuLinkItemDataAttributes.ts`: the `{ highlighted }`-only state
        (`:67,86-91`), `role="menuitem"` + the roving tab index (`useMenuItemCommonProps.ts:62-63`), the
        composite-list registration (`:36`), `useBaseUiId` (`:40`), the close request through the ONE mutation
        gate with reason `itemPress` (`:52-61`), and the `...elementProps` rest as `element_attributes` —
        the bag that actually carries an `<a>`'s `href`, because upstream has NO `href` prop. The facade's
        invented surface is gone with it: `href`/`target_blank`/two aria props, eight invented helpers
        (seven returning constants, one returning `MenuLinkItemProps::default()`), a `window.open` navigation
        upstream never performs (the anchor navigates natively), and its
        `closeOnClick` default of **`true`** where upstream's is `false` (`:29`) — now a named constant the
        tests assert. 7 host tests (`cargo test -p base-ui-leptos --lib link_item` 7/7). (b) the namespace
        surface the gate reads: `Menu::LinkItem` (new), `Menu::Separator` (new `menu/separator.rs`, delegating
        to `crate::separator::separator_element` — the otp_field precedent for the identical upstream
        re-export at `index.parts.ts:19`), and `Menu::Root` + `Menu::Trigger`: `root.rs`/`trigger.rs` already
        held those parts under the pre-rewrite names `MenuRootComponent`/`MenuTrigger`, which is exactly why
        the gate read them MISSING; the definitions now carry the part name and the old spellings stay as flat
        aliases, so `context_menu/mod.rs:36`'s `pub use crate::menu::{…, MenuTrigger}` keeps compiling.
        (c) the four scaffold-only files with NO upstream counterpart are REMOVED with the decision recorded:
        `simple.rs` ("the simplest possible implementation that compiles and works", leptos-0.6 idioms),
        `primitive.rs` (`packages/react/src/primitive/` does not exist), `constants.rs`
        (`packages/react/src/menu/constants.ts` does not exist; it invented `MENU_DEFAULT_STYLES`), `types.rs`
        (an invented second `pub mod menu` carrying a conflicting `MenuRootProps`). MEASURED AT THIS TREE:
        `cargo test -p base-ui-leptos --lib` 520 passed / 0 failed / 0 ignored (7 of them this part's:
        `cargo test -p base-ui-leptos --lib link_item` 7/7); `check-part-surface
        --components menu` 17/20 (missing `SubmenuRoot`, `SubmenuTrigger`, `Viewport`); `check-component-strict
        --component menu` — parts FAIL 17/20 (same three), sections OK, namespaced path OK, hygiene OK (56
        tests, none ignored); the full gate is recorded in the commit message. TWO FINDINGS THE LEDGER DID NOT
        CARRY, both appended to `ralph/logs/spec-discrepancies.md` and both the reason two `done-when` clauses
        above were amended rather than quietly passed: (1) the "no fabricated body remains" clause was
        measured by ONE marker string, and `submenu-root.rs` (156 lines of leptos-0.6 code, `#[prop]` on plain
        struct fields, imports of a `MenuStoreContext` and a `MenuParent::Submenu{…}` that do not exist) is a
        fabrication that carries NO marker and was never compiled — the clause now measures the undeclared-file
        set; (2) `Menu.Viewport` is a delegation to the shared `usePopupViewport`
        (`MenuViewport.tsx:28-32` -> `packages/react/src/utils/usePopupViewport.tsx`, 396 lines), which is
        UNPORTED and homed in `leptos-ui-internals`, so the last part cannot be reached from this item's crate
        at all; it is scoped as its own item and the clause is qualified. ALSO APPENDED: `tooling:
        check-part-surface counts DEFINITIONS only`, since three DONE units (menu until this iteration,
        context-menu, and any unit that exposes a part by alias) read as missing their parts for that reason
        alone. WHAT REMAINS, in order: (1) `submenu-trigger.rs` + `submenu-root.rs` together — the trigger
        reads the PARENT menu's store through the SubmenuRoot bridge, so they are one port; (2) `viewport.rs`
        once `usePopupViewport` lands; (3) the item parts' `useButton`/`getItemProps` half (deferred in
        `item.rs`, `checkbox_item.rs`, `radio_item.rs` with their reasons); (4) the listener checkpoints (the
...
- [x] infra: utils — usePopupViewport (the shared morphing-viewport engine) is unported
      crate: base-ui-leptos-internals
      specs: specs/library/utils/behavior.md, specs/library/utils/implementation.md, specs/library/menu/parts/arrow-backdrop-portal-viewport.md
      blocked-by: []
      status: done
      priority: high   # four parts across four units are written off as "the deferred pass" on it
      step0-note: CHOSEN OVER the mechanical suggestion (`library: drawer`; `node ralph/scripts/pick-next-todo.mjs` re-run this iteration prints it). WHY NOT DRAWER, re-MEASURED this iteration rather than quoted: `find packages/react/src/drawer -name '*.ts*' | xargs wc -l` = 18,290 lines total, 4,763 of them non-test source, and the entry itself carries `needs-batched-mining: true`, so one bounded iteration cannot close it (repeated prior entries record the same measurement). WHY NOT A BLOCKED ITEM FIRST: re-verified rather than trusted — exactly one `status: blocked` line exists (`docs-content: components/accordion (prose + snippet completion)`, L2512) and its third dep `docs-chrome: API reference code blocks (prop Type cells + Additional Types bodies)` (L3391) is still `not-started`, so that block HOLDS and no broken-thing-first candidate outranks this pick. WHY THIS ITEM: it is the declared unblock path of the in-flight `library: menu — the view layer (Positioner/Portal/Popup/Item) is a fabricated stub, not a port` item, whose LAST missing part is blocked on this one. MEASURED at this tree, not inherited: `node ralph/scripts/check-part-surface.mjs --components menu` = `PARTIAL menu: 19/20 — missing menu::Viewport`, and `Menu.Viewport` is absent from `crates/leptos-ui/src/menu/mod.rs` (only a stale module-doc mention survives there). This item is `blocked-by: []` with both deps satisfied, and it unblocks four Viewport parts across four consuming units. No `blocked-by` was narrowed, blanked or rewritten.
      done-when: `usePopupViewport` (`packages/react/src/utils/usePopupViewport.tsx`, 396 lines) is ported into this crate — the content-swap state machine (`data-current` vs the exiting `data-previous` container with its `inert` flag), the `data-activation-direction` geometry over the previous/next trigger rects with the ~5px tolerance, the frozen `--popup-width`/`--popup-height` on the exiting container, `data-transitioning`, `popupViewportStateMapping` — with host tests over the state walk and the direction math (AMENDED 2026-09-17: the DELEGATION clause below was SPLIT OUT into its own item — `library: the four Viewport parts delegate to the shared engine` — because those four parts live in another crate and three of the four do not exist; see the progress-note for the measurement), and each consuming `Viewport` part delegates to it instead of deferring: `Menu.Viewport` (`MenuViewport.tsx:28-32`), `Popover.Viewport`, `PreviewCard.Viewport`, `Tooltip.Viewport`; `bash ralph/scripts/run-regression.sh "<this id>"` exits 0
      progress-note: CLOSED THIS ITERATION for the ENGINE half, which is the half this crate owns; the delegation half is split out (above) and NOT claimed. WHAT LANDED (crate `base-ui-leptos-internals` only; no other crate, no spec file edited): `crates/leptos-ui-internals/src/use_popup_viewport.rs` — (a) `popup_viewport_state_mapping` (`:21-30`) in the crate's `StateAttributesMapping` shape, plus `popup_viewport_state_record`/`popup_viewport_attributes`, so the `transitioning` key default-handles into the bare `data-transitioning` marker the morphing CSS selects on and the mapped key emits `data-activation-direction` with the direction as its value; (b) the pure geometry — `get_value_with_tolerance` (`:325-340`), `get_activation_direction` (`:307-313`) with the 5px tolerance and the `"<horizontal> <vertical>"` composition, `calculate_relative_position` (`:345-362`) over an explicit `ElementRect` so the center-to-center math needs no layout engine, and its DOM arm reading `getBoundingClientRect`; (c) the remount-key walk `PopupContentKeyState` (`:367-396`) — the trigger-id bump plus the second bump for a payload that arrives a render late; (d) `PopupViewportContainers`, the observable contract of the render output (`:226-265`): `current_attributes` (data-current, plus data-starting-style while armed `:259`), `previous_attributes` (data-previous + the `inert` attribute `:239`, plus data-ending-style exactly when the starting style is NOT armed `:253`) and `previous_style_declarations` (the frozen `--popup-width`/`--popup-height` as `"<number>px"` `:243-248`, then `position: absolute`); (e) the engine hook `use_popup_viewport` over the five store reads the engine itself consumes, the `open ? activeTrigger : null` previous-value channel, the five-conjunct trigger-change walk (`:156-176`), the clone capture (`:204-224`), the imperative population of the previous container (`:268-275`), the reset on close (`:150-154`), the `AbortController`+frame cleanup re-arm (`:137-146`, `:184-202`), the measure pair (`:119-135`) and the `useAnimationsFinished(currentContainerRef, true)` watcher (`:101`). SUPPORTING CONSTANTS THE UNIT ALREADY CARRIED are now consumed rather than orphaned: `common_viewport_data_attributes::ACTIVATION_DIRECTION` and `common_popup_css_vars::{POPUP_WIDTH,POPUP_HEIGHT}` — both of which name `usePopupViewport.tsx` in their own module docs as their consumer. VERIFIED BY EXECUTION at this tree: `cargo test -p base-ui-leptos-internals --lib use_popup_viewport` = 15 passed / 0 failed / 0 ignored; `cargo check -p base-ui-leptos-internals --lib --target wasm32-unknown-unknown` clean (so the DOM target still builds — the class of wasm-only breakage a host-only green hides); `cargo check -p base-ui-leptos-internals --lib` clean, 0 errors. CITATION RE-CHECK (this is a citation check too): every upstream line range cited in the module docs was re-read before being written, and two claims were corrected from the reading rather than left as paraphrase — `side`'s occurrence set (`:53`, `:76`, `:284`) and the `--popup-width` conditional spread (`:243-248`). WHAT THIS CLOSE DOES NOT CLAIM: (1) the DOM half is UNVERIFIED here rather than passed — this box refuses browsers by design (`ralph/generated/env-health.json`: `browser: DEGRADED — browser gates REFUSED here by lib/browser-budget.mjs (4 GB cgroup)`) and no CI job runs this crate's wasm suite (its own ledger item), so the real mount, the clone move, the animation watcher and the layout writes rest on the wasm-target compile check plus the host suite over the state walk and the direction math, which is what the done-when names; (2) `usePopupAutoResize` (`:277-286`) is NOT ported — it is a separate util this engine composes, it was unowned in the ledger, and it is now scoped as its own item (below) rather than re-derived here; (3) the two store reads that exist only to feed that composition (`popupElement`/`positionerElement`, `:85-86` and `:278-279`) are deliberately not taken, and the reason is recorded in the module docs; (4) `store.set('adaptiveOrigin', …)` (`:112-117`) is re-homed to the positioner's own `adaptive_origin` option, which is where the port's `use_anchor_positioning` reads it (`use_anchor_positioning.rs:502,1057-1058`) — this crate's `PopupStoreState` has no such field; (5) no `blocked-by` was narrowed, blanked or rewritten, and no recorded baseline was touched with `--update`. LEDGER CORRECTION THIS ITEM OWES: this entry's own `note` asserts that `specs/library/utils/*` documents no such util and that ownership was therefore "either a spec gap in that unit or a false-done half of it". MEASURED this iteration, the first reading is FALSE: `grep -rln usePopupViewport specs/` returns `specs/library/utils/implementation.md` AND its citation sidecar, and that spec documents the engine in detail — the section "Viewport / auto-resize (`usePopupViewport.tsx` + `usePopupAutoResize.ts`)" at `implementation.md:173`, the seven store reads at `:196-197`, the content-key walk at `:198-200`, the trigger-change offset at `:200-204`, the container choreography at `:204-206`, the clone capture and its move at `:206-210`, the cleanup re-arm at `:210-213` and the `adaptiveOrigin` registration at `:213-215`, plus `:307-315` and `:582-587`. So the ownership question RESOLVES as the second reading: this was a false-done half of `infra: utils` (marked done with its own "the grab-bag is complete" note while its own implementation spec documents an unported util), the same failure shape as the `useSwipeDismiss` false-done that entry records. Appended to `ralph/logs/spec-discrepancies.md` rather than left in this note.
      commit: 2bb0f29247d (the engine port; recorded by the follow-up commit that carries this line, because a commit cannot contain its own sha)
      note: FOUND 2026-09-17 while working `library: menu — the view layer (Positioner/Portal/Popup/Item) is a fabricated stub, not a port`, whose LAST missing part is blocked on this. MEASURED, not inferred: upstream's only `usePopupViewport` consumers are the four viewport parts plus the util itself (`grep -rln usePopupViewport packages/react/src/`); the crate has no port of it anywhere (`grep -rln "activation_direction\|data-current\|data-previous" crates/leptos-ui-internals/src/` returns nothing); and `crates/leptos-ui/src/preview_card/parts.rs:583` already recorded the deferral in as many words ("the morphing container is the deferred pass (usePopupViewport)"). OWNERSHIP IS THE UNRESOLVED PART, recorded instead of assumed: the file lives in `packages/react/src/utils`, i.e. the Phase A unit `infra: utils`, which is marked DONE with its own note ("the temporal-adapter checkpoint landed and the grab-bag is complete") — yet `specs/library/utils/*` documents no such util (`grep -rln usePopupViewport specs/` returns the menu/popover/preview-card/tooltip/direction-provider specs, NOT the utils one), so this is either a spec gap in that unit or a false-done half of it; both readings are written up in `ralph/logs/spec-discrepancies.md` (finding 7) for the audit loop rather than decided here. NOT done from the menu item: it is another crate, and a menu-local re-derivation would be the "own replacement tool" this loop forbids — which is why `Menu.Viewport`'s clause is qualified rather than satisfied.

- [ ] tooling: check-part-surface counts DEFINITIONS only — a part exposed by alias reads as missing
      crate: ralph/scripts (tooling — no crate)
      specs: ralph/scripts/check-part-surface.mjs, specs/docs-content/CONTRACT.md
      blocked-by: []
      status: not-started
      done-when: `crateSurface()` recognises the other legal Rust spellings of an exposed part — at minimum `pub use <path>::<Name> as <Part>;` and `pub use <path>::<Part>;` — so a unit that exposes a real component under the documented part name is counted, verified by a unit whose parts are ONLY aliases reading at its true count and by the parts that are genuinely missing NOT disappearing from the `--strict` failure list; the change is recorded as a reviewed gate edit (what it now measures, and whether the bar moved), never a silent widening
      note: FOUND 2026-09-17 while working `library: menu — the view layer…`. MEASURED: the `crateSurface()` regex (`check-part-surface.mjs:112`) is `pub\s+(?:fn|struct|enum|type|use)\s+([A-Za-z_][A-Za-z0-9_]*)`, so after `use` it captures the module-path segment and never the alias — `pub use root::MenuRootComponent as Root;` contributes `root`, not `Root`. Consequences measured at this tree: `menu` read `Root` and `Trigger` as MISSING although `menu/root.rs` and `menu/trigger.rs` export those exact parts (under the pre-rewrite names), and `context-menu` — a unit marked DONE with real root and trigger parts — still reads `0/9`. The port half was fixed this iteration (the definitions now carry the part names, the pre-rewrite spellings stay as aliases — which is the ergonomics the docs contract wants anyway); this item is the INSTRUMENT half, and the reason it is an item rather than an edit here is the loop's own rule that a gate change needs review as a tooling change. Related, NOT this item: `library: namespaced part surface (menus batch)` and `(inputs batch)` gate on this script, so a unit whose parts are only aliases would fail them for a measurement reason, not a surface one.

- [ ] library: the four Viewport parts delegate to the shared engine
      crate: base-ui-leptos
      specs: specs/library/menu/parts/arrow-backdrop-portal-viewport.md, specs/library/utils/implementation.md
      blocked-by: [infra: utils — usePopupViewport (the shared morphing-viewport engine) is unported]
      status: not-started
      priority: high
      done-when: each consuming `Viewport` part whose UNIT EXISTS delegates to `leptos_ui_internals::use_popup_viewport` instead of recording a deferral — `Popover.Viewport` and `PreviewCard.Viewport` (both units are `status: done`, and both stores are `PopupStore<()>`, so the engine's `P: PartialEq + Clone` bound is met) — so each renders the `data-current` container by default and the `data-previous`/`data-current` pair (with the previous container's `inert` and its frozen `--popup-width`/`--popup-height`) during a trigger-to-trigger transition, with each part's `data-activation-direction` drawn from the engine's state; verified by `bash ralph/scripts/run-regression.sh "<this id>"` exiting 0. AMENDED 2026-09-17 — SCOPE NARROWED, with the measurement inline rather than assumed: `Menu.Viewport` and `Tooltip.Viewport` are NOT reachable inside this item's crate. Tooltip has no unit at all (`library: tooltip` is `status: not-started`; `grep -rln "Tooltip" crates/leptos-ui/src` returns nothing), and the menu store CANNOT be passed to the engine (`MenuStore = PopupStore<MenuExtraState>` at `crates/leptos-ui/src/menu/store.rs:194` and the engine requires `P: PartialEq` at `crates/leptos-ui-internals/src/use_popup_viewport.rs:518-524`, while `MenuExtraState` derives only `Clone, Debug` and cannot derive `PartialEq` because its `parent: MenuParent` arm holds a store). Both halves are now their own ledger items instead of clauses this item cannot turn green; the menu one is `library: menu store — the payload slot carries the menu's extra state …` at the end of this file.
      note: SPLIT OUT 2026-09-17 from `infra: utils — usePopupViewport (the shared morphing-viewport engine) is unported`, whose done-when carried this clause while its `crate:` field is `base-ui-leptos-internals`. The split is a measurement, not a preference: the four parts live in THIS crate (`base-ui-leptos`) and three of them do not exist at all, so no amount of correct work inside `base-ui-leptos-internals` turns the clause green, and the loop's own rule is `Do not touch any other crate` — the recipe the stage prompt gives for exactly this shape is to split the scope into its own item rather than re-pick or pad. MEASURED at the splitting tree: `crates/leptos-ui/src/menu/mod.rs` declares no Viewport module (`grep -n Viewport crates/leptos-ui/src/menu/mod.rs` matches only a stale module-doc line 18) and `node ralph/scripts/check-part-surface.mjs --components menu` reads `PARTIAL menu: 19/20 — missing menu::Viewport`; `crates/leptos-ui/src/popover/` and `crates/leptos-ui/src/preview_card/` hold no viewport module and `crates/leptos-ui/src/preview_card/parts.rs:583` still records the deferral in as many words ("the morphing container is the deferred pass (usePopupViewport)"); `crates/leptos-ui/src/navigation_menu/viewport.rs` exists but is a DIFFERENT unit's part (navigation-menu, `status: not-started`) and sets its own `activation_direction: None` placeholder at `root.rs:67`, so it is not one of the four. The engine half landed in the split source item with 15 host tests over the state walk and the direction math; the mined contract this item renders against is `specs/library/menu/parts/arrow-backdrop-portal-viewport.md:15-21,41-43,52-54` (the `data-current` container, the `inert` previous container with its `px`-formatted size vars, the removal after the animation, the space-separated direction tokens with the ~5px tolerance, and the `data-transitioning` ancestor the CSS selects on). NOT claimed: the DOM/rendered axes — this box refuses browsers and no CI job runs the crate's wasm suite, so they are CI scorecard work.  || AMENDED 2026-09-17 (the iteration that closed `docs-content: components/toggle-group`): the menu half of this item was TRIED and MEASURED blocked rather than trusted. A `Menu.Viewport` implementing this done-when was written (`crates/leptos-ui/src/menu/viewport.rs`, the port of `MenuViewport.tsx:17-46` over the landed engine: the two required context reads, the `{activationDirection, transitioning, instant}` state record through `popupViewportStateMapping`, and the `data-current`/`data-previous` container pair committed from the engine's own `previous_attributes()`/`previous_style_declarations()`) and it does not compile against this tree for one reason only: `use_popup_viewport<P>` requires `P: PartialEq + Clone` (`use_popup_viewport.rs:518-524`) while the menu store's payload type is `MenuExtraState`, which has no `PartialEq` and cannot get a faithful one (upstream's payload slot carries the ACTIVE TRIGGER's payload — `MenuRoot.tsx:166,639` reads it for the render-prop children and `MenuTrigger.tsx:93-106` writes it — while this port put the menu's extra state in that slot, `menu/store.rs:14,68,216`). Deriving or hand-writing `PartialEq` there would make the engine's content-key payload comparison (`usePopupViewport.tsx:367-396`) mean "any menu state changed", which is not upstream's semantics, so the file was removed rather than shipped with a semantic hack; the finding is recorded as its own item and in `ralph/logs/spec-discrepancies.md`. NOTHING of that attempt is in the tree (no `menu/viewport.rs`, no `mod` line, no re-export) — verified by `grep -n viewport crates/leptos-ui/src/menu/mod.rs` matching only the stale module-doc line 18, and `library: menu` still reads `19/20 — missing menu::Viewport`. The tooltip half is likewise unblocked by nothing: `Tooltip.Viewport` needs `library: tooltip`, which is `not-started`.

- [ ] infra: utils — usePopupAutoResize (the measurement cycle) is unported
      crate: base-ui-leptos-internals
      specs: specs/library/utils/implementation.md, specs/library/utils/behavior.md
      blocked-by: []
      status: not-started
      priority: medium
      done-when: `usePopupAutoResize` (`packages/react/src/utils/usePopupAutoResize.ts`, the measurement cycle the spec documents at `specs/library/utils/implementation.md:175-193`) is ported into this crate — the per-open initial-open path (override `position: static`/`transform: none`/the positioner's available-size vars, measure intrinsics with `getCssDimensions`, commit as the positioner's CSS-var size, restore; `usePopupAutoResize.ts:89-106`), the content-change path (freeze the popup at the previous dimensions via the popup CSS vars, then transition to the new size on the next frame and reset the vars to `auto` once animations finish; `:108-139`), and `getPopupAnchoringStyles` (`:188-203`) — with host tests over the size/anchoring math and `bash ralph/scripts/run-regression.sh "<this id>"` exiting 0
      note: FOUND 2026-09-17 while closing `infra: utils — usePopupViewport (the shared morphing-viewport engine) is unported`. MEASURED, not inferred: that engine composes `usePopupAutoResize` (`packages/react/src/utils/usePopupViewport.tsx:277-286`) and this crate has NO port of it (`grep -rln "popup_auto_resize\|PopupAutoResize" crates/` returns only `get_css_dimensions.rs`, which is the measuring primitive, and `lib.rs`); the ledger had no item naming it (`grep -n "usePopupAutoResize\|AutoResize" TODO.md` was empty before this entry), so the gap was unowned. `specs/library/utils/implementation.md:173` documents BOTH utils under one heading ("Viewport / auto-resize (`usePopupViewport.tsx` + `usePopupAutoResize.ts`)") — the same false-done half of `infra: utils` the split source item records, and this entry is the second half. The engine port exposes the seam this util drives rather than re-deriving it: `handle_measure_layout`, `handle_measure_layout_complete` and `previous_content_dimensions` on `UsePopupViewportReturnValue`, and deliberately does NOT read the two store fields (`popupElement`, `positionerElement`) that exist only to feed this composition.

- [ ] library: menu store — the payload slot carries the menu's extra state, so the shared viewport engine cannot be composed
      crate: base-ui-leptos
      specs: specs/library/menu/implementation.md, specs/library/menu/parts/arrow-backdrop-portal-viewport.md
      blocked-by: []
      status: not-started
      priority: high   # it is the measured blocker of the menu half of `library: the four Viewport parts…`, and of the menu unit's LAST part.
      done-when: the menu store carries the ACTIVE TRIGGER's payload in the slot upstream keeps it in — an `Option<...>` value written from `Menu.Trigger`'s `payload` prop through the trigger data-forwarding path (`packages/react/src/menu/trigger/MenuTrigger.tsx:93-106`, upstream's `useTriggerDataForwarding(..., { payload, ... })`), readable by `Menu.Root`'s render-prop children (`packages/react/src/menu/root/MenuRoot.tsx:166,639`), and comparable on exactly that axis so that `leptos_ui_internals::use_popup_viewport` accepts the store (`P: PartialEq + Clone`, `crates/leptos-ui-internals/src/use_popup_viewport.rs:518-524`) and its content-key walk (`packages/react/src/utils/usePopupViewport.tsx:367-396`) compares what upstream compares; pinned by host tests over the write path and the comparison, and by `bash ralph/scripts/run-regression.sh "<this id>"` exiting 0.
      note: FOUND 2026-09-17 while working `library: the four Viewport parts delegate to the shared engine` (whose menu clause this blocks). MEASURED, not inferred: (1) the engine's bound is `P: PartialEq + Clone` (`crates/leptos-ui-internals/src/use_popup_viewport.rs:518-524`) and the menu store is `PopupStore<MenuExtraState>` (`crates/leptos-ui/src/menu/store.rs:194`); (2) `MenuExtraState` (`menu/store.rs:69-107`) derives `Clone, Debug` only, and a derive is impossible because its `parent: MenuParent` field (`menu/store.rs:141-152`) has a `Menu { store: MenuStore }` arm — a compile-checked failure, not a guess; (3) the conflation is the PORT's: upstream's `payload` slot holds the trigger's payload, read by the root's render-prop children (`MenuRoot.tsx:166` `store.useState('payload') as Payload | undefined`, rendered at `:639`) and written from `Menu.Trigger`'s `payload` prop (`MenuTrigger.tsx:62,98,315-317`) via `useTriggerDataForwarding` (`:93-106`) — while this port seeds the slot with the menu's own extra state (`menu/store.rs:216` `state.payload = Some(MenuExtraState::default())`) and every menu part reads its extra fields back through `selectors::payload(state)` (backdrop `:152`, popup `:238,243,245`, portal `:161`, positioner `:464,477,480`). Consequence, stated as the two things it actually blocks: the shared viewport engine cannot be composed with the menu store at all, and the root's render-prop payload dispatch (spec'd at `specs/library/menu/parts/arrow-backdrop-portal-viewport.md:7-8,12`, `MenuViewport.test.tsx:50-93`) has no channel to read. A degenerate or all-fields `PartialEq` was considered and REJECTED: it would make the engine's payload comparison (`usePopupViewport.tsx:367-396`) fire on every menu-state change (hover/activeIndex/openChangeReason all ride that struct), which is not upstream's semantics — the honest fix is the channel above, which is this item. Written up in `ralph/logs/spec-discrepancies.md` as well.
      note: the honest limit of the measurement: no test at this tree can observe the difference (the DOM axis is CI's — `ralph/generated/env-health.json` reports `browser: DEGRADED`), so this item's evidence is the type-level failure above plus the host tests its own done-when names; the fact that the compiler refuses the composition is what makes the gap undeniable rather than stylistic.

- [ ] tooling: check-component-strict counts `#[test]` only — a wasm-only test module reads as "0 test(s)"
      crate: ralph/scripts (tooling — no crate)
      specs: ralph/scripts/check-component-strict.mjs
      status: not-started
      priority: medium
      done-when: the `hygiene` axis counts a DOM-target test case as a test — `#[wasm_bindgen_test]` (and its argument form `#[wasm_bindgen_test(...)]`) contributes to the test count and to the `#[ignore]` scan exactly as `#[test]` does, so `node ralph/scripts/check-component-strict.mjs --component otp-field` reads that unit's real case count (7 in `crates/leptos-ui/src/otp_field_view_tests.rs`) instead of `hygiene: FAIL — 0 test(s) for 9 spec section(s)`; verified by a before/after snapshot of EVERY spec-bearing unit proving the only verdict moves are `FAIL -> OK` on `hygiene` for units whose coverage is genuinely wasm-side and that no unit moves `OK -> FAIL`, plus `bash ralph/scripts/run-regression.sh "<this id>"` exiting 0; a measurement change, so it carries a review-note like every other gate edit in this loop
      note: FOUND 2026-09-17 by `tooling: check-component-strict measures NOTHING for a hyphenated unit id (props/sections/hygiene read vacuous or false-FAIL)`, whose fix made this visible rather than caused it. MEASURED, not inferred: the count is `(testText.match(/#\[test\]/g) || []).length` and the disabled-test check is `/#\[ignore[^\]]*\]/`, so `crates/leptos-ui/src/otp_field_view_tests.rs` — 7 `#[wasm_bindgen_test]` cases, 0 `#[test]` — reports `hygiene: FAIL — 0 test(s) for 9 spec section(s)` in the same run where that same file satisfies `sections` (all 9) and `namespaced path` (14 uses of `<OTPField::…>`), while `menu_view_tests.rs` (23 `#[test]`) and `radio_group_tests.rs` (26 `#[test]`) read correctly. The ledger elsewhere treats the wasm suite as evidence ("wasm suite 5/5 in Chrome for Testing"), so the axis and the audit trail disagree about the same file. NOT folded into the parent item on purpose: that item's verified clause is that every unit's move is `vacuous -> measured`, and counting wasm cases additionally moves `hygiene` FAIL -> OK, which is a wider measurement change needing its own review. COMPANION GAP, deliberately NOT this item: counting those cases does not make them run — no runner executes the crate's wasm suites on this box or in CI (`tooling: the crate DOM/wasm suite runs nowhere but this box`), so this item makes the axis honest about what EXISTS, not about what PASSES. Full mechanism and blast radius in `ralph/logs/spec-discrepancies.md`, finding 11.
