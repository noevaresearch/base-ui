...
- [ ] docs-copy: Leptos-only mentions + the @noevaresearch/base-ui alias (no React leakage)
      ...
      gate-fix-note: ROOT FIX LANDED THIS ITERATION, tooling only (commit ae7a6a1b8: `ralph/scripts/mentions-rendered-evidence.mjs` new; `ralph/scripts/run-regression.sh` and `ralph/scripts/check-react-mentions.mjs` edited — no crate, no spec, no release path, no workflow touched). This is the item's own blocked-reason's unblock path (b), "teach this ONE clause the budget module's documented contract", done so that UNMEASURED still cannot become a pass. THREE CHANGES.
        ...
        (2) `ralph/scripts/mentions-rendered-evidence.mjs` (new) is what "read the recorded measurement" MEANS, or the deferral would be exactly the hole this clause was widened to close. It admits a record only when it is ALL FOUR of: committed at HEAD (a working-tree report cannot be tied to a tree), clean for the classes the caller claims (parsed per class from the report's defect bullets, so a `snippet-react` defect owned by the snippet-translation items is reported, never charged), site-wide (a `--route` scan writes the same file, so 1 route cannot stand in for 41), and current — no verdict-changing path changed since (`crates/docs-app/src`, `specs/docs-content`, `packages/leptos`, `lib/source-scope.mjs`). Its second source is the CI scorecard's per-route `react mentions` axis, the budget module's own "instead:" text. Pure text + git, no browser, so the loop can afford it every iteration, and it carries a self-test that runs on every invocation (13 report/scorecard fixtures + 2 parser controls, both directions) per `gate-selftest.mjs`'s doctrine — it caught its own author's wrong fixture (exit 3) before printing any verdict.
