...
- [ ] library: menu — the view layer (Positioner/Portal/Popup/Item) is a fabricated stub, not a port
      ...
      done-when: no fabricated body remains in the unit (`grep -rn "In a real implementation" crates/leptos-ui/src/menu/` is empty), every part the unit compiles is a translated upstream counterpart cited to the same upstream files it cites, each part is exposed as `Menu::<Part>` so `node ralph/scripts/check-part-surface.mjs --components menu --strict` exits 0, the scaffold-only files that `menu/mod.rs` never declares are either ported or removed with the decision recorded, and `bash ralph/scripts/run-regression.sh "<this id>"` exits 0
