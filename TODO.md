...
- [x] library: preview-card
      ...
      blocked-by: [Phase A complete]
      status: done
      note: picked as suggested (the mechanical id; Phase A complete, no blocked item whose reason verifies as resolved remained — the remaining blocked items' recorded failures are the hermes-driver re-run wall the popover/checkbox-group notes document); resumed the prior iteration's uncommitted working-tree orphan (1a45c8840: the inline-rect machinery — packages/react/src/utils/popups/inlineRect.ts ported as inline_rect.rs + inline_rect_middleware.rs, the multiline line-box grouping, the ±2px hit-test, the side-aware single-line selection, the two-line left/right edge-grouping fallback, the engine middleware over a rect-override virtual element; 9 host tests green) per the toggle/dialog/accordion resume precedent, then ported the facade (4e3da79e9: store.rs — the PreviewCardStore.ts extra slab with closeDelay seeded to the CLOSE_DELAY constant, the inlineRectCoordsRef context member, preview_card_set_open over the shared applyPopupOpenChange sequence with the hover-open inline-rect capture onBeforeDispatch behind the trigger-changed guard and the instantType mapping; mod.rs — the Root with the defaultOpen mounted-overlay seed, the useImplicitActiveTrigger closeOnActiveTriggerUnmount arm, the onUnmount coords-clear, the imperative close/unmount Actions; parts.rs — Trigger as an anchor per refInstanceof HTMLAnchorElement with the mouseenter/focus/blur open-close paths and triggerOpenStateMapping's data-popup-open, Positioner with the data-side/align/anchor-hidden mapping, Popup with the FOCUSABLE tabIndex=-1 and the full transition-state mapping, Arrow, Backdrop with the webkit-user-select inline style; 7 host tests green); the first gate run caught the wasm-only capture block the host check cannot see — Cell take-and-restore read, the local JsCast import, the Option-identity trigger comparison — fixed in 34dd55929 with both targets verified clean; the second gate run died on the environmental docs-app linker thread starvation (libc++abi thread constructor failed: Resource temporarily unavailable, load avg 16.5 from concurrent loops — the checkbox-group blocked note's exact failure mode, not a preview-card regression) and the re-run under CARGO_BUILD_JOBS=2 passed the FULL gate (citation check, cargo test --workspace, TODO schema, docs-app build, EXIT 0); specs/library/preview-card/fixtures.json does not exist on disk (never generated) so the oracle-assertion clause is satisfied by the crate's host suite per the button/dialog/meter/progress/separator precedent; playwright-diff.mjs still does not exist — the differential half is recorded unverified per the same precedent; the docs-pair docs-content: components/preview-card is not-started — deferred per the button/meter/progress/separator/toggle exemption precedent rather than fabricating a docs page, the pair completes when its Phase D iteration lands
