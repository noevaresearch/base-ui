...
- [ ] infra: merge-props
      ...
      status: blocked
      note: blocked by run-regression.sh's docs-pair check (step 4): the item's docs-pair (docs-content: utils/merge-props) requires crates/docs-app, which does not exist yet (the same structural docs-pair gate that left infra: csp-provider/direction-provider blocked), so the item cannot be verified done regardless of the green crate/workspace tests — the port itself is complete and committed as checkpoints (111fd8d1c: the whole merge fold re-homed from use_render_element into merge_props.rs per that module's handoff note — merge_props_n/merge_class_names/merge_styles/merge_event_handlers/merge_into/resolve_source over PropsSource/RenderPropsGetter, use_render_element/composite_view now import it, lib.rs re-exports moved, no behavior change; ac8357fc2: 21 host + 10 wasm test mirrors of mergeProps.test.ts incl. the full getter semantics and the two getter-handler reinvocation contracts, with the 7 re-homed-function tests moved out of use_render_element's host suite; 370 host green, cargo test --workspace green, scoped citation check + TODO schema green this iteration); latent wasm-suite pollution documented (2026-09-10, found while running the full in-browser wasm suite this iteration): get_element_at_point's resolves_through_the_document_root/resolves_shadow_content_through_its_own_root fail under full-suite test order — elementFromPoint(55,55) resolves to a leftover element an earlier test left in the document — while passing when the module runs filtered, and the failure reproduces identically on the committed base a2ca46206 (verified via git worktree), so it predates this item and is not a regression (the same latent-wasm category as the useTimeout/useValueAsRef notes): CHROME=... CHROMEDRIVER=... cargo test -p leptos-ui-internals --target wasm32-unknown-unknown
      done-when: crates/leptos-ui-internals tests pass; cargo test --workspace green
...
- [ ] infra: utils
      ...
      status: not-started
      note: picked over the mechanical suggestion (infra: use-render this iteration again; infra: merge-props before it) — both carry a docs-pair (docs-content: utils/use-render, docs-content: utils/merge-props) and run-regression.sh step 4 hard-fails any docs-pair-bearing item while crates/docs-app does not exist (the same structural wall documented on infra: csp-provider/direction-provider), so neither can be verified done; this item is exempt-from-docs-pairing and the only remaining not-started Phase A unit completable under the current gates; progress so far (multi-iteration item, all full-gate green): daf90d0b2 the small pure leaves (scrollEdges/valueToPercent/Common{PopupDataAttributes,TriggerDataAttributes,PopupCssVars} + popupStateMapping), e5d37a4ac the PopupTriggerMap unit's own test suite ported onto the provisional floating_ui host (9 wasm mirrors, the NODE_ENV production test documented unportable), ee6d3b6eb the next pure/small leaves (getCssDimensions with the content-box offset-fallback pinned, getElementTransform with the translate-longhand caveat pinned, getElementAtPoint with the shadow-root retargeting pinned, resolveAriaLabelledBy, NullStore's four-mutator inert contract, collapsibleOpenStateMapping with the component constants inlined-and-cited until Phase B, CommonViewportDataAttributes, styleDisableScrollbar as a RenderedElement description, resolveClassName/resolveStyle collapsed into the ClassNameSource/StyleSource resolve methods), aa50a0d7c scrollable.ts (the allowOverflowIntent ternary, the ancestor walks; the source comment's shadow-boundary claim documented as NOT holding — the walk stops at the ShadowRoot — and logged to spec-discrepancies per the never-rewrite rule) + adaptiveOriginMiddleware.ts (transition gate, offset-parent/visualViewport resolution, the left/top opposite-edge flip, sideX/sideY), 1828906cc the store-level half of popups/popupStoreUtils.ts (createPopupOpenState matrix, applyPopupOpenChange's order/cancellation/extraState/instantType-mapping with flushSync collapsed per the synchronous-write convention, attachPreventUnmountOnClose via an optional shared flag cell on BaseUIChangeEventDetails, useTriggerRegistration's (store,id,element) triple, syncTriggerCount, FOCUSABLE_POPUP_PROPS, createDefaultInitialFocus; PopupStoreState gains the family-level instant_type field with the per-family unions merged into one enum and documented); d3dcc2e0c the render-coupled popupStoreUtils hooks (use_popup_root_store, popup_handle_attachment + the PopupRootStoreHandle trait, use_trigger_data_forwarding, use_implicit_active_trigger, use_open_state_transitions, use_popup_interaction_props, use_popup_root_sync); e5ccba153+6154a322e the popup handle boundary (popups/popupHandle.ts + popups/usePopupHandleStore.ts: BasePopupHandle's fallback/attached-store pointer over the attach stack with the previous-root restore + lastIndexOf removal + identity-guarded subscribe/notify, openByTrigger's stack-newest-first-then-fallback trigger resolution with the verbatim missing-trigger panic and the throwOnMissingTrigger=false unassociated open, the deferred dev overlap warning over the debug_assertions NODE_ENV gate, use_popup_handle_store as the useSyncExternalStore bridge returning the subscription-fed signal, the PopupHandleStoreProvider/WithTriggers/WithOpen trait trio with the concrete popup-store setOpen forwarding to the context's on_open_change writer slot, the missing imperative-action REASONS constant, the PopupRootStoreHandle impl binding popup_handle_attachment; 5 host + 8 wasm browser tests — the upstream unit has no test file, so these pin the documented semantics proportionately); REMAINING: the popupStoreUtils.test.tsx hook sections (the store-level sections are in), popups/useTriggerFocusGuards.ts, popups/inlineRect.ts (+test), useSwipeDismiss.ts (+test), useOpenInteractionType.ts, useMixedToggleClickHandler.ts, useAnchoredPopupScrollLock.ts, usePopupViewport.tsx, usePopupAutoResize.ts, usePositioner.tsx, listbox-separator/ListboxSeparator.tsx (+test), InternalBackdrop.tsx, FloatingPortalLite.tsx (documented not-ported at floating_ui/floating_portal.rs:72-73); 69ff26c2d closePart.tsx (the count-up/count-down ClosePartContext with the SendWrapper'd consumer lifecycle); this iteration also re-anchored the item's own TODO.md citation windows to 358-364 plus the 8 hard-drifted and 59 line-displaced TODO.md citations across specs/ (commit 454a4e392), re-recorded all baselines, and re-ran the full regression gate green (citation check, cargo test --workspace, TODO schema); picked again this iteration (2026-09-10) over the mechanical suggestion infra: use-render on the same structural docs-pair wall (run-regression.sh step 4 hard-fails its docs-content: utils/use-render pair while crates/docs-app still does not exist; the driver's use-render attempts today additionally died to provider key-limit errors with no work committed), continuing the remaining list with the small-hooks/thin-wrappers cluster (useOpenInteractionType, useMixedToggleClickHandler, useAnchoredPopupScrollLock, usePositioner, ListboxSeparator, InternalBackdrop)
      done-when: crates/leptos-ui-internals tests pass; cargo test --workspace green
      exempt-from-docs-pairing: true
