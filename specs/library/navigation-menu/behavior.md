...
(`TODO.md:449`) has no `wraps-external:` field, so the behavior below is derived entirely from
...
select are — `TODO.md:449`), and the suite is ~5.6k lines across 15 files, so this single file
