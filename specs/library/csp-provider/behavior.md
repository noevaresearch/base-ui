...
Mined Stage 1 from test files only. Unit: `csp-provider` (infra unit; React source in `packages/react/src/csp-provider/`). The TODO.md entry (`TODO.md:295-303`) has no `wraps-external:` field, so no third-party delegation applies — behavior below is fully scoped to this unit's own test file.
