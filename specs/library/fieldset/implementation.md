...
External delegation: none. `TODO.md`'s `library: fieldset` entry has no `wraps-external:` field (`TODO.md:711-768`), consistent with the statement in behavior.md's front matter — there is no third-party npm package whose internals this spec would otherwise need to derive, and no Rust crate stands in for one.
