...
The `TODO.md` entry `infra: unstable-use-media-query` (`TODO.md:339-346`, target crate `leptos-ui-internals` at `TODO.md:340` — consolidated from the mined per-unit `leptos-unstable-use-media-query` name by the crate-workspace decision in `specs/architecture.md`) carries **no `wraps-external:` field** (compare an entry that does, `TODO.md:317`), so there is no third-party delegation to document.
