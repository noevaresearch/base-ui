...
The unit's `TODO.md` entry (`library: toggle-group`, `TODO.md:1097-1108`) has no `wraps-external:` field and no `needs-batched-mining` flag, so behavior is derived entirely from this single test file; no third-party delegation applies.
