...
`wraps-external:` — the unit's `TODO.md` entry has no `wraps-external:` field (`TODO.md:1097-1108`), and the source confirms it: nothing here delegates to an external npm package; all behavior is in-repo.
...
5. **`ToggleGroupDataAttributes.ts` is dead code from the component's perspective** — nothing in the repo imports it (only `ralph/generated/components.json:516` lists the file). The attribute strings it declares are verified only *indirectly*, through rendered-attribute assertions. A port that renames state keys would silently diverge from these constants.
