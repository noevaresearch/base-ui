...
6f. **The page is the unit of done.** Every axis below is a *component* of one verdict, and the loop was
   never told what that verdict is: `node ralph/scripts/check-page.mjs --route <route> [--strict]` runs
   structure, page parity (>=90), widget parity (>=97), snippet language (react = 0), example length
   (>=80% of upstream), attribute density (>=0.8x upstream), copy coverage (>=95%), React mentions (0) and
   the package alias, and prints ONE verdict per route, writing `ralph/logs/scorecard/<name>.md`. An axis it
   could not measure reports UNMEASURED and never passes.
   WHY THIS MATTERS MORE THAN ANY SINGLE SCORE: five axes can be green while the page is not finished —
   accordion had prose at 100% and `react: 0`, and still taught nothing, because its example was 12 lines
   against upstream's 474. Fixing one axis and stopping is how that happened. Work a page until the
   scorecard says PASS, and treat the per-axis reports as the work list that gets you there.
   The same data drives the deployed `/status` page (generated by `ralph/scripts/build-status.mjs`), so
   whatever you leave undone is visible on the site, with your component's name on it.

6g. **If a regression fails and you cannot see how your work could fix it, ASK WHY before trying again.**
   `node ralph/scripts/check-unpassable.mjs --todo-id "<your item>"` compares the bars that are actually
   enforced against the ones your item's `done-when` claims, and prints one of three verdicts:
   * **UNPASSABLE** — a failing gate is on an axis your done-when does not claim, so no amount of correct work
     here turns it green. The recipe is explicit: (a) narrow that clause in `ralph/scripts/run-regression.sh`
     so it matches only the items that claim the axis, (b) split the scope into its own item with its own
     done-when and `blocked-by`, or (c) record a `blocked-reason` naming this verdict. Do NOT re-pick the item
     and try again — that is how five consecutive iterations once produced four ledger-only commits and no
     progress at all.
   * **UNVERIFIABLE** — your done-when claims something no gate measures, so "done" would be an unverifiable
     claim: add the gate, or reword the clause to something checkable.
   * **WORK** — every failing axis is yours. Ordinary unfinished work; keep going.
   `run-regression.sh` now prints this verdict automatically whenever a gate fails, so a red run always tells
   you which of the three you are in.
