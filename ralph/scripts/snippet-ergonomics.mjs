...
import { scoreWithheldReason } from './lib/metric-invariants.mjs';
...
async function main() {
  ...
  try {
    ...
    // ---------------------------------------------------------------------------------------------
    // A PUBLISHED NUMBER IS A CLAIM — so do not publish one this run did not measure.
    //
    // The two rules above exclude blocks from the metrics, but until now the report still PUBLISHED a
    // `score` beside them. Every consumer reads that field as the page's fidelity verdict, so a page whose
    // blocks are upstream's React (or were all excluded) still read as a measured port: MEASURED
    // 2026-09-16, `merge-props` published 28 and `use-render` 8 while their own counters said
    // `reactToReactBlocks 1` and `5` and `blocksExcludedFromScoring 5` of 5 — i.e. every ratio in that
    // number came from an input with nothing in it. That is the "measure that rewards copying" class the
    // owner named, and it is also why `metric-invariants.mjs` flags exactly this shape.
    //
    // Withholding is NOT a soft failure and does not change the verdict: the page still FAILS below (the
    // React-to-React arm returns 1; the all-excluded arm cannot clear the length floor with a null ratio).
    // What changes is that no reader can quote a fidelity number for a page nobody measured. UNMEASURED is
    // the honest state — it never passes and never fails — and the numeric diagnostic goes to the console
    // and the .md for the operator, deliberately NOT into the JSON where a machine consumer would pick it up.
    // ---------------------------------------------------------------------------------------------
    const scoreValue = result.score;
    const scoreWithheld = scoreWithheldReason({
      reactToReactBlocks: reactToReact,
      extractedBlocks: lxTexts.length,
      scorableBlocks: scoringTexts.length,
    });
    if (scoreWithheld) {
      result.score = null;
      result.scoreWithheld = true;
      result.scoreWithheldReason = scoreWithheld;
    }
    ...
    const md = [
      ...
      result.scoreWithheld
        ? `**Ergonomics score: WITHHELD — NOT MEASURED.** ${result.scoreWithheldReason}. The scorer's own arithmetic would have produced ${scoreValue}/100; that number is a diagnostic for whoever fixes the page and must NOT be quoted as a fidelity verdict, which is why the JSON report publishes \`score: null\`.`
    ...
    fs.writeFileSync(path.join(OUT_DIR, `${name}-snippets.json`), `${JSON.stringify({ route, generatedAt: new Date().toISOString(), ...result, upstreamSnippets: upTexts.length, leptosSnippets: lxTexts.length }, null, 1)}\n`);
        ...
        console.log(`snippet ergonomics: ${route} — ${result.scoreWithheld ? `WITHHELD (${scoreValue}/100 diagnostic only — NOT a fidelity verdict)` : `${result.score}/100`}` +
    ...
    if (!lengthOk || (target !== null && scoreValue < target)) {
      console.error(`\nFAIL: ergonomics ${result.scoreWithheld ? `WITHHELD (${scoreValue}/100 diagnostic — no measurement to publish: ${result.scoreWithheldReason})` : `${result.score}/100`}` +
